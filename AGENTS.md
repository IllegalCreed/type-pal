...
- 数据迁移类缺陷具有优先级(用户拍板,2026-07-13):只要问题根因在提取器、迁移器、数据映射、生成脚本或 overlay,必须先修上游并重新生成;只改 `projects/pal` 等生成产物不算完成。不得在上游缺陷未解决时继续堆叠会被下一次迁移覆盖的手工演出修补。
...
- `draft -> build`: 必须有 Codex、Kimi、GLM 三方对设计/风险/验收条件签 `agree`,或由用户明确批准缺签豁免。
