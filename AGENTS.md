...
本文件是 Codex、Claude Opus、GLM 以及后续任何 coding agent 在本仓库协作时共同遵守的根协议。

## 多 Agent 协作协议

本项目把三个订阅 AI 账号作为一个协作团队使用。不要把聊天记录当作唯一真相,可持久追踪的状态必须落在仓库里:

- 工作流: [`docs/ops/agent-workflow.md`](docs/ops/agent-workflow.md)
- 看板: [`docs/ops/board.md`](docs/ops/board.md)
- 任务卡: [`docs/ops/tasks/`](docs/ops/tasks/)

### 核心规则

设计和审核是三方共识工作;编码是单 Owner 工作。

- 非平凡任务在实现前必须通过 **Design Consensus Gate**。
- `implementation` 阶段只能有一个 Agent 负责改实现代码。
- 代码完成后必须通过 **Review Consensus Gate** 才能进入用户验收。
...
| Agent | 主要职责 | 必须参与 |
|---|---|---|
| Codex | 本地实现、测试、集成、git 收口 | 设计共识、审核共识、最终验证 |
| Claude Opus | 架构审查、设计压力测试、风险发现 | 设计共识、审核共识 |
| GLM | 清单覆盖、中文文档、测试矩阵、数据/schema 检查 | 设计共识、审核共识 |
| User | 产品判断、优先级、范围取舍、最终验收 | 阻塞裁决、验收 |

这些是默认分工,不是隔离墙。非平凡任务的设计和审核关卡中,三方都必须给出明确立场。
...
- `Current Owner`: 当前负责推动任务的人。
- `Next Actor`: 当前状态完成后的下一棒。
- `Coding Owner`: `implementation` 阶段唯一允许修改实现文件的人。

### 何时必须走完整流程

以下任务必须走完整看板/任务卡流程:

- 任何触及 `packages/reforge`、`packages/editor`、`packages/content` 或 `docs/phase2` 的改动。
- 任何第一阶段 `packages/game` 行为改动。
- 任何 schema、迁移、存档格式、资产管线、渲染、战斗、编辑器或 UX 工作流改动。
- 任何可能触及多个包的改动。

错别字修正、直接状态问题、简单本地命令可以跳过任务卡;但只要改代码,仍然需要常规验证。
...
这是一个用 TypeScript 重写的浏览器版 **仙剑奇侠传(PAL, 1995 DOS/Win95 RPG)**。第一阶段是对 **sdlpal** 的忠实移植;sdlpal 源码位于 `reference/sdlpal/`,作为游戏行为、公式和渲染数学的权威参考。运行时读取从原版 MKF 二进制档案中提取出的资源。

pnpm workspace,关键包:
- **`@type-pal/shared`** — 第一阶段共享类型和资源工具。
- **`@type-pal/pal-extract`** — 把原版 MKF 解码成 JSON 数据、PNG 资产和事件字节码的 CLI。
- **`@type-pal/game`** — 第一阶段 Vite 浏览器运行时。
- **`@type-pal/reforge`** — 第二阶段干净重写的新引擎。
- **`@type-pal/editor`** — 第二阶段内容编辑器。
- **`@type-pal/content`** — 第二阶段内容 schema 和校验。
- **`@type-pal/migrate`** — 迁移和烤图工具。
...
**Dev server 端口规划**(game 6005 / e2e 6001 / editor 6010 / reforge 6050 起;避开 vite 默认 517x 与 Chrome unsafe ports(6000=X11 会被 `ERR_UNSAFE_PORT` 拒开),已烤进各包 `dev` 脚本 + strictPort):启动命令速查 **[docs/dev-servers.md](docs/dev-servers.md)**。Codex 起验证实例直接复用这些脚本/端口(先探测,活着就复用)。
