...
| D35 | **测试按功能域独立分组,不做真实游戏流程 E2E(推全工程完工最后)** | 用户 2026-05-24 关键澄清:M3.5 之前我一度把"明雷遇怪流程 E2E"(scene jump → 走入草妖 → 进战斗 → 任意打 → won → 回 explore)当 M3.5 主测试,**错**。测试应按**功能域**独立分组:① 场景(loadScene / 角色移动 / 边界 clamp);② 战斗(直接构造 BattleState 测 input wire / 5 actions / 公式 / 未来合体技能);③ 明雷遇怪机制(只测 contact triggerMode → 自动 runScript);④ 探索对话(M2 已建);⑤ 拾取道具 / 开箱 / 触发机关(未来里程碑);⋯。**每组独立维护、独立 fail 排查**。**真实游戏流程的 automated E2E**(跨功能域整链路)推**所有功能域都完工后的最后阶段**做。 → M3.5 内只做 ③ 明雷机制单测 +② 战斗 input wire 单测 +① 场景 loadScene 单测,不做跨功能域 E2E。Dev manual smoke(手动跑 dev server 看视觉)允许并鼓励,不算 automated E2E。M3.5 brainstorm 出:[`plans/2026-05-24-m3-5-scene-encounter-design.md`](plans/2026-05-24-m3-5-scene-encounter-design.md) §5。 |
