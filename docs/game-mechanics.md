...
> 以下相克 / 致死规则**据你口述**;代码侧能看到的痕迹是——毒的 apply 脚本用 `0x5E`(查目标是否已中某毒)配 `0x60`/`0x5F`(秒杀),毒末尾用 `0x2A`/`0x2B`(按 id 解毒)。**精确到每一对 / 每条链的脚本我尚未逐一枚举**,先按你给的规则记录。

- **相克链(单向:新下的毒解掉它所克的旧毒)**:金蚕蛊 解 鹤顶红、鹤顶红 解 血海棠 …… 成环到 孔雀胆 解 金蚕蛊。
  - **方向固定**:金蚕蛊能解鹤顶红,但若**先中金蚕蛊、后中鹤顶红**,鹤顶红**不会**被解(鹤顶红不克金蚕蛊)→ **两毒并存**。
- **三对致死组合(同时中即暴毙)**:① 金蚕蛊 + 断肠草;② 孔雀胆 + 鹤顶红;③ 血海棠 + 三尸蛊。下第二种毒时若已中其配对毒 → **立即秒杀**。
- **可用来秒杀敌人**:给敌人凑齐一对致死毒即可秒。**但下毒命中看巫抗**——所以**巫抗满(10)的 boss 不会中毒,这招对它们无效**(呼应开头"巫抗 ≠ 毒抗")。
- **凑致死对的第二下必须用投掷**(据你):投掷道具脚本带"查配对毒(`0x5E`)+ 秒杀(`0x60`)"的检查;用**三尸咒**(对敌下三尸蛊的巫术)作第二下**无效**——巫术下毒脚本不带这道检查,只单纯加毒。
- **投掷解毒道具不解敌人的毒**(据你):解毒走 `0x2C`(按等级)/ `0x2B`(按种类),两者都**只作用于玩家 / 队伍**;没有"按等级解敌人毒"的 opcode(仅 `0x2A` 能按 id 解敌,且只供毒脚本末尾自解)。所以拿解毒道具投敌,解不掉敌人对应的六大毒。
