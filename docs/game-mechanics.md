...
ts 实现状态(2026-06-13 更新):`actions/flee.ts` 已实现 —— 公式 1:1(含 `(SHORT)` cast / 死敌跳过 / boss 必失败恒消费 RNG)、失败累计吉运经验 +2、成功 16 步滑出动画。敌方抵抗项**照搬原版 bug 用敌人身法**(`be.e.dexterity`),源码注释已标注;若将来要修复版,换 `be.e.fleeRate` 一行即可。
