...
| 0x98 | set follower | ✅A | 数据✅(gs.followers+nFollower)+ 视觉✅(present computeFollowerRenderItems,trail[3+k]/恒3帧/iStepFrameFollower[0,2,0,1],sdlpal scene.c:210-226/732-743/767-771,6 单测)。**operand = MGO sprite chunk 直接**(res.c:335-348 follower 路径,**不**走队员 rgwSpriteNum[role] 查表,res.c:325)→ 临时同行 NPC(scene 102 书生 = chunk 82/83,非 6 人角色表)直接 npcSpriteFrames.get(chunk)。chunk 未载入→跳过(防御)。残:跨场景 follower sprite 持久加载(sdlpal kLoadPlayerSprite)未做→走出该 scene 可能消失,follow-up |
