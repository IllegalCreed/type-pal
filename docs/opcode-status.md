...
| 0x9F | enemy transform | ✅ 变身:非隐身/睡眠 → self.e={...base, health:keepHealth}(summonTables 取 base id op0)(script.c:009F),更新 maxHealth 与 yPosOffset 后的 pos/posOriginal;有 battle bus 时播放 `iColorShift=0..5` 闪色并末帧 47 音效(无 bus 保留 pendingSounds fallback)。battle-opcodes.ts,anim-timeline.ts |
