...
// 9 针（image-import 签名门/battle-background 尺寸门/返回预览误交主图、battle-sprite 10 帧门/
// id 递增、codec 块缓存逐出/来源越界、worker-client 传输副本、video-metadata meta 偏移）+ 3 对照；
...
const cases = [
  ...
  },
  {
    ...
    redTest:
      'battle-background：320×200 + palette 通过并产出 preview；宿主产物与实际 canvas 尺寸/像素一致；尺寸不符拒绝、缺 palette 拒绝',
    expected: 1,
  },
  {
    name: 'returned-preview-replaced-by-main',
    group: 'stages',
    file: 'image-import.ts',
    from: 'effectPreviewBytes = await canvasPng(canvas)',
    to: 'effectPreviewBytes = (await canvasPng(canvas), bytes.slice(0))',
    red: '仍执行第二次编码但返回预览误交主图',
    redTest:
      'battle-background：320×200 + palette 通过并产出 preview；宿主产物与实际 canvas 尺寸/像素一致；尺寸不符拒绝、缺 palette 拒绝',
    expected: 1,
  },
