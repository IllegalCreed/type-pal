...
**后续fast更新**：保存恢复最终收口退役5个旧作者模块、将旧命令/复制测试迁至当前模型后，ratchet仍为**6,385项**（editor205文件/2,146项），质量门详见[最终收口](editor-save-recovery-closeout.md#质量门与剩余分支)。
当前613生产文件；全仓行47,935/68,763（69.71%）、语句53,083/78,549（67.58%）、函数10,033/14,397（69.69%）、分支38,005/61,633（61.66%）。editor行79.15%、语句76.73%、函数75.04%、分支68.62%。
本次与7087dbad的测试总数相同，不代表测试清单没变：旧模型用例退役/迁移并补当前行为回归，仍存在的commands.test.ts保持114项；源码/测试退休单列，不把分母减小全称新增覆盖。首次微小加权行下降及中间测试计数保护均被门禁拒绝，未降低基线。
