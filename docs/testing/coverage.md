...
即使各自受控并发，两套重型进程仍会争抢 CPU；E-06 的独立完整检查为 194 文件 / 1,755 项通过。
...
[质量门禁修复](../ops/audits/pre-e2e/quality-gate-remediation.md)新增 15 项用例：6 项原生标签关联、
1 项目录行角色/选择状态切换，以及 8 项迁移写入计划守卫与无变化发布回归。
`coverage:ratchet` 验证 608 个生产文件、486 个 fast 测试文件 / 5,690 项测试，所有包四项指标均未回退；
本次未重跑 full coverage，前文 Full 表仍为首次基线，不把普通完整 check 当成 full 覆盖率报告。

| 当前 Fast 全仓 | 精确计数 | 展示值 |
|---|---:|---:|
| Lines | 47,079 / 68,387 | 68.84% |
| Statements | 52,072 / 78,182 | 66.60% |
| Functions | 9,859 / 14,375 | 68.58% |
| Branches | 37,330 / 61,578 | 60.62% |
