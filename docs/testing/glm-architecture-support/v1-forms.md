...
- **V1-002 covered（r2 撤回 risk 并更正）** 多选弹层合同 = DS-C.5a 即时提交离散动作 + DS-C.6 Esc 关
  选择层（规范条款比对，见顶部更正②）；与 quick-trial 草稿模型是两类控件合同，无需产品裁决。
  无 placeholder 保留为独立可发现性小建议（非 accessible name 缺失）。
- **V1-003 covered（r2 撤回 risk 并更正）** 撤回"过滤输入无可访问名"：multi-select.tsx:119-121
  `aria-label=搜索${label}`；Codex 6010 实测 AX 名与过滤行为一致。r1 的 ARIA 快照判读有误（教训入
  自检：判"缺名"前须先查组件源码的 aria-label 透传）。
- **V1-004 N/A** 360px 只记录既有边界（V0/工作包纪律），未测，不提新要求。
- **V1-005 covered（r2 补齐）** 新增 6 张截图：角色 v1-06、物品 v1-07、战斗技能+效果链 v1-08、
  敌队五槽 v1-09、战场 v1-10、敌队试打弹窗 v1-11（打开→Esc 关闭验证）。布局层 covered；
