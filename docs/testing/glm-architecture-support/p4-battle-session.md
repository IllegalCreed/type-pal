...
| 两级资源屏障 | `beginTurnPreparation` :603-632（token=preparationSerial）+ `settleTurnPreparation` :575-601（五重守卫 token/closed/doneSettled/phase/ui）+ `writeBackHp` :604 preparing 早退 | 屏障期间 tick :1202 提前 return 锁全部输入；资源失败 SfxReadinessResourceError 降级 / fatal 停留 |
...
|---|---|---|
| battle-session.test.ts（**37 条 test(**） | 屏障/token/迟到 resolve、终态、hook、逃跑、成长写回 | `pending 后取消会作废旧 token；迟到 resolve 不得推进 core`（:646）、`done 已 resolve 时，迟到的屏障回调不得再推进 core`（:755）、`合击消费其余队员后仍先冻结完整动作快照，再进入行动`（:690）、`endBattle terminate…`（:1284）、`fleeBattle…`（:1405）、`固定成长定位失败时在任何 mutation 前 fail-loud`（:1526） |
...
**与 P4 差异**：无——P4 不新增动态证据（避免与 battle-workflows 卡重复取证），只做所有权图与
拆分建议；静态新发现见下。

## 5. 证据条目

- **P4-001 covered** 上表 87 条精确标题（37+46+4）。
...
- **P4-004 N/A** 两级屏障与取消语义已有 87 条测试钉住，本包无新增反证——如实不发明缺陷。
