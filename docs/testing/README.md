...
- [STAT-1覆盖率合并根因](coverage-initializer-diagnosis.md)、[机账](coverage-initializer-evidence.json)、[原生最小复现](coverage-initializer-probe.mjs)与[raw捕获/离线复算](coverage-initializer-capture.mjs)（修复卡draft；同range双初始化身份混并，12组与旧1378对照，未安装patch/未改官方基线）
