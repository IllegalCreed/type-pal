...
- [试放方案调整：共享战斗模拟器](../ops/tasks/EDITOR-SKILL-TRIAL-1-isolated-battle.md)与[原技能入口前提探针](skill-trial-premise.mjs)（rework；r2首批草案与交互草图已备，待UI/保存选择；旧r1不授权新范围）
- [共享战斗模拟器全域复用评估](battle-simulator-assessment.md)（8模块27子页：7直接/8上下文/12专用验证；含预设、配置边界与三批接入建议，未实施）
- [战斗模拟器r2首批实施草案](battle-simulator-r2-design.md)（四目录、工程内配置、事务/撤销/重开、独立运行与验收矩阵；待产品确认与三席准入，非已实现）
