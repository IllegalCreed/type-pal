...
| `c4-sprite-asset-in-use` | `精灵资产 ${this.asset} 仍被定义引用` → `void 0` | 重叠保护：红侧仍 AssertionError+exit1，但实际抛后层 `资源 sprite.generated.starter 仍被 1 处引用`，不是定义引用文案 |
| `c5-enemy-profile` | `敌人只能引用 enemy profile` → `void 0` | expected function to throw |

## 验证

作者自验，不能替代 Codex 独立接收。Status 保持 build。产品相对 `7cac1d72` 空 diff。本段计数以本轮新鲜 JSON 为准，提交前覆写。

- `pnpm --filter @type-pal/editor typecheck` exit 0。
- 改动 Biome：5 个 TS 文件 `biome check` 0 error（fixture、C2/C3/C5/C6 residual）。
- `git diff --check` 干净。
- `node scripts/docs/check.mjs`：`docs: PASS (0 issues)`（620 Markdown / 3326 local links / 199 tasks）。
- 负控 `--self-test` 通过；五针新鲜目录 `/var/folders/f3/8n7sqr293cl0rtxknfv8x4sc0000gn/T/cursor-command-boundaries-r3-mutants-xH2Ggw`：`count=5`、`allOk=true`、`allHit=true`、`allRedExit1=true`、`allHashUnchanged=true`。C4 红侧仍为重叠保护文案。仓内已跟踪 JSON 未回写。
- 定向+相邻 JSON `/tmp/cursor-command-boundaries-r3-directed-1790430222.json`：11 files / **52** tests exit 0。其中残项 **7 files / 29 tests**；相邻 4 files / 23 tests（`actor-commands.test.ts`、`tileset-lifecycle.test.ts`、`asset-label-command.test.ts`、`commands-wave2.actor.test.ts`）。
- 全 editor JSON `/tmp/cursor-command-boundaries-r3-editor-1790430682.json`：**331 files / 2858 tests** `success=true`（`vitest run --passWithNoTests --maxWorkers=2`）。本 worktree 本地依赖 + gitignored 环境内 PAL `projects/pal/assets/{runtime,migrated}` 符号链接，指向既有主仓资产，不入 Git，不改用户工程，不动 6010。相对准入 324/2829，多出的 7 文件 / 29 项即本包 residual。
