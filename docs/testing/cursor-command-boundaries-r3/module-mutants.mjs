...
const cases = {
  c1: {
    id: 'c1-actor-covered-by',
    sourceFile: 'src/core/actor-commands.ts',
    testFile: 'src/core/actor-commands.residual.test.ts',
    from: `    if (
    ...
    to: 'void actor',
    title: 'UpdateActor 只改 coveredBy 为不存在人物 → 援护者不存在',
    fullName: '人物命令残项 UpdateActor 只改 coveredBy 为不存在人物 → 援护者不存在',
  },
  c2: {
    id: 'c2-zone-facing',
    sourceFile: 'src/core/entity-commands.ts',
    testFile: 'src/core/entity-commands.residual.test.ts',
    from: 'throw new Error(`触发区 "${this.sceneId}/${this.entityId}" 无朝向`)',
    to: 'void 0',
    title: 'UpdateEntity facing 对触发区抛无朝向，输入态不变',
    fullName: '实体命令残项 UpdateEntity facing 对触发区抛无朝向，输入态不变',
  },
  c3: {
    id: 'c3-tileset-empty-name',
    sourceFile: 'src/core/tileset-commands.ts',
    testFile: 'src/core/tileset-commands.residual.test.ts',
    from: "throw new Error('瓦片集名称不能为空')",
    to: 'void 0',
    title: 'UpdateTilesetMetadata name 空白 → 瓦片集名称不能为空',
    fullName: '瓦片集命令残项 UpdateTilesetMetadata name 空白 → 瓦片集名称不能为空',
  },
  c4: {
    id: 'c4-sprite-asset-in-use',
    sourceFile: 'src/core/sprite-commands.ts',
    testFile: 'src/core/sprite-commands.residual.test.ts',
    from: 'throw new Error(`精灵资产 ${this.asset} 仍被定义引用`)',
    to: 'void 0',
    title: 'DeleteUnusedSpriteAssetCommand 删除 hero 资产 → 精灵资产仍被定义引用',
    fullName: '精灵命令残项 DeleteUnusedSpriteAssetCommand 删除 hero 资产 → 精灵资产仍被定义引用',
  },
  c5: {
    id: 'c5-enemy-profile',
    sourceFile: 'src/core/battle-sprite-commands.ts',
    testFile: 'src/core/battle-sprite-commands.residual.test.ts',
    from: 'throw new Error(`敌人只能引用 enemy profile：${this.definitionId}`)',
    to: 'void 0',
    title: 'AddEnemy 后 SetEnemyBattleSprite(starter-fighter) → 敌人只能引用 enemy profile',
    fullName:
      '战斗精灵命令残项 AddEnemy 后 SetEnemyBattleSprite(starter-fighter) → 敌人只能引用 enemy profile',
  },
}
...
const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cursor-command-boundaries-r3-mutants-'))
...
fs.writeFileSync(
  ...
  `${JSON.stringify(
    {
      kind: 'cursor-command-boundaries-r3-module-mutants',
