...
2. **继续其余批次**：建立独立worktree分支 `codex/glm-pre-e2e-prep`，以含本包r1且packages对10c84238零diff
   的main提交为起点，记录完整SHA。若已有同名分支先核归属，不能reset/覆写或恢复旧stash。
   若main产品已前进，停止从活跃主树取代码，找最近满足冻结条件且含本包的提交；记录证据，不自行合并Codex实现。
