...
取代下文2026-09-22的覆盖率优先排期。见[当前讨论稿](e2e-route-proposal.md)；
本文2026-09-21的缺陷状态/行号不是当前全量审计结论，R4准入仍须针对当前树核所选路径。
固定三席已按现行AGENTS暂停，不继续用旧三签条款阻塞。
...
排期更新（用户2026-09-22）：**先补测试覆盖率，E2E不着急**。下方R4准备清单保留为后续工作，当前不立即
扩展检查点/runner实施；先做GLM第二波及Codex不重叠补测。此裁决是优先级调整，不是取消E2E/N6b，也不是
签字豁免或要求全仓先达90%/85%。所有未完成项继续保留，不能因后置写成已通过。
