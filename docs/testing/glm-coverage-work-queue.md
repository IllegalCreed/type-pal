...
未达whole-file目标和无现行caller等残项单列；后续按最新7790/633基线另选批次，不重复领取已交内容。
...
Codex独立补测/复核双线推进，领取前核不重叠及现行caller；不因0覆盖就给无消费者的导出函数补“续命”测试。
不扩大产品修复范围、不取消签字、不降低门槛，也不把长期90%/85%新设为薄E2E强制前置。

2026-09-22新分配：[第二波六组25模块](glm-coverage-wave2.md)已按7538项/633文件的最新基线重新筛选；GLM先交逐族准备，再由Codex/Kimi并行审设计，三签后整包实施。下面TB00～TB10仍为已done历史，不重新领取；新任务不复用旧签字或空窗期豁免。

**当前状态（2026-09-21）**：TB00/TB01候选44b9b763已三席accept齐、用户授权后分别done归档，见[收口记录](tb00-tb01-completion.md)。TB02～10此前已分别收口，见[看板历史入口](../ops/board.md)。下文顺位、计数及交接为规划/首轮接收时点，不是仍待返工或可再次领取的任务；不新增实施授权，frame在途invalidate及full/Q1/Q2边界保持。

现有一批11模块继续走原卡；另备十批78个互不重复的候选模块。每批都是可连续做的整包，
不是每写一个测试就请用户签字。GLM交一批、Codex审一批，同时提前准备后面的批次。

2026-09-19首轮接收状态（历史）：[TB-02～10九批统一接收](glm-nine-intake-review.md)已完成当轮独立复核，九批分别counter/rework；205定向、原73针+27对照、tc通过，但当时不满足证据/范围/格式验收，未合并、未改官方7049基线。
用户本轮明确批准额度空窗期九批先行实施，原“两批待接收”限制不追溯阻挡这九个交付；不把该例外泛化为未来新范围授权。
当时TB00/TB01另排、返工优先；后续两卡已完成，见上方当前状态。设计三签不重开（TB02/03 r2，TB04～10 r1已齐）。用户已确认Mimosa为GLM私有MCP，Codex无需处理、不作门禁；测试与质量门保持。
下表保留规划时冻结计数，不等于本轮已接收覆盖。
...
1. **返工优先**：有本人的counter先闭环，不能靠领新包绕过证据问题。TB00已按
   [原任务卡](../ops/archive/tasks/done/TEST-RUNTIME-STATE-BOUNDARIES-1-state-and-metadata.md)与[39族工作包](glm-runtime-state-boundaries.md)完成并收口，不再领取或扩展其r1。
