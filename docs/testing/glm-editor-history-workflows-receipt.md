...
> 以下GLM正文保留冻结树历史，不代表主线最终证明强度。Codex接收候选为远端
> `1f043a66970a77987c012af8a4875ba608223a54`，不是文末旧amend前SHA。
> 接收适配与勘误见文末；已删除测试中的旧App回退路由和整文件ts-nocheck。
...
## Codex 接收与主线适配（2026-09-13）

- 独立复核后接收为**测试贡献**，不作为GLM独立终审自证。主线不merge冻结产品或覆盖他席签字；仅接入测试/回执和机械README索引。
- 删除手写historyOwnerRef fallback，全部真实undo/redo直接走当前Owner；P12/P20改Vite源码raw导入，移除node fs/grep及整文件ts-nocheck。编辑器typecheck通过。
- 修正P09：第三次只撤正文，seed创建私有脚本仍在，正确为price=0/body=[]，不是私有脚本消失。
  冻结树失败位置是第二撤价格（原回执写第一次不准确）；P10两次撤销终点明确price=0/ms=null，撤掉对象toBeTruthy弱断言。
- snap补完整两侧内容、redo与通知/历史版本；P11据完整内容比对恢复。P13/P14在尝试redo前核双侧canRedo=false，redo返回false且状态不变，不再用if(revived)掩盖缺口。
- P04/P05使用正式实体放置工厂；P16从clean状态故障注入，核全状态/dirty/版本/历史不变后，关闭故障立即重试同一对命令成功；不以中间插入其他编辑当失败保全证据。
- P17主/脚本订阅均有见证，每次都必须观察完整price=66/body=wait6；失败零通知。P19补真正两步undo/两步redo及各步价格/正文断言后再序列化重开。
- P20明确只证明入口标记和数量，静态grep不能证明任意命令对象无复用。Codex独立caller审查：App五处和ItemTab两处每次操作新建命令；其它传入命令均即时派发，未见跨动作保留实例。Root新建双session及Owner，生命周期另由timeline/App/真实Root回归证明。
- 最终适配定向20/20通过（`/tmp/type-pal-history-main.WcgNcg/glm-adapted-first.log`）；合并Codex timeline24与foundations14，共58/58绿。
  Codex另以git对象隔离加载复跑原测试1f043a66/四核心dded6f27，独立得到15绿/5红（`glm-frozen-recomputed.log`）；
  P12/P20直接读取主线相同七caller形状，只作为静态入口核对，不混作冻结产品动态证据。
  同一最终树隔离五针反控：order10红、split26红、future2红、pop3红、save6红。不是每项都错误放行：
  order部分被局部索引守卫拒绝；future被全局日志重叠防护挡住，只以另一侧historyVersion未失效鉴别；save含4错误放行和2下游拒绝层位变化。
  配置与逐针日志在同目录negative.config.mts/nc-*.log；未改磁盘产品/原审计探针。全仓质量门以主卡最终回执为准。
