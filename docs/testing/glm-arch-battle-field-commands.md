...
任务卡：[ARCH-F2-EDITOR-BATTLE-FIELD-COMMANDS-1](../ops/archive/tasks/done/ARCH-F2-EDITOR-BATTLE-FIELD-COMMANDS-1.md)。
状态：**Codex独立accept，done归档**。实现候选`83833719`由GLM贡献，主线统一验证和收口由Codex完成；
下列GLM自验按历史保留，最终独立证据见文末。本卡与ARCH-REGRESSION-LAB剩余counter分开裁决。
...
`git diff --name-status f5f166aa..83833719`（Codex接收时校准为提交后的最终树）：

```text
A	docs/testing/glm-arch-battle-field-commands.md
M	docs/testing/README.md
A	packages/editor/src/core/battle-field-commands.test.ts
A	packages/editor/src/core/battle-field-commands.ts
M	packages/editor/src/core/commands.ts
```

合计 commands.ts 13 insertions / 206 deletions；其余为上列两个新源码/测试文件与两份文档。
`packages/content`、`reforge`、`main/App/MapMode/ScriptEditor`、schema/SAVE8、
...
本节为GLM交付时的权限边界；当前收口状态见文末Codex验收。

- 本回执不更新任何官方覆盖率基线/计数；不宣称行数减少为收益。
- 不合 main、不标 done；Codex 接收后统一串行全仓 check→ratchet→受保护 strict-fast 与最小功能核验。

## Codex 最终独立验收（2026-09-26）

**accept / done。** 原203行正文（含末尾换行）与新模块末段精确相等：7276 bytes，SHA256
`8a56bbfcc9dd487a92363195deb92fcc475a60cb02a9a1478ea34165ec0351d1`；AST公开出口119→119，
TypeScript擦除类型后的本地依赖闭包13模块，无回到commands的运行期路径。156/156、typecheck、Biome绿。
本人用Vite加载钩子直接删除引用阻断门、把Add.invert单点改为return state，原commands.test的对应
D24用例各执行1/失败1、exit1/AssertionError；对照8/8，源文件未改。该证据独立于GLM已删除的mock脚本。

最新主线串行`pnpm check`（8657项）、`coverage:ratchet`、以`7a18eaa6`为保护基线的单次严格
`coverage:fast`均exit0。fast8165项/644文件（+6测试/+1源模块）；四项覆盖率分子和分母与拆分前完全相同，
未宣称架构搬移提高覆盖率。日志：`/tmp/codex-bfc-check.log`、`/tmp/codex-bfc-ratchet.log`、
`/tmp/codex-bfc-strict.log`。

隔离浏览器实测新建#058、名称/火属性修改、复制#059、undo/redo保真；默认#024删除被引用阻断。
IAB原生确认框操作接口阻塞，单列为工具限制；随后使用仅替换`window.confirm`宿主边界的6014
测试页（产品源码不变）验证真实删除与撤销恢复，同一编号条目消失/恢复，测试新增最终撤销回原52项。
原生确认框自身的自动化交互未计作通过；未向磁盘保存测试编辑。

本次只完成战场命令族窄拆。其它架构债、ARCH-REGRESSION-LAB剩余工作独立推进；无下一位Agent提示词。

集成提交`22a4492c`已推送main；原候选保存于远端`archive/glm-arch-battle-field-commands-r1`标签。
原独立worktree及本地/远端贡献分支已清理，可从该标签恢复历史。
