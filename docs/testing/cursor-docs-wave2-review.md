...
日期：2026-09-25。正文候选`3852afe58137b5f1b5dddddbdfb56cd12006a0b0`，
登记tip`e3da44ca9a655586e360c3682b9fd9dd282ed19b`；证据冻结`dab017e7`。
[Cursor原回执](cursor-docs-wave2.md)原样保留，不由Codex代改贡献者结论。

## 结论

**counter，仅回执R1～R4；不要求重新执行十二组或重算已通过的资源散列。**
候选满足一文件白名单，事实表大部分可采信；但“6条确定不符”含三条未排除替代解释的误报，
一条替换句仍承诺目标文档并未提供的内容，部分引用/工作目录/未核分类需校准。
这是取证材料返工，不是发现四个产品bug。原DOC-CURSOR-1 accept保持；五份指南修订卡未开门。
...
Cursor只修回执R1～R4，不重新跑已通过项，交新正文SHA/登记tip；可复制提示词见
[DOC-CURSOR-2本人席位](../ops/tasks/DOC-CURSOR-2-package-tools-indexes.md)。
