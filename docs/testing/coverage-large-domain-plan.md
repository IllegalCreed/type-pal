...
2026-09-23，用户批准从零散补测改为优先完整业务流程。当前只是两批 **draft 设计**，不是绕过三签的实施授权。
...
当前Codex已签两卡本人设计席。GLM与Kimi可并行核同一r1，两卡分别裁决；必须至少一位非Owner独立取证。三席齐后仍由Codex统一核build，不因用户说“开始”默认豁免。完整两席提示词见各卡。
