...
| C9 | 商店 BuyMenu / SellMenu | ⚠️ partial | ✅ unit | uigame.c:1615 `PAL_BuyMenu` + 1755 `PAL_SellMenu` | core/menu/shop-menu.ts + menu-driver.ts applyShopTransaction + present/menu/draw-shop.ts | commit 3f5b644 全栈 1:1 接入(修曾伯卡主流程):状态机 + **真扣金/真改背包**(买 cash-=price+AddItem / 卖 cash+=price/2,menu-driver.ts:220)+ draw-shop 真渲染 + ITEMBOX 预览 + confirm 框。shop-menu.test.ts。残:卖菜单紧凑布局而非 sdlpal fullscreen PAL_ItemSelectMenu(功能等价)。残(2026-06-02 对抗复核 high-conf REAL-WORK medium):卖菜单走 sdlpal PAL_SellMenu(uigame.c:1777)= **全屏 PAL_ItemSelectMenu**(itemmenu.c:379-466),非紧凑;ts buy/sell 当前共用紧凑 drawShopMenu。**user 决策不简化 → 本轮补全屏**(复用现成 drawInventoryMenu fullscreen 渲染器 + cash/price overlay)|
