...
| D18 | 战斗 UI + **选择动作菜单**(InfoBox + 主菜单/二级/目标光标) | ⚠️ partial | ✅ partial | uibattle.c 12 函数 + `PAL_BattleUIUpdate` kBattleMenuMain | present/battle/draw-battle-ui.ts(渲染)+ battle-system.ts tickSelectAction `handleMainMenuInput`/`handleMagicMenuInput`/`handleItemMenuInput`/`handleTargetSelectInput`(输入) | **战斗菜单已做**:主菜单 5 项(攻击/法术/物品/防御/逃跑,`>` 高亮)+ 法术/物品二级菜单 + 目标 ▽ 光标 + HP/MP + 队员状态栏。输入经 mode.ts:64 `tickBattle` 真喂、可导航 Up/Down/Confirm/Cancel(**非 stub**)。draw-battle-ui.test.ts 11 + battle-system.test.ts 菜单输入序列。残:① 法术/物品二级**只前 5 个**(无滚动)② targetSelect **只敌方目标**(友方治疗目标不可选 → 治疗法术 / 0x5A 等"对队员" opcode 不可达,根因同 item 队员目标路由)③ 无 sdlpal `R 重复上次动作` / 自动战斗 ④ 投掷物已接 throwable→throw-item 路由(2026-05-30) |
