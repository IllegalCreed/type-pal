...
| # | 功能 | 状态 | 测试 | sdlpal 真值出处 | ts 路径 | 备注 / 差异 |
|---|---|---|---|---|---|---|
| D1 | 战斗启动 StartBattle | ✅ claimed | ✅ partial | battle.c:1531 `PAL_StartBattle` | core/battle/battle-system.ts startBattle + bootstrap startBattleHandler | battle-system.test.ts;opcode 0x07 全链通。enemyTeam→enemies.json id 映射**已对齐**(2026-06-02 对抗复核 high-conf):两级 OBJECT→wEnemyID 间接在提取期完成(enemy-teams.ts translate,buildObjectIndexToEnemyIdMap),enemy-teams.json 存已解析 enemyId(扫全 380 team:0 槽落 OBJECT 区 398..550),运行时直 find 即真值,非简化。原 T23 对拍待作废 |
