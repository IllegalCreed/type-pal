...
| D17 | **战斗动画/演出**(攻击/法术/受击/死亡/敌 idle/伤害数字) | ⬜ todo | ⬜ todo | fight.c `PAL_BattleShowPlayer{Off,Def,Summon}MagicAnim`/`PreMagicAnim`/`ShowPostMagicAnim` + 6 ShowMagic*Anim + rngplay.c | core emit + present/battle/present-battle.ts + draw-battle-sprites.ts | **present 层基本 stub**:① present-battle.ts:91 **全跳过** `playPlayerAttack`/`playEnemyAttack`/`playMagicAnim`/`flashEnemy`/`flashPlayer`/`playEnemyDeath`(攻击/法术/受击/死亡动画都没画)② draw-battle-sprites.ts:111 敌人**只画 frames[0] 静态**(无 idle 帧轮播)③ **伤害数字**:present-battle.ts:82 有消费 `showDamageNum`,但所有 emit 点(performAttack/E1 magic)写死 **x:0,y:0**(画在角落非目标头上)+ 0x42/0x66/0x21/毒 tick 因 BattleCtx 无 bus **根本不 emit**。法术效果精灵(MAGIC.MKF/magic-sprite)提取另见 resource-status |
| D18 | 战斗 UI + **选择动作菜单**(InfoBox + 主菜单/二级/目标光标) | ⚠️ partial | ✅ partial | uibattle.c 12 函数 + `PAL_BattleUIUpdate` kBattleMenuMain | present/battle/draw-battle-ui.ts(渲染)+ battle-system.ts tickSelectAction `handleMainMenuInput`/`handleMagicMenuInput`/`handleItemMenuInput`/`handleTargetSelectInput`(输入) | **战斗菜单已做**:主菜单 5 项(攻击/法术/物品/防御/逃跑,`>` 高亮)+ 法术/物品二级菜单 + 目标 ▽ 光标 + HP/MP + 队员状态栏。输入经 mode.ts:64 `tickBattle` 真喂、可导航 Up/Down/Confirm/Cancel(**非 stub**)。draw-battle-ui.test.ts 11 + battle-system.test.ts 菜单输入序列。已补(2026-05-30):① 法术/物品二级菜单**滚动窗口**(f889cf1,全法术可选)② **AoE 法术/攻击跳过选目标**(magic.type AttackAll/Whole/Field/Party/Summon + attackAll 武器 → 直接全体,对齐 FIGHT_DetectMagicTargetChange)③ 投掷物 throwable→throw-item 路由。残:targetSelect 仍**只敌方目标**(友方治疗目标不可选 → 治疗法术 / 0x5A 等"对队员" opcode 不可达)+ 无 sdlpal `R 重复上次动作` / 自动战斗 |
