...
| D12 | 战斗逃跑 PlayerEscape | ⚠️ partial | ✅ regress | battle.c:1438 `PAL_BattlePlayerEscape` + fight.c:4119-4148 | core/battle/actions/flee.ts + battle-system.ts | **⚠️ 公式 str 简版**(2026-06-01 审计):flee.ts:37 `str = role.fleeRate` 用 **raw 字段**,sdlpal 真值是 `PAL_GetPlayerFleeRate(role)` = **装备感知** getter → 装备逃跑加成被忽略(flee.ts:7 自注 M3 简化)。BOSS 不许逃(flee.ts:32 对齐 fight.c:4143)。**2026-05-31 真机修**:① 改**全队逃**(commitFleeAllPlayers,sdlpal fFlee fight.c:1773-1799,原只逃一人=没用)② 补**逃跑动画**(tickBattleFleeAnim,16 步右下滑 p0+4/+6·p1+4/+4·p2+6/+3 + 移出屏,battle.c:1438-1527;commit f451cb7/5b44fed)。残:音效45(M6)|
...
| # | 功能 | 状态 | 测试 | sdlpal 真值出处 | ts 路径 | 备注 / 差异 |
|---|---|---|---|---|---|---|
| L1 | 炼丹系统(紫金葫芦 + 灵葫咒) | ⬜ todo | ⬜ todo | script.c case 0x34/0x33 + 0x64/0x60 + fight.c collect | — | **玩法(user 2026-05-31 明确)**:① 战斗中敌人 HP 低于阈值 → 灵葫咒可**秒杀**该敌 + 积累**灵葫值**;② 大世界用紫金葫芦**消耗灵葫值制作丹药**。映射:灵葫咒 scriptOnSuccess=0x64(jump if enemy HP>阈值% → 高于则失败跳走,低于才往下)+ 0x60(KO 秒杀)+ 0x33(collectEnemy → gs.wCollectValue += collectValue = 灵葫值);紫金葫芦 scriptOnUse=0x34(大世界消耗 wCollectValue 产丹药)。**0x33 collectEnemy 已实现**(battle-opcodes.ts:536,非 default-skip);缺的是 0x34/0x60/0x64。灵葫仙丹(纯治疗 0x1D)已能跑(2026-06-01 审计订正"全 default-skip"措辞)|
| L2 | 蛊虫 / 练蛊皿系统 | ⬜ todo | ⬜ todo | script.c case 0x66/0x33/0x34 + fight.c PAL_BattleThrowWeapon | — | **玩法(user 2026-05-31 明确)**:① 战斗中向敌人投掷蛊**附身**(poison-like);② 若干回合后获得**更强力的蛊道具**;③ 炼蛊皿大世界使用,**消耗蛊制作高级蛊道具**。映射:蛊 flags.throwable → 战斗投掷 0x66(+ 0x28 apply poison 附身 + 毒 tick 若干回合);炼蛊皿(id 207)scriptOnUse=0x20。**0x66 throw-weapon + 0x28 apply-poison 基础 opcode 已在**(battle-opcodes.ts:81/87);真 gate = 蛊升级链 + 炼蛊皿组装(非"全 opcode 未做",2026-06-01 审计订正)|
