...
> **最后更新**:2026-06-06 — 大世界主循环/脚本演出复核:对齐 `PAL_StartFrame` 顺序(trigger zone → autoScript → blocker push → chase timer → party update → menu/search),补阻挡 NPC 压到 party anchor 时四向推离,并订正 `sState` / `loadScene` 旧注释。战斗演出复核:订正 2026-06-02 的 Trance 不可达旧结论,梦蛇 object-magic 295 可触发 Trance,已接 PreMagic 前摇/闪色变身/PlayerAll 即时提交。此前 2026-06-02: **user 指令「剩余非音频功能都做」+「对不上现实的备注及时清理」**:先 4-agent 逐条 source/code 核验 + **15-agent 对抗复核(refute 全 held,high-conf)**,订正 stale 残注 —— D1(映射非简化,提取期已解析)/D21(0x30 已写 Extra)/D3(crit 无独立视觉,仅音 M6)/D26(scriptOnBattleEnd resume 已做,仅剩 2 处可见 inline 等键)/M5(magic 菜单已接 role.magic)/C-cash(已集成测)/E2·E章(战斗 E 类全收口非剩 26)/G2(present 测已有挂 G13)/K3(RNG mirror 软链已服务·WORD regress 已有)/opcode 0x92(非正确 no-op,待 wire);应用 user 裁定 D22→verified·F3→close(旧档不兼容)·J2/J3/J4→N/A(非原版)。confirmed REAL-WORK 待做:C9 卖菜单全屏·L1/L2 物品框·0x92 wire·G8/B2 测·WORD content-pin。此前同日:过时「残」注清理(4-agent 核验 workflow,逐条 source 复核):**11 处过时残注订正**(D19 palette cycle + D20 战斗精灵中毒色 = N/A;D2 R重复 + D16 聚拢动画 + D24 隐身效果 + D25·D26 0x90/0x79 + D8 silence灰显 = **已实现注过时**;D2 equip-battle action = sdlpal 不存在)。**真残留保留 2**:D8 confused 走入精灵动画 / D11 hidden-exp 数字 x 偏差 15px。状态列留 user 拍板,本次只订正残注 prose。此前 2026-06-02 物品系统专项(蛊孵化链 / 战斗单目标治毒 / 大世界状态 / _item 重标注 / EnemyMagic iBlow / 0x31 精灵替换)。
...
| B2 | NPC 跟随队伍 trail | ✅ claimed | ✅ | scene.c:779 `PAL_UpdateParty` | core/event-system.ts partyWalkTo + present.ts | event-system.test.ts trail;进场景 0x46 setPartyPos 预填 rgTrail[0..4](commit f159e32);**多步 trail 时序**(连走 4 步 unshift 累积 leader 历史 cap5 + follower 滞后跟随,event-system.test.ts B2)+ follower 单帧位置(present/follower-render.test.ts) |
| B3 | NPC 自动行走 autoScript | ✅ claimed | ✅ partial | script.c:3482 `PAL_RunAutoScript` + play.c:169-228 | core/event-system.ts:962 tickAutoScripts + core/scene-system.ts | 见 E3(已搬到 event-system);0x00/0x01/0x02/0x03 控制流全 port + goto 同帧续跑(commit eaaa1d5 修张四划船);1 op/tick 本就是 sdlpal 真值。2026-06-06:explore 主循环按 `PAL_StartFrame` 顺序在 trigger 后、party update 前跑 autoScript,并补 blocker NPC 压到 party anchor 时的四向推离。event-system / scene-system 有 autoScript 与推离断言 |
| B4 | 场景切换 door / trigger zone | ✅ claimed | ✅ partial | play.c:81-166 fTrigger 段 + play.c:534 StartFrame | core/scene-system.ts updateEventObjectsAndTrigger | mode-dependent Manhattan threshold + NPC 转向 + 触发后进入脚本;2026-06-06 对齐帧序:走进 trigger zone 后下一帧触发,菜单/Search 在 party update 后处理。多处转场黑屏根因已修(9791497 失败兜底解冻 / 9293dac onEnter cutscene 门控)。见 M3 |
...
| B8 | tilemap 遮挡 cover tile | ✅ claimed | ✅ partial | scene.c:77-180 `PAL_CalcCoverTiles` | present/draw-tilemap.ts addCoverTileEntries | 5×5 scan + iTileHeight bit;队员 + 0x98 follower 都接入。present.test.ts P0.b cover-tile 4 例(2026-06-02 核验测试列⬜过时) |
...
| B10 | 295 个 scene 资源加载 | ✅ claimed | ✅ partial | res.c:191 `PAL_LoadResources` | assets/loader.ts | loader.test.ts;无 295 scene 全 fixture。loader.test.ts loadAll 404 + SceneAssetsCache 缓存/透传(2026-06-02 核验:测试列⬜过时,success path 拉20+JSON 不另测) |
...
| C4 | 物品使用菜单 ItemUseMenu | ✅ claimed | ✅ partial | uigame.c:1289-1473 `PAL_ItemUseMenu` | core/menu/inventory-menu.ts use-target + present/menu/draw-inventory.ts | inventory-menu.test.ts use-target phase + draw-inventory fullscreen(itemmenu.c:28-466 1:1);**INNER while loop 已加单测**(menu-driver.test.ts C4 describe:revert use-target + count→0 auto-cancel,2026-06-02,此前仅 user 实测)。残:无 |
...
| C9 | 商店 BuyMenu / SellMenu | ✅ claimed | ✅ unit | uigame.c:1615 `PAL_BuyMenu` + 1755 `PAL_SellMenu` + itemmenu.c:28-466 `PAL_ItemSelectMenu` | core/menu/shop-menu.ts(买)+ core/menu/sell-menu.ts(卖,全屏)+ menu-driver.ts dispatchSellMenu/applyShopTransaction + present/menu/draw-shop.ts drawSellOverlay | commit 3f5b644 全栈接入 + **C9 卖菜单全屏(2026-06-02)**:买仍紧凑(sdlpal PAL_BuyMenu 本就紧凑,忠实);**卖改全屏 PAL_ItemSelectMenu(kItemFlagSellable)** —— SellMenuState 复用 InventoryMenu grid(8-key 导航)+ drawInventoryMenu(noDesc=g_fNoDesc TRUE,itemmenu.c:413→不画描述)+ PAL_SellMenu_OnItemChange overlay(金钱@100,150 / 售价@224,150,uigame.c:1709-1752)+ confirm(默认No)→ AddItem(-1)+cash+=price/2+refreshSellGrid。真扣金/真改背包 menu-driver.ts。sell-menu.test.ts 6 + shop-menu.test.ts 卖 e2e。残:无 |
| C10 | 9-slice 边框 box 渲染 | ✅ claimed | ✅ partial | ui.c:131-240 `PAL_CreateBoxWithShadow` | present/menu/draw-box.ts | draw-box.test.ts |
| C11 | 中文字体渲染 Unifont | ✅ claimed | ✅ partial | font.c PALFONT 字模 | present/font.ts | font.test.ts;Unifont CN stroke 跟 sdlpal 原 PALFONT 字模不同。**by-design**:PALFONT 字模属版权资产不提取,Unifont 永久架构替代(同 AVI→mp4 模式),渲染算法已 1:1 |
...
| C15 | 确认框 ConfirmMenu(否/是) | ✅ claimed | ✅ partial | uigame.c:242-365 `PAL_ConfirmMenu` | present/menu/draw-confirm.ts + event-system.ts(waiting='confirm') + menu-driver.ts | drawConfirmBox 共享渲染:否(130,100)/是(205,100),默认否。两处消费:opcode 0x0A goto-if-no(commit 5c208d4)+ 商店 confirm phase。primitives 状态机 + event-system 0x0A goto-if-no + draw-menu 渲染 + shop confirm 覆盖(2026-06-02 核验测试列⬜过时) |
| C16 | 三选菜单 TripleMenu | N/A | ✅ unit | uigame.c:320 `PAL_TripleMenu` | core/menu/primitives.ts createTripleMenu | primitives.test.ts 有纯状态原语,但**未接 menu-driver / 无 draw 层 / 运行时无消费者**(预留)。关联 C2 quit 二次确认。**by-design**:唯一 caller 在 #if PAL_HAS_CONFIG_PAGE(PAL_CLASSIC=FALSE 编译掉),primitive 已 port+测,无 runtime caller |
| C-cash | 金钱面板 drawCashBox | ✅ claimed | ✅ partial | uigame.c:451 `PAL_ShowCash` | present/menu/draw-menu.ts inline | 残:无(2026-06-02 对抗复核:drawCashBox 已 drawMenuStack 集成测覆盖 draw-menu.test.ts:40/79,vitest 6/6;抽独立 export helper 仅代码组织偏好,无功能缺口 N/A) |
...
| # | 功能 | 状态 | 测试 | sdlpal 真值出处 | ts 路径 | 备注 / 差异 |
|---|---|---|---|---|---|---|
| D1 | 战斗启动 StartBattle | ✅ claimed | ✅ partial | battle.c:1531 `PAL_StartBattle` | core/battle/battle-system.ts startBattle + bootstrap startBattleHandler | battle-system.test.ts;opcode 0x07 全链通。enemyTeam 的 OBJECT→wEnemyID 两级间接在提取期完成;enemy-teams.json 的 `enemies` 存属性用 enemyId,`enemyObjectIndexes` 保留每槽原对象号供运行时精确挂 AI 脚本。解决同一 enemyId 多 OBJECT_ENEMY 变体取错脚本的问题。 |
| D2 | 5 actions(攻/技/防/逃/物) | ✅ claimed | ✅ partial | fight.c:3577 `PAL_BattlePlayerPerformAction` | core/battle/actions/* | actions.test.ts;5 基础 action 真做 + **throw-item action 已接**(performThrowItem + 投掷物路由)。**(2026-06-06 复核订正)R 重复 prevAction 已做并修跨战斗致死回合、换队错位**(commitRepeatAction + backupRepeatActions;按 fight.c:1427-1437 在 actionQueue 填充前备份,Repeat 本轮不覆盖缓存;跨场按 roleId 保存并重映射我方目标,队伍人数/顺序变化不再串用其他角色动作);**战斗物品菜单已补 ITEMBOX + BALL 图标 + scriptDesc 说明**(复用 itemmenu.c 布局与大世界说明解析);**summon 已做**(buildSummonGodSequence);**equip-battle action sdlpal 任何 build 不存在**(battle.h:49-60 enum 无该项);**trance 独立 action = N/A**(sdlpal 无该 action,梦蛇走 object-magic 295 的 Magic type Trance,见 D17)。注:0x6B BlowAway 已写 iBlow 状态(opcode), 但**吹飞位移视觉**待 D17 |
| D3 | 物理伤害公式 | ✅ claimed | ✅ partial | fight.c:131-289 三公式 + 3636-3748 perform | core/battle/formulas.ts + actions/attack.ts | formulas.test.ts。**B6(2026-05-31)收口玩家物理攻击全套真值**:**单体**(fight.c:3636-3663)base + RandomLong(1,2) jitter → crit(RandomLong(0,5)==0‖bravery ×3)→ 李逍遥 role0 RandomLong(0,11)==0 ×2 → ×RandomFloat(1,1.125) → max(1);**群攻**(fight.c:3681-3748)crit 整轮一摇 + 命中序 {2,1,0,4,3} + division 逐敌减半(无 jitter/float);**DualAttack 双击**(fight.c:3628/3681 t-loop)装备授(仙女剑等)→ 整段做两次。actions.test +11 例(rng rangeFloat / scriptedRng / forceFloat 控制)。残:**enemy→player** 的 str+RandomLong(0,2)/+RandomLong(0,1)/fAutoDefend evade(归 D27残/B2);crit **声音** criticalSound 归 M6;**crit 无独立视觉**(2026-06-02 对抗复核:fight.c:2008-2263 fCritical 仅选音 + ×3 伤害,无 crit-only 闪;通用每击受击闪 iColorShift=6 已做 anim-timeline.ts:212)。**P0#1 修(2026-06-02 第三方审计核源)**:玩家物攻 str 此前误加 `(level+6)*6`(M3 把敌方公式套玩家)→ 已删,改 `str=role.attackStrength`(=PAL_GetPlayerAttackStrength,global.c:1757-1764 base+装备无 level;role 已投影含装备故不双计)。敌方分支 (level+6)*6 是真值不动 |
...
| D16 | 协力法术 CooperativeMagic | ✅ claimed | ✅ unit | fight.c:3856-4043 / global.c:2013 | core/battle/actions/coop-magic.ts | **执行收口**:performCoopMagic — healthy contributors / **HP 代价**(非 MP,costMP)/ str=Σ(atk+mag)/4 / applyMagicDamage(minDamage=1)。**装备 override**(佛珠/圣灵珠改合击)经 rgwCooperativeMagic 末非 0 槽 override 投影进 role.cooperativeMagic(equip-effect + projectRuntimeToBattleRoles)。**非召唤聚拢动画已实现**:buildCoopMagicTimeline(7 阶段 port fight.c:3856-4107)。**召唤型协力已接入**(2026-06-06):武神等 `magic.type='summon'` 走 PreMagic→全员变亮→召唤神 F.MKF → 二次 FIRE → PostMagic→fadeOut,修巫后装备覆盖成武神时只响音效无动画。12 单测 |
| D17 | **战斗动画/演出**(攻击/法术/用物/受击/死亡/敌 idle/伤害数字) | ✅ claimed | ✅ partial | fight.c `PAL_BattleShowPlayer{UseItem,Off,Def,Summon}MagicAnim`/`PreMagicAnim`/`ShowPostMagicAnim` + `PAL_BattleFadeScene`(battle.c:608) + idle 帧 | core/battle/anim-timeline.ts + battle-anim-driver.ts + battle-positions.ts + present/battle/* | **2026-05-30 时间线架构全套补齐**:**D17a**(d42a58f)per-fighter render state(pos/posOriginal/currentFrame/iColorShift,battle.h 25fps)+ `state.battleAnim` 时间线驱动 tickPerformAction(逐 BattleAnimFrame ~40ms 推进)+ 物理攻/受击动画。**D17b**(2873312)伤害数字:BattleCtx 透 bus + 全 HP-mutate opcode emit `showDamageNum` + 真值坐标(精灵底锚 offset)/ 颜色(blue 损 / yellow 回 HP / cyan 回 MP)/ 11 帧寿命(drawNumber UI sprite)。**D17c**(413f9d3)敌人 idle 帧闭式轮播(computeIdleFrameIndex)。**法术动画**:9f412b7 攻击魔法链 PreMagic→OffMagic→PostMagic(FIRE.MKF,4 落点)/ a199e16 DefMagic 治疗辉光 + EnemyMagic 敌方攻击魔法镜像。**UseItem 前摇已接(2026-06-06)**:buildUseItemTimeline port fight.c:2266-2335(Delay(4)、使用者前移 frame5、sound 28、目标队员 colorShift 0..6..0),并通过 battleAnim.afterComplete 保证动画后才跑 scriptOnUse / consuming 扣库存。**死亡淡出**:4b56296 + 9669a47 PAL_BattleFadeScene 逐像素 rgIndex 抖动(72×16ms),修"先消失再淡出"。**iBlow 吹飞 / wWave 屏波 / keepEffect 烙背景 已做**(W4,见 D19;EnemyMagic iBlow 2026-06-02 补)。**音频已接(M6,2026-06-03,见 H2)**:攻击/法术/受击/死亡各演出点的 AUDIO_PlaySound 全套(出招/暴击/武器/windup 28·29/法术效果/敌 cast/敌 action·call 帧同步/濒死/阵亡)。**Summon 召唤神演出已做(2026-06-01 审计订正)**:anim-timeline.ts:24 `buildSummonGodSequence`(port fight.c:3072-3187 — 主角隐去 + 召唤神精灵 F.MKF chunk `wSummonEffect+10` 淡入逐帧 + 72 步 dither crossfade + bgColorShift)+ magic.ts:303 dispatch(`magicType==='summon'`→buildSummonGodSequence,伤害 inline 全体结算)+ 召唤神精灵 chunk 已提取(commit 94cc1e0)。**此前"Summon 完全没做/F.MKF 未提取"备注已过时**。**Trance/梦蛇已接入(2026-06-06)**:object-magic 295 回退解析 magicNumber=47,按 PAL_BattleShowPlayerPreMagicAnim 前摇 + 0x30/0x31 success 脚本 + iColorShift 0..10 闪色,末帧切 sprite;重复施放仍播放前摇,友方全体目标即时提交不画 PlayerAll 箭头。残:Trance 末尾 PAL_BattleFadeScene 级别场景淡换待抠。实引擎 chrome-devtools 数据级验证多轮 |
| D18 | 战斗 UI + **选择动作菜单**(InfoBox + 主菜单/二级/目标光标) | ✅ claimed | ✅ partial | uibattle.c 12 函数 + `PAL_BattleUIUpdate` kBattleMenuMain | present/battle/draw-battle-ui.ts(渲染)+ battle-system.ts tickSelectAction `handleMainMenuInput`/`handleMagicMenuInput`/`handleItemMenuInput`/`handleTargetSelectInput`(输入) | **战斗菜单已做**:主菜单 5 项(攻击/法术/物品/防御/逃跑,`>` 高亮)+ 法术/物品二级菜单 + 目标 ▽ 光标 + HP/MP + 队员状态栏。输入经 mode.ts:64 `tickBattle` 真喂、可导航 Up/Down/Confirm/Cancel(**非 stub**)。draw-battle-ui.test.ts 11 + battle-system.test.ts 菜单输入序列。已补(2026-05-30):① 法术/物品二级菜单**滚动窗口**(f889cf1,全法术可选)② **AoE 法术/攻击跳过选目标**(magic.type AttackAll/Whole/Field/Party/Summon + attackAll 武器 → 直接全体,对齐 FIGHT_DetectMagicTargetChange)③ 投掷物 throwable→throw-item 路由。**友方目标选择已做**(07d51e2:targetSide 域按 magic.flags.usableToEnemy / item kItemFlagApplyToAll 分敌/友方,治疗法术 / 治疗物品可选队友 + 修 perform 目标域硬编码 → "对队员" opcode 可达)。**目标光标真值修(2026-05-31 真机 + 27-agent workflow 核对,commit 2c15596)**:敌方目标**无箭头**,改选中敌人精灵 ColorShift 7 闪烁高亮(uibattle.c:1495-1510);补**当前行动队员**头顶箭头(CURRENTPLAYER 68/69,DX-8/DY-74,uibattle.c:994-1007)。颜色全项经核对对齐无 bug。残:无 —— 自动战斗(0x8A fAutoAttack)+ R 重复均已做;**友方死目标重选 = N/A**(2026-06-02 核验:sdlpal 友方治疗目标由玩家 UI 选,无死目标自动重选,选死队员则治疗 no-op,与 sdlpal 一致)|
| D19 | 战斗背景 wave / cycle / fade | ✅ claimed | ✅ partial | battle.c:34 `PAL_BattleDrawBackground` + 609 BattleFadeScene | present/battle/draw-battle-bg.ts + draw-battle-sprites.ts(死敌 fade) | 背景静态全屏 blit;**BattleFadeScene 已用于敌人死亡淡出**(D17/D20,逐像素 rgIndex 抖动)。**W4 战斗演出视觉(2026-06-02,3 commit)**:① wWave 屏波(magic 动画帧带 screenWave→applyScreenWave 扭曲场景,fight.c:2667/2895)② iBlow 吹飞(player off-magic 逐帧吹全体活敌 (blow,blow/2) 末复位,fight.c:2681-2694;gated iBlow!=0)③ keepEffect 烙背景(末帧 0xFFFF&&wave<9→overlays 入 persistentBgBlits 持久画 bg 上,fight.c:2757-2762)。**EnemyMagic 演出对齐(2026-06-02)**:敌方魔法 wWave/keepEffect 早已做,**iBlow 抖队员补齐**(buildEnemyMagicTimeline 镜像 OffMagic,吹全体队员 (blow,blow/2) 末复位,fight.c:2901-2909;magic.ts 注入 iBlow/blowTargets=全队/rng)。战斗**入场 fade-in 已做**(introFade,W5)。残:palette cycle = **N/A**(2026-06-02 核验:PAL_CLASSIC 战斗主循环 battle.c:769-798 无 palette cycle;BATTLEFIELD struct global.h:377-380 无 cycle 字段);Trance/梦蛇末尾场景淡换待抠(见 D17)|
| D20 | 死敌 colorShift / 中毒紫色 | ✅ claimed | ✅ partial | battle.c:505 `PAL_BattleDrawAllSpritesWithColorShift` + 608 BattleFadeScene | present/battle/draw-battle-sprites.ts blitFrameDeathFade + blitFrame(iColorShift) | **死敌渐隐已做**(4b56296+9669a47,deathFadeStep 0..72 逐像素 rgIndex={0,3,1,5,2,4} 抖动 + RG_INDEX_INV)+ **iColorShift 受击变白 blit**(palcommon.c:398-411 低 nibble +shift)。**中毒/死亡头像染色已做**(1de0444,uibattle.c:114-162:PAL_PlayerInfoBox 最高 level≤3 毒 wColor mono / 死亡 mono0,computePlayerFaceColor)。残:无 —— **战斗精灵本体中毒色 shift = N/A**(2026-06-02 核验:sdlpal iColorShift 只走攻击/法术动画,无任何 call-site 把毒色写进战斗精灵体;毒色仅头像 infobox,已做 1de0444);**Trance colorShift cycle 已接**(梦蛇 object-magic 295 触发,见 D17)|
| D21 | 战斗结束 status / 毒 / Extra 清 | ✅ claimed | ✅ unit | battle.c:1822-1830 + global.c:2311 `PAL_ClearAllPlayerStatus` | core/battle/battle-system.ts finalizeBattleCleanup | **B1(2026-05-31)**:commit 42eda6b。finalizeBattleCleanup 无条件(won/lost/fleed/forced)每队员 curePlayerPoisonByLevel(3)(清持久 rgPoisonStatus,毒等级上限 3=全清)+ removeEquipmentEffect(Extra=6)。ClearAllPlayerStatus:ts status battle-local 随 battleState 丢弃→自动满足。battle-system.test。残:无(2026-06-02 对抗复核:0x30 已改写 rgEquipmentEffect[6] Extra slot + 战末 removeEquipmentEffect(Extra) 清,见 opcode-status 0x30 / D14;原"0x30 不写 Extra"过时) |
| D22 | 偷盗 StealFromEnemy | ✅ verified | ✅ unit | fight.c:5193-5298 `PAL_BattleStealFromEnemy` 全段 + 含 Bug-2 | battle-opcodes OP_STEAL_FROM_ENEMY + anim-timeline buildStealTimeline + battle-system tickBattleDialog 守卫 | **逻辑**(2d6f98d)+ **偷窃动画**(buildStealTimeline)+ **居中框提示(2026-05-31 二修)**:偷取成功"获得 X"改推 `battleDialogQueue` style='narration'(=kDialogCenterWindow 居中单行阴影框 drawNarrationDialog,跟**大世界"获得XX"同一 UI**),非 banner —— CLASSIC 真值 fight.c:5267-5296 `PAL_StartDialog(kDialogCenterWindow)+PAL_ShowDialogText`,banner 那条在 `#ifndef PAL_CLASSIC`(text.c:1671)。`@` toggle 红(text.c:1504);WORD34 获得/WORD10 文钱。**排序**:tickBattleDialog 加 `!battleAnim` 守卫 → 结果框在偷窃动画播完后才显示。**user 2026-06-02 真机验过 → verified** |
...
| D24 | 战斗隐身 0x5C iHidingTime | ✅ claimed | ✅ unit | script.c:1907-1911 `g_Battle.iHidingTime = -op0` | core/battle/battle-opcodes.ts OP_HIDE_PARTY + battle-state.ts | commit 444d307:BattleState.iHidingTime 字段 + 0x5C 写入(state.iHidingTime=-op0)真做。**隐身战斗效果已做(2026-06-02 核验订正)**:sdlpal 真值是 iHidingTime>0 时敌方**整轮 goto end**(fight.c:1716),**非**原注"敌不可选队员为目标"(误述);battle-system.ts:1508-1509 整轮跳过 turnStart 脚本 + activate/decrement(port fight.c:1670-1672)。原注已订正 |
...
| D26 | 战斗内对话(scriptOnReady / scriptOnTurnStart 0xFFFF showDialog) | ✅ claimed | ✅ partial | text.c:1660-1772 PAL_ShowDialogText(CLASSIC,#ifndef PAL_CLASSIC 战斗飘字编译掉)+ 1701 DialogWaitForKeyWithMaximumSeconds(1.4) | core/battle/battle-system.ts tickBattleDialog + event-system.ts(runScript battle showDialog 入队)+ present/battle/present-battle.ts | commit 74334e3:boss 嘲讽对话(蜘蛛精/拜月/灵儿/林月如等 30 敌脚本)。原 ts 只 emit 不渲染的 showBattleMessage stub。**忠实 CLASSIC:battle dialog 走普通 dialog box(复用大世界 gs.dialogBox 渲染),覆于战斗场景**。runScript 同步无法跨 tick 阻塞 → showDialog 入 state.battleDialogQueue,tickBattleDialog(phase-agnostic hold)逐 tick 喂进 gs.dialogBox(打字 100ms cadence / 翻页 / 等键 / narration 1.4s)+ 暂停战斗。触发见 D10(turnStart 行动前 / scriptOnReady 出场)。battle-dialog 9 + event-system 3 + 集成 2 例。**2026-06-06 订正**:scriptOnBattleEnd 旧实现虽会跑脚本,但结算后立刻 finalize 会清掉战后脚本排入的"获得X" narration;现改为结算屏后 runPostScripts → tickBattleDialog 播完 → Phase F 半血恢复 + finalize。残(2026-06-02 对抗复核):collect-then-replay 丢 dialog↔非 dialog 间 inline 等键 —— runScript 同步收集架构:扫全 ~840 battle 脚本,105 处 dialog→raw→dialog 中 102 是 0x05/0x8E ClearDialog(TS battleDialogPendingClear 已正确保边界),**仅 2 处真插可见 effect**(蛇女灵儿 obj502 turnStart@41060 插 0x69 逃跑动画 / 林月如二@41386 插 0x77 停乐=音 M6)。**0x69 已做(D26(2b),2026-06-03)**:dialog 序列中的内联 effect 入队为 effect 条目(BattleDialogLine.effect),runScript 收集时**仅 queue 非空才 defer**(空则立即跑),tickBattleDialog 按位置 dispatch(set enemyEscapeAnim)→ 嘲讽对话 → 逃跑动画(tickBattleEnemyEscapeAnim 优先 hold)→ narration → fleed,忠实 sdlpal 脚本序。battle-dialog.test(effect 消费+顺序)+ event-system.test(defer/立即跑)。**0x77 停乐已接(M6,2026-06-03)**:OP_STOP_MUSIC 设 gs.wNumMusic=0 → shell 见 track 0 停 BGM(林月如二@41386 停乐生效;fade-out 时长 cosmetic 待补)|
...
| F3 | 读档从 IndexedDB | ✅ claimed | ✅ regress | global.c:689 `PAL_LoadGame_WIN` + 888 `PAL_ReloadInNextTick` | core/save/api.ts loadSlot + shell/bootstrap.ts loadGameFromSlot | C8 读档 IO + scene reload;OpeningMenu/SystemMenu/opcode 0x4E 共享 handler;eecfbc4 修读档卡死。残:无(**user 决策 2026-06-02:旧存档全抛弃不兼容**,未发布无真档;新档已含 allEventObjects,无旧档边界。fallback bootstrap.ts:626 仍在位兜底但非必需)|
...
| G2 | FBP 整图 blit | ✅ claimed | ✅ partial | palcommon.c:651 `PAL_FBPBlitToSurface` | pal-extract dump FBP → png | M4 P3 提取层(经 cli extract 全量 dump,misc-mkf 解码已测);运行时全屏显示/滚动见 G13。**present 视觉测已有**(2026-06-02 对抗复核:fbp-player.test.ts 7 例 framebuffer 级 — fade/dither/scroll/effectSprite,挂 G13;原测试列 ⬜ 过时)|
| G3 | sprite color shift | ✅ claimed | ✅ partial | palcommon.c:245 `PAL_RLEBlitWithColorShift` | pal-extract io/rle.ts | rle-decode test;runtime 死敌/状态 colorShift ⬜(见 D20)|
| G4 | tilemap 双层渲染 | ✅ claimed | ✅ partial | map.c 全 6 fn | present/draw-tilemap.ts | draw-tilemap.test.ts |
| G5 | palette 加载 / 切换 | ✅ claimed | ✅ partial | palette.c:25/93 + 0x8B / 0x53·0x54 | event-system.ts:2077 setPalette + 2765 昼夜 flag | 0x8B `_fetchPalette` 异步注入 + 0x53/0x54 昼夜 flag(commit 8fe20e4)。夜色经 resolveNightColors 在 fade target 生效。event-system.test.ts setPalette + nightPalette 状态测(2026-06-02 核验) |
...
| G8 | SceneFade 72 帧 dither(0x73/0x9B) | ✅ claimed | ✅ | video.c:1130 `VIDEO_FadeScreen` | present.ts:505-542 + event-system.ts | 72-step nibble dither(backupPixels 快照);0x9B 复用 fadeState。注:0x93 SceneFade 改走 palette ramp(buildSceneFade)。dither 数学 dither-fade.test.ts;**present 层 e2e present.test.ts:G8**(snapshot clear 前 + elapsed 0→36→72 步进 + 显示 backupPixels) |
...
| G12 | RNG.MKF 动画播放(0x36/0x37) | ✅ claimed | ✅ partial | rngplay.c:371-448 `PAL_RNGPlay` | shell/rng-player.ts + event-system OP_SET_RNG/PLAY_RNG | commit 8872b54:0x36 set iCurPlayingRNG(instant)+ 0x37 阻塞 modal 逐帧 PNG 直写 fb + Space 跳过(suspendRaf);全 12 chunk 提取。**2026-06-06 修 0x50→0x37 首帧淡入**:PAL_RNGPlay 消费 fNeedToFadeIn,恢复 base/night palette 后以 RNG 第一帧从黑淡入,修山神庙酒剑仙传剑 CG 实际已加载却全黑。rng-player/event-system test。开场 trademark DOS 亦复用 |
...
| # | 功能 | 状态 | 测试 | sdlpal 真值出处 | ts 路径 | 备注 / 差异 |
|---|---|---|---|---|---|---|
| H1 | BGM(MIDI/CD) | ✅ 待听验 | ✅ unit + build | audio.c + midi.c:67 + battle.c:728/1849/1032 | core 意图 + shell/audio.ts + shell/audio-midi.ts(SpessaSynth) | opcode 0x43/0x45/0x77/0xA3 → wNumMusic/wNumBattleMusic/musicLoop;战斗 BGM 切换(pickMusicTrack:战斗→wNumBattleMusic、退出→场景乐);**战斗胜利曲**(battleVictoryTrack:'won' 且 expGained>0 → isBoss?2:3 不循环,battle.c:1030-1032;结算完场景乐恢复);0x77 停乐→wNumMusic=0(shell 停,fade 时长 cosmetic 待补)。**播放后端 = SpessaSynth 运行时 MIDI 合成**(WorkletSynthesizer + Sequencer,直接播 Musics/{NNN}.mid,开箱即响)。typecheck + vite build + 单测过;**user 前置:放 GM soundfont 到 public/soundfont.sf3**(缺则 BGM 静默+warn,见下 §soundfont)。残:soundfont + per-track 听验 |
| H2 | SFX sound.c | ✅ 待听验 | ✅ unit | sound.c + fight.c(全 AUDIO_PlaySound 调用点)+ battle.c:1397/1459 + script.c:1708/2937/2980 | core(0x47→gs.pendingSounds / 战斗各 action push / bus playSound + frame.sound 帧同步)+ shell sfxForBattleEvent + bootstrap drain | **战斗 SFX 全套接全(逐 sdlpal AUDIO_PlaySound 核源,2026-06-03)**:explore 0x47 / 我攻起手 attackSound(暴击换 criticalSound,fight.c:2065/2069 每击)+ 命中 weaponSound(2124)/ 法术 windup 28(2300,普通+召唤)+ 合击 29(3875)+ 法术效果 magic.sound(2501)/ 敌攻 attackSound(4934)+ 接近 actionSound(5003)+ 命中 callSound(5084,**帧同步** frame.sound,非挤一帧)/ 敌施法 cast 音 magicSound(4695)/ 敌死 deathSound(756)/ 玩家濒死 dyingSound(834-850)+ 阵亡 deathSound(4816/4851/5110,集中 tickPostAction sweep)/ 召唤 212(script 2937)+ 变身 47(2980)/ 投掷符卵蛊·武器 0x42·0x66 OffMagic 音 / 混乱攻友军 weaponSound(3810)/ 逃跑 45(battle.c:1397/1459)。**N/A(忠实,2026-06-03 字节级/源级 + 对抗复核确认)**:① sound 0(fight.c:1122 战斗胜利 `AUDIO_PlaySound(0)`)—— SOUNDS.MKF chunk 0 **字节级实测 0 长度空槽**(offset[0]==offset[1]==2024;505 chunk 中 142 个为空,0 是其一),sdlpal 自身 `SOUND_Play` 无 `==0` 特判但 `sound.c:778 len<=0 return FALSE` → 胜利那帧本就不出声;真正胜利声是 H1 音乐 2/3(battle.c:1032)。我方 `soundId>0` 守卫与 sdlpal `len<=0 return` 行为一致(接 0.wav 反成 bug,文件不存在)。② 出手提示 78(uibattle.c:618,全仓唯一调用点)严格在 `#ifndef PAL_CLASSIC`(609-620)块内 → CLASSIC build 预处理器编译期删除该行;基准三重锁 PAL_CLASSIC(common.h:25 默认 + pal-classic-on.patch 强制 + build 脚本校验),ENABLE_REVISIED_BATTLE 全仓无定义;ATB/WIN95 修订特性,原版回合制无此叮(M3 计划硬约定「#ifndef PAL_CLASSIC 分支不实现」)。残:听验 |
...
| J2 | 鼠标 | N/A | N/A | input.c:436 `PAL_MouseEventFilter` | — | **user 决策不做(非原版功能)** |
| J3 | 手柄 / Joystick | N/A | N/A | input.c joystick 全组 | — | **user 决策不做(非原版功能)** |
| J4 | 触屏 移动支持 | N/A | N/A | input.c touch 全组 | — | **user 决策不做(非原版功能)** |
...
| # | 功能 | 状态 | 测试 | sdlpal 真值出处 | ts 路径 | 备注 / 差异 |
|---|---|---|---|---|---|---|
| M1 | 主循环 渲染/逻辑解耦 | ✅ claimed | ✅ partial | main.c `PAL_GameUpdate` 帧语义(等价) | shell/main-loop.ts | commit 3ebf1cb:逻辑 tick 恒速(explore 100ms / battle 40ms),不被 scene 淡入提速;dither/paletteFade 进行中每 raf present(wall-clock 平滑)。修香兰 cutscene 走入/对话被加速 6×。main-loop.test.ts advanceRafFrame 三不变量(logicInterval/clamp/present门控)+ tickN replay(2026-06-02 加测,顺修 clamp while-后死代码 bug) |
| M2 | suspendRaf modal 独占 canvas | ✅ claimed | N/A | —(浏览器特有,SDL 阻塞等价) | shell/main-loop.ts onPresent + bootstrap suspendRaf gate | commit a01cd73:modal 播放器(AVI/trademark/splash/RNG/FBP/结局)期间主循环 present 跳过,修闪烁。含阻塞 fade/colorFade/WaitForKey 工具(ending-player.ts,有 ending-player.test.ts)供 modal 编排。测试列 ⬜ = suspendRaf 是 bootstrap 闭包 1 行 browser-infra,by-design 不孤立单测 |
| M3 | scene 转场淡入门控 + 失败兜底解冻 | ✅ claimed | ✅ partial | scene.c:503-508 `PAL_MakeScene` fNeedToFadeIn | core/event-system.ts:515 tickSceneAutoFadeIn | commit 9293dac/ef70491/9791497/39b5848:onEnter cutscene 黑屏真因=自动淡入门控旧码 explore-only → 对齐 PAL_GameUpdate;scene-load 失败兜底解冻 sceneLoading(架构根因);0x05 redraw 对齐(修仙灵岛靠岸黑屏)。event-system.test.ts tickSceneAutoFadeIn + mode.test.ts sceneLoading gate(2026-06-02 核验) |
