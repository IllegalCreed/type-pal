...
| C9 | 商店 BuyMenu / SellMenu | ⚠️ partial | ✅ unit | uigame.c:1615 `PAL_BuyMenu` + 1755 `PAL_SellMenu` | core/menu/shop-menu.ts + menu-driver.ts applyShopTransaction + present/menu/draw-shop.ts | commit 3f5b644 全栈 1:1 接入(修曾伯卡主流程):状态机 + **真扣金/真改背包**(买 cash-=price+AddItem / 卖 cash+=price/2,menu-driver.ts:220)+ draw-shop 真渲染 + ITEMBOX 预览 + confirm 框。shop-menu.test.ts。残:卖菜单紧凑布局而非 sdlpal fullscreen PAL_ItemSelectMenu(功能等价)。**by-design(visual-only)**:残=卖菜单紧凑布局非 sdlpal fullscreen,扣金/背包/confirm 全测,功能等价 |
...
| C-cash | 金钱面板 drawCashBox | ✅ claimed | ✅ partial | uigame.c:451 `PAL_ShowCash` | present/menu/draw-menu.ts inline | 残:抽 helper 后单测便于覆盖 — 留 follow-up。draw-menu.test.ts 经 drawMenuStack 集成覆盖 cash box(2026-06-02 核验) |
...
| F3 | 读档从 IndexedDB | ⚠️ partial | ✅ regress | global.c:689 `PAL_LoadGame_WIN` + 888 `PAL_ReloadInNextTick` | core/save/api.ts loadSlot + shell/bootstrap.ts loadGameFromSlot | C8 读档 IO + scene reload;OpeningMenu/SystemMenu/opcode 0x4E 共享 handler;eecfbc4 修读档卡死。残:旧档(无 allEventObjects)NPC sState 可能不齐 —— **fallback 已就位**(bootstrap.ts:626 sliceSceneEventObjects 返 undefined 则保留存档 npcs,2026-06-02 核验);窄旧档边界,未发布无真档,重建可选 |
...
| G2 | FBP 整图 blit | ✅ claimed | ⬜ todo | palcommon.c:651 `PAL_FBPBlitToSurface` | pal-extract dump FBP → png | M4 P3 提取层(经 cli extract 全量 dump,misc-mkf 解码已测);运行时全屏显示/滚动见 G13。测试列 ⬜ = FBP 整图 present 视觉,逻辑由提取层覆盖 |
