...
一句话:**普攻练武术/体力,法术练灵力/真气,防御练防御,逃跑(失败)练吉运** —— 玩家听到的说法方向正确。但有 4 个容易踩的坑,见下方 [重要约束](#4-重要约束容易误解的点)。
...
> **用户补图核对（2026-09-05）**：群体普攻对每个敌人按命中顺序应用100%、50%、25%、12.5%、6.25%的
> 递减系数。三张图补充首个目标的物抗公式、五敌站位标号及其余四个系数。本次以原始站位数据和SDL源码
> 核对，纠正本节旧的空间顺序文字；没有运行原版实机重演。

这里指**我方攻击全体敌人的普通物攻**，不是全体仙术或合击。
源 [fight.c:3676-3748](../../reference/sdlpal/fight.c#L3676-L3748)。

### 各目标分别算基础伤害，再按命中序递减

沿用[百分比口径](#伤害公式的百分比口径与属性收益)，令`B_k`为对第k个命中目标按其有效防御算出的基础伤害，
`r_k`为该目标的有效百分比物抗，截图未计暴击/取整的公式为：
...
通式：伤害ₖ ≈ [Bₖ ÷ 2 × (1 − rₖ) × 暴击倍率] ÷ 2^(k−1)
```

每个目标的防御、物抗独立取值，**不是把上一个敌人的实际伤害除2后交给下一个敌人**。
敌人属性不同，后一个实际数字不一定恰好是前一个的一半；相同属性也还受整数扣血边界影响。
这里100%是该目标**未递减的群攻基准**，不代表等于带额外随机项的单体普攻伤害。
物抗百分比不是原始除数字段，特别是原始除数0的分支，仍按上文换算说明处理。

### 五敌标准站位与图中标号

命中顺序按固定源槽序`{2,1,0,4,3}`，不是运行时按屏幕距离重新排序。
原始`data/extracted/data/enemy-pos.json`的`layouts[4]`对应下表；坐标为320×200逻辑画面中的基准底锚，
实际另加敌人自己的Y偏移（[battle.c:936-939](../../reference/sdlpal/battle.c#L936)）。
截图的“1～5号位”是**命中顺序标号**，不能直接当作原始数组下标。

| 命中序 | 源槽（零基） | 五敌基准坐标 | 图中大致位置 | 递减系数（取整前） |
|---|---:|---|---|---|
| 第1个 | 2 | (160,100) | 右侧，近玩家 | 100%（÷1） |
| 第2个 | 1 | (100,110) | 中间 | 50%（÷2） |
| 第3个 | 0 | (70,140) | 下方 | 25%（÷4） |
| 第4个 | 4 | (90,80) | 上方 | 12.5%（÷8） |
| 第5个 | 3 | (30,110) | 左侧 | 6.25%（÷16） |

### 空槽、双击和取整边界

- `division`每次扫描从1开始，每处理一个参与的敌槽后乘2。空槽/已清槽及超过最大槽位者跳过，
  **不占递减次数**；不足五敌时仍按源槽顺序扫描，并按实际参与数递减。
- SDL只检查`wObjectID==0`和最大槽位，**不检查HP**（[fight.c:3698](../../reference/sdlpal/fight.c#L3698)）。
  首次横扫打倒但尚未清槽的敌人，在同一动作的后续横扫中仍可能参与、使除数翻倍。
  不能把“已清槽”与“HP刚降为0”混为一谈。
- 每次横扫只掷一次全体共享的暴击，触发时×3，发生在除division之前。
  **没有**单体普攻的随机+1/+2、1～1.125末尾浮动，也没有李逍遥额外会心×2。
  双击执行两次横扫，**各自重置division并重掷暴击**（[fight.c:3681-3688](../../reference/sdlpal/fight.c#L3681)）。
- 递减后不能先把伤害向下取整：SDL用FLOAT除division，非正伤害置1，然后才回写整数HP
  （[fight.c:3709-3726](../../reference/sdlpal/fight.c#L3709)）。第一阶段对应实现也是先扣血再取整。
  例如`314/4=78.5`，HP2000回写为1921，实际掉79，而不是先截成78再扣。
  因而表中的比例是取整前系数，不保证最终整数数字严格成等比数列。

第一阶段实现锚点：[attack.ts:159](../../packages/game/src/core/battle/actions/attack.ts#L159)命中序、
[:172-199](../../packages/game/src/core/battle/actions/attack.ts#L172)每次横扫/取整/倍增。
已有测试覆盖空槽、未清槽阵亡、递减及双击（[actions.test.ts:326](../../packages/game/src/core/battle/__tests__/actions.test.ts#L326)、
[:362](../../packages/game/src/core/battle/__tests__/actions.test.ts#L362)、[:418](../../packages/game/src/core/battle/__tests__/actions.test.ts#L418)、
[:497](../../packages/game/src/core/battle/__tests__/actions.test.ts#L497)）。以上为既有测试位置，本次核对源码和站位表，
仅补充文档，没有修改战斗实现，也未重跑这些单测或完整战斗E2E。
