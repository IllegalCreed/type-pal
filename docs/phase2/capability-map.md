...
```
   ...
   1. E6：authority 运行态可视化 + Esc 关闭后反引号重开
   2. PAL 四条错误角色条件引用：修 current-publication 上游、重生成、删四条豁免
   3. ED-3 统一引用边 + 场景/商店生命周期分片闭合
   4. R4 薄 E2E：runner、001-010、四种取物 canary、编辑器保存重开试玩
   5. N6b：四种内置取物意图 + content20；复跑薄基线，第三阶段再做通用化
   6. Q2 完整战斗专项五批（B5 并入）→ Q1 完整通关链
   7. 敌队大目录虚拟滚动（编辑器综合 E2E 前）
   8. 编辑器综合工作流 E2E
   9. `capture` profile + Content Studio 窄录制适配 + 分镜映射
   10. A1 服务器版本化预制工程资源库
   11. A8 独立可玩包与干净环境代表性 E2E
...
- 当前表内真实 `⚠️` 只剩 E6、B5；真实 `❌` 只剩最终收口 A1，但这只是能力格状态，不能替代 §4
