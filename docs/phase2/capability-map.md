...
```
   ...
   已确认的后续大专项（尚未开卡）:
   - N6b：R4 薄 E2E 基线后、完整 Q1/Q2 前完成四种内置取物意图并切 content20；第三阶段再做通用化
   - Q2：完整战斗专项 E2E；B5 作为动画与音画表现批并入，不再单独宽泛盘点

   第二阶段最终收口（当前不提前实施）:
   - A1：服务器版本化预制工程资源库；主体能力与 Q1/Q2 完成后、最终发布前实施

   当前已锁定的下一项:
   - E6 编辑器侧闭合：调试器显示 authority 运行态，并把 Esc 关闭后的快捷键重开纳入同一批；
     无 schema/save/migration。完成后才进入 R4 薄 E2E 基线

   当前可先做的小候选（不是已承诺任务）:
   - 敌队大目录虚拟滚动：第二阶段性能增强，安排在编辑器综合 E2E 前

   第二阶段 E2E 完成后的发布协作:
   - `capture` profile、Content Studio 窄录制适配和分镜映射；以 Q1/Q2 已验证碎片为前置
...
- 当前表内真实 `⚠️` 只剩 E6、B5；真实 `❌` 只剩最终收口 A1。
