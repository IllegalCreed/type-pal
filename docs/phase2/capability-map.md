...
| 格 | 名字 | 引擎 | 编辑器 | 原版考题 | 备注 |
|---|---|---|---|---|---|
| W1 | 地图渲染(tile 墙面) | ✅ | ⚠️ | 客栈/盛渔村市集 + OwnMap v1 | 引擎旧 Tilemap / OwnMap v1 双路渲染;编辑器已有笔刷/矩形/填充/擦除与 N 层显隐、重排,但地图仍从当前场景推导,缺独立地图列表、稳定 id、场景绑定与保存重开闭环。原版复用图只读仍是兼容层边界 |
| W2 | 移动与碰撞 | ✅ | ✅ | 走路/撞墙 | done |
...
| W7 | 自有地图编辑器 | ✅ | ⚠️ | W7D OwnMap v1 + W7B tileset 库 | **绘制能力已落地**:N 视觉层/独立碰撞/稳定 layer.id/尺寸编辑、tileset 上传绑定与作画均可用;但没有独立地图库、稳定 map id、场景选择/复用、未引用地图重开和删除保护。W7E 完成七环后恢复 ✅;随机笔刷/stamp/autotile、per-tile 高度仍属后续增强 |
