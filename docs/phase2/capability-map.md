...
这张地图把「第二阶段要做成什么」从一团迷雾变成 **8 领域、57 格** 的可数集合(活账,撞到新东西就长)。一格 done = 引擎能跑 AND 编辑器能编。原版撞到新东西就加格,地图会长大。
...
| W5 | 音乐/氛围 | ✅ | ⚠️ | 场景 BGM | 引擎已播(场景 musicId/playMusic op/战斗曲+胜利小调/读档恢复);编辑器已有场景 BGM 下拉、命令选择器、试听/别名和氛围表,但音乐仍靠数字 id 隐式映射文件,缺稳定资源 id/path、导入/替换/删除闭环,待 R7 恢复 |
...
| W7 | 地图编辑器 | ✅ | ✅ | W7F 地图库 + 图层/高度导航 | **W7F 基础闭环**：稳定 MapIndex、全量地图列表、CRUD、场景 mapId 换绑、未引用地图、懒加载/copy-through、N 视觉层、每格实例高度、独立碰撞、图层与高度组合聚焦、吸管和撤销均已接入。**已登记 W7G 后续必做**：树等组合地物作为图章一次落笔，模板预存各格 tile/图层/实例高度/碰撞并支持原子撤销；随机笔刷/autotile 仍属后续增强 |

### 实体(Entity)— 9 格

| 格 | 名字 | 引擎 | 编辑器 | 原版考题 | 备注 |
|---|---|---|---|---|---|
| E1 | 实体定义与摆放 | ✅ | ⚠️ | NPC/家具/宝箱 | EntityDef 二选一(actor/prop);编辑器已有画布图层、网格/禁入格、prop 放置、换精灵、移动与 undo,但场景生命周期不完整,actor/zone 无正式创建工具,地图绑定与实体引用删除未闭环。ED-4 完成后恢复 ✅ |
...
| 格 | 名字 | 引擎 | 编辑器 | 原版考题 | 备注 |
|---|---|---|---|---|---|
| N1 | 对话播放 | ✅ | ✅ | 客栈开场李大娘 | review(N1-1,2026-07-15):唯一 `DialogueCue + rows` 模型、旧码只在 migrate、全量 PAL 重生成与 MG2 零计划已完成；待 Opus/GLM 三方复验后改 done |
| N2 | 事件触发 | ✅ | ✅ | onEnter/物品触发 | done |
| N3 | 脚本演出 | ✅ | ✅ | 客栈开场自动演出 | done;async 解释器+预览。R2 build(2026-07-14):可执行 `unmigrated` 与旧 opcode 第二解释器已退役，全地址索引/0x6D 三态覆写/0x78 no-op/0xA0 回标题已落地并重生成，待三方复验 |
...
| N6 | 共享脚本/子程序 | ✅ | ✅ | s001+s002 双调用方 | build 完成、三方复验中：作者库 CRUD/稳定 id/callScript/引用安全/保存重开/MG2 保留/运行时懒加载均已落地；普通对话与获得道具仍是命令，参数化重复录入仍由模板负责 |
| N7 | **演出接管** | ✅ | ✅ | 隐龙窟/试炼窟 | done(2026-07-07):显式 take/release(E6b)+ mount/follow 权威,非冻帧。两考题实测:隐龙窟门口演出全链(对话/转向/切场景/setParty)+ 试炼窟芦苇漂共乘 |
| N8 | 过场编排(mp4/RNG CG) | ✅ | ❌ | 开场 CG/RNG 序列 | 引擎已有 playVideo/playRng 运行时;当前编辑器页只浏览硬编码 `/extracted` 的原版 1-6 号资源,不进入工程 EditSession,不能导入、登记、编排、引用、保存重开或删除。工程过场注册表与资源闭环完成后恢复 |
...
| X2 | 音频(BGM/SFX) | ✅ | ⚠️ | 场景音乐 | SFX+BGM 运行时基建全落(SpessaSynth MIDI BgmPlayer/assetBase.music/系统开关);编辑器可试听和选择已有音乐,但资源生命周期仍靠数字 id 与隐式文件名,缺导入/替换/删除。R7 稳定资源注册表完成后恢复 ✅。边界:X2=音频基建,W5=场景侧引用与切换 |
...
| A4 | 用户上传自有素材 | — | ✅ | **当前 done 范围仅限精灵、瓦片集、角色/敌人战斗外观**:精灵 PNG 上传、切帧、量化、入库与未保存预览已落地;瓦片集上传见 W7B;战斗外观上传见 A4c。音乐/SFX、图片/视频、字体/UI 等资源尚未纳入本格,统一由 A7/R7 建注册表、导入与资源闭环。全链 Playwright 已实测;保存→引擎真跑仍留用户 FSA 烟测 |
