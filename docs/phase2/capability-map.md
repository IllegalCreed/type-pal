...
> **(最近补账:2026-07-16 —— A7-3 视频/完整帧动画工程闭包与作者工作台进入 review；R2 脚本单一模型 N3✅ / N1-1 对话控制码退出 N1✅ / X3-1 场景入场呈现事务 / ED-4A 精灵/触发区创建闭环 / W7F 单格式地图 / N6 共享脚本 / ED-2 八模块导航 / A7-0 音乐闭包)**
...
| N8 | 过场编排(mp4/帧动画 CG) | ✅ | ✅ | 开场 CG/帧动画序列 | **A7-3 build 完成、待三方 review**：视频与完整帧动画使用稳定 AssetId 和工程内文件；运行时支持全段/分段/帧率/跳过，编辑器支持双列表、内嵌视频、完整帧时间轴、导入/替换/量化/多选重排/时长/引用保护/保存重开。BGM/SFX 仍由脚本编排，不绑素材 |
...
| A4 | 用户上传自有素材 | — | ✅ | **已覆盖精灵、瓦片集、角色/敌人战斗外观、音乐、视频与完整帧动画**：A7-3 支持 MP4/WebM 替换和 PNG/JPEG/WebP 序列导入、排序排除、量化、逐帧编辑与 TPFS 保存。SFX、字体/UI 等剩余资源仍由 A7/R7 后续闭包。真实 FSA 句柄烟测继续作为各切片 review 门禁 |
