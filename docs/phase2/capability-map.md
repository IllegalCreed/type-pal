...
| 格 | 名字 | 引擎 | 编辑器 | 原版考题 | 备注 |
|---|---|---|---|---|---|
| W1 | 地图渲染(tile 墙面) | ✅ | ✅ | 客栈/盛渔村市集 + OwnMap v1 | 引擎旧 Tilemap / OwnMap v1 双路渲染;编辑器已有笔刷/矩形/填充/擦除与 N 层显隐、重排。原版复用图仍只读(兼容层边界,不是自有地图能力缺口) |
| W2 | 移动与碰撞 | ✅ | ✅ | 走路/撞墙 | done |
...
| W7 | 自有地图编辑器 | ✅ | ✅ | W7D OwnMap v1 + W7B tileset 库 | **W7D+W7B 已落地**:N 视觉层/独立碰撞/稳定 layer.id/尺寸编辑(W7c-4);tileset 库 = 上传 PNG→切片→量化贴盘0(D25)→原版同构 .rle 入库→绑定作画;渲染单路零改。后续增强:随机笔刷/stamp/autotile(W7B 卡方法论节)、per-tile 高度编辑(tiles 元数据留字段) |
