...
```
   ...
   [商店生命周期](../ops/tasks/ED-SHOP-LIFECYCLE-1-shop-crud-and-safe-delete.md) 同日三方accept、720宽补验与用户验收通过，已收口。
   1. 第一、第二阶段全仓代码审计及 E2E 阻断项修复；一般优化单列排期
   2. R4 content20 薄 E2E：runner、001-010、四种取物 canary、编辑器保存重开试玩
   3. N6b：四种内置取物意图 + content21；复跑薄基线，第三阶段再做通用化
...
- 该句是 2026-09-03 对账时点快照；此后 ED-3 与场景生命周期已于 2026-09-05 收口，
  商店生命周期也已三方终审、720宽补验与用户验收通过并done。实时状态以任务看板为准。
...
- 当前表内真实 `⚠️` 只剩 B5（并入Q2专项）；真实 `❌` 只剩最终收口 A1，
  但这只是能力格状态，不能替代 §4
