...
> **(最近补账:2026-07-17 —— 新登记 W8 地图内容选择与实例属性编辑（W7G 前置）；X7-1 manifest 工程工作台与 A7-3 视频/完整帧动画工程闭包 done；R2 脚本单一模型 N3✅ / N1-1 对话控制码退出 N1✅ / X3-1 场景入场呈现事务 / ED-4A 精灵/触发区创建闭环 / W7F 单格式地图 / N6 共享脚本 / ED-2 八模块导航 / A7-0 音乐闭包)**
...
这张地图把「第二阶段要做成什么」从一团迷雾变成 **8 领域、58 格** 的可数集合(活账,撞到新东西就长)。一格 done = 引擎能跑 AND 编辑器能编。原版撞到新东西就加格,地图会长大。
...
| X2 | 音频(BGM/SFX) | ✅ | ⚠️ | 场景音乐 | **BGM 资源生命周期已由 A7-0/A7-0A 闭环**：MIDI+soundfont 都是工程资产，运行与编辑试听只经 AssetResolver/FileSource，标题菜单曲经第五个封闭角色临时播放并在离开菜单前清账停止，数字文件名和应用根 soundfont 已退役。X2 编辑器仍为 ⚠️，因为 SFX 的稳定 id、导入/替换和资源闭包要到 A7-1；边界:X2=音频基建,W5=场景侧引用与切换 |
...
| A4 | 用户上传自有素材 | — | ✅ | **已覆盖精灵、瓦片集、角色/敌人战斗外观、音乐、视频与完整帧动画**：A7-3 支持 MP4/WebM 替换和 PNG/JPEG/WebP 序列导入、排序排除、量化、逐帧编辑与 TPFS 保存；FSA secure-context 分类、LAN HTTPS、用户激活顺序及 OPFS 真句柄保存重开已验证。SFX、字体/UI 等剩余资源仍由 A7/R7 后续闭包 |
