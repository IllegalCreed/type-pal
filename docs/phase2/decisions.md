...
## D16 · reforge 渲染地基：逻辑 / 显示分离（格坐标 + 素材像素解耦）（2026-06-28）

**决定**：reforge 渲染分**逻辑层 / 显示层**两层、彻底解耦。
- **逻辑层**（存储 / 游戏逻辑 / 碰撞 / 触发 / 寻路 / 存档 / 编辑器）：实体位置 = **格坐标 `(col, row[, h])`**（非像素）；尺寸 = 逻辑单位。
- **显示层**（渲染）：格 → 像素（直接定位）；**移动显示 = 格到格步进、保持原版卡顿感（~10fps 步进），不做平滑插帧** —— 作者明确喜欢卡顿感，插帧留后续可选、非必做。素材按「逻辑尺寸 × 目标倍率」缩放，**素材实际像素与逻辑尺寸解耦** —— 素材在 manifest 标自己的逻辑尺寸，加载 / 渲染层负责缩放（16px 占位放大 / 64px 原生高清，对程序透明）。
- ⚠ 注意区分：格坐标根治的是**「半格站位 bug」**（站到格缝）；**卡顿步进感是保留的**（一步一格、跳变），两者不冲突。
- **物理分辨率** 提到 **1280×800（4x）**，逻辑保持 **320×200**（"4 像素当 1 像素"，世界整数放大 = 原版像素观感）。

**理由**：reforge 现在**反着来** —— `player.pos` 存像素、`render.ts TILE_W=32` 绑死素材像素、`drawImage` 按素材原尺寸画。两个硬伤：① 像素 + iso 步进奇偶错位 = "撞墙半格"bug 根因（[movement.ts](../../packages/reforge/src/movement.ts) 注释）；② 素材精度写死 → 换高清要改渲染逻辑 + **全量素材同时换**（作者明确不能接受）。格坐标根治半格 + 碰撞 / 触发 / 编辑器按格简单；素材解耦让「16 占位 → 64 高清」**一张张无痛替换、程序零改**。是最底层地基，后置 = 大面积返工（坐标 + 全量素材），必须先于菜单及一切 UI。是 [D15](decisions.md) RGBA 化 + roadmap P3 资产管线的前提。

**影响**：
- reforge 渲染 / 坐标核心重构：`render.ts` / `movement.ts` / `collision.ts` / `main.ts` 坐标改逻辑格；**会动到已做的对话**（dialog-box 现 320 像素坐标）、移动、碰撞 —— 一次性投资。
- 素材 manifest 增「逻辑尺寸」字段；加载层做素材实际像素 → 逻辑尺寸缩放。
- **UI（对话 / 菜单）与世界同一套逻辑 / 显示分离**：UI 元素（文字 / 边框 / 装备格 / 头像）按物理精度**真高清渲染**（逻辑坐标 + blit 按逻辑尺寸缩放），**不是「320 整体放大」低清**。世界与 UI 一套机制、不分两套（作者明确：UI 也要相同升级、非独立低清）。
- 故**字模高清化是必做**（非留后）：原版 16px 点阵放大 4x 糊 → 高清字模（更大点阵 / 矢量；是否保留像素字观感由 spec 定）。
- **菜单及后续所有 UI / 美术 / 编辑器长在此地基上**；菜单（[D17](decisions.md)）暂搁待此落地。
- 待写专门 spec 细化（坐标变换 / 素材缩放 / 插帧 / manifest 格式）。
