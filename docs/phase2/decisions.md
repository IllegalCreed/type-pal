...
### D25 补 · 压缩靶子分类(2026-07-06,作者细化 + 实测)

作者细化:「UI 保留 RGBA 烘焙没问题(少);主要是角色精灵和道具,这两个多。」实测各资产存储:
- **角色精灵**:✅ 已是 `gzip(YJ2 索引块)`(pal-extract 早从逐帧 RGBA PNG 换掉,line 93)。**已压缩**,运行时按**场景盘**上色(随日夜/场景变——**不能**烘死盘0,否则夜景错色)。保持。
- **道具图标**:⚠ 251 个 BALL 存成 **RGBA PNG(colorType 6,1MB)**,但 RGB 三通道全是同一索引值(冗余)+ 运行时上色。道具是 UI(**恒盘0**)→ 应改**真索引 PNG(colorType 3,PLTE 嵌盘0)**:1 字节/像素 + 浏览器自带上色 → **~4× 小 + 零运行时上色**。**压缩靶子**。
...
### D25 补³ · 帧动画作者模型与 codec 分层(2026-07-16,用户拍板)

D25 的“RNG 不烘 RGBA”只保留为 2026-07-06 的阶段性压缩判断，**不再约束新版帧动画作者模型**。A7-3
用全部 1,464 帧真实数据复测后，32 帧 block 的“完整 RGBA8 首帧 + 相邻完整帧 XOR + Deflate”选型
原型为 8,271,766 B；正式迁移器固定 zlib level 9 后产物为 7,960,282 B，比当时的脏矩形 PNG 方案
31,525,705 B 更小。新版因此采用以下边界：

- 作者、编辑器和脚本只认识完整真彩帧与稳定 `frame-animation` AssetId；不认识块、关键帧、脏矩形、
  `paletteId` 或原版 RNG chunk。
- PAL 迁移按段使用已考证颜色表烘焙真彩，3/6/7 段分别使用 2/3/6，其余使用工程标准色彩；运行时不再
  解释活调色板。
- TPFS 的 block/XOR/Deflate 只是可替换的保存/加载 codec。以后找到更优无损算法，可以替换 codec，不能
  反向改变完整帧作者语义。
- “贴合工程标准色彩”是导入/编辑时的显式量化工具，输出仍是真彩帧；UI 不暴露颜色表编号。
...
## D27 · 遮挡反馈方案：前景半透明（2026-08-06，用户拍板）

**背景**：[backlog 议题 6](design-backlog.md) 的「审美待拍」——原版人物被前景（屋顶 / 树冠 /
山坡等）挡住就完全看不见，新引擎要做现代化反馈，候选方案 A（被遮挡时前景半透明，人透出来）
与方案 B（前景保持不透明，人物描轮廓 / 剪影）。两方案技术上 reforge 都已具备（遮挡按 baseY
深度排序已对，[render.ts](../../packages/reforge/src/render.ts:2)；渲染器支持 alpha，
render.ts:295-346），只差审美取向。

**决定**：采用**方案 A：被遮挡时前景半透明**。方案 B（描轮廓 / 剪影）不选。

**理由**：作者审美取向 = 现代透视、信息优先；像素美术里整块前景变「玻璃」的观感在
目标作品中可接受（且可随 P1 渲染实际效果微调透明度 / 触发阈值）。

**影响**：
- 议题 6 的「审美待拍」关闭；P1 渲染实现方向 = 遮挡关系可判定（schema 保证）+ 被遮挡时
  前景层 alpha 化，不做角色描边剪影。
- D4 / D10 的 RGBA + 合成渲染管线已为此留位，无新技术债。
- 触发阈值（遮挡面积 / 距离）与透明度曲线属 P1 渲染实现细节，届时按实际观感定，不重开
  方案选择。

## D28 · 演出/分镜段验收口径：分段 e2e 优先，视觉抽验（2026-08-07，用户拍板）

**背景**：D14-2（CutsceneController 重构）后，求雨 / 酒剑仙 / 结局等分镜段的最终验证
若走 Kimi 浏览器视觉并排，成本高且难以机器复核（Kimi 已如实标注「未实跑」）。用户指出
这类段最后用**分段 e2e** 验证更合适，视觉验证成本太高。

**决定**：

1. **演出 / 分镜 / 过场段的最终验收 = 分段 e2e**：复用
   [e2e-checkpoints 碎片模型](../../projects/pal/e2e-checkpoints/README.md)
   （起始存档 `NNN-名.save.json` + 流程说明 + 验证点 + 结束存档 = 下一碎片起点），
   `?e2e-load` 秒进 + 确定性输入 / 状态断言（`__tpE2e.dumpSave()` 导出）。
2. **Kimi 视觉验证降级为「抽验 + 并排参考」**，不再作为演出段 done 门禁；视觉抽验只用于
