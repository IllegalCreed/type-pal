...
| D30 | 五项增强移交第三阶段 | 有效 |
| D31 | 性能/录制/无障碍/内容补标阶段归属 | 有效（敌队大目录虚拟滚动留二阶段） |
| 新增决策 | 直接按编号续写正文；本索引行随增 | — |
...
- 未决子问题（A1 素材规模与托管版本 / A2 下载协议与进度 UI / A4 支持的素材格式）留「做 A 领域」那轮 brainstorm 细化，不在本决策范围。

**2026-09-03 用户补充裁决**：第二阶段只完成工程资源自包含、作者导入/替换与分发能力；原版受保护
素材的实际批量替换、生成/授权记录和公开发布前版权验收统一移交第三阶段。A1 仍是服务器版本化预制库
基础设施，并保留为主体能力与 Q1/Q2 完成后的**第二阶段最终收口项**，只在最终发布前实施；A6 仍是
可选的主动更新检查。A1 不移交第三阶段，且 A1/A6 都不得把“全量清版权”写成完成条件。
...
深度排序已对，[`render.ts:2`](../../packages/reforge/src/render.ts)；渲染器支持 alpha，
...
## D30 · 五项增强移交第三阶段（2026-09-03，用户拍板）

**决定**：以下能力不在第二阶段实现，也不作为第二阶段完成条件：

1. W6 的真实时间流逝与天气；第二阶段只保留已经完成的昼夜氛围。
2. A6 用户主动检查并导入新版预制库。
3. X5 试玩前配置世界变量、剧情标志等前置状态。
4. W7 随机笔刷与自动拼接地图块。
5. D13 时间旅行调试。

五项统一进入第三阶段待排期能力池。既有 W6 昼夜、X5 跳转试玩、W7 地图库与 D13-1 调试工具首批均保持
第二阶段 `done`，不得因扩展项移交而降级；A6 在第二阶段改为不适用。A1 仍是第二阶段最终收口项，未随
A6 移交。

## D31 · 性能、录制、无障碍与内容补标的阶段归属（2026-09-03，用户拍板）

1. **敌队大目录虚拟滚动留在第二阶段**，作为编辑器性能增强，在综合编辑器 E2E 前完成；不改变敌队
   数据模型或当前功能闭环。
2. **自动录像下游留在第二阶段发布链**：`capture` profile、Content Studio 窄录制适配和分镜映射
   必须建立在 Q1/Q2 已验证碎片、checkpoint 和执行回执之上；不得在 E2E 前抢跑，也不得用录像代替 E2E。
3. **无障碍设置移交第三阶段**：文字速度、对话回看、高对比、输入重映射和脚本树键盘导航届时统一设计，
   可参考第一阶段内置生产工具、对话历史与输入抽象。移交不允许第二阶段现有 ARIA、焦点和基础键盘操作回退。
4. **内容语义补标移交第三阶段**：C1 剩余人物语义候选和 C2 未自动迁移精灵的人工动作语义标注，不再是
   第二阶段内容收口项，也不降级已完成的 C1/C2 能力。
