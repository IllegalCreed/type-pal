...
- **逻辑层**（存储 / 游戏逻辑 / 碰撞 / 触发 / 寻路 / 存档 / 编辑器）：实体位置 = **格坐标 `GridPos = { col, row, height? }`**（非像素），落脚点 = cell 中心、一格一点；`height` 为垂直高度 / 楼层 / 飞行扩展轴，**先留口、默认 0**（地面站立时无值），将来接遮挡投影（[render.ts:232](../../packages/reforge/src/render.ts) 瓦片 baseY 已 key 在 `iTileHeight` 上，精灵接入即正确遮挡）。
- **显示层**（渲染）：格 → 像素（直接定位）；**移动显示 = 格到格步进、保持原版卡顿感（~10fps 步进），不做平滑插帧** —— 作者明确喜欢卡顿感，插帧留后续可选、非必做。
- ⚠ 注意区分：格坐标根治的是**「半格站位 bug」**（步长 `(16,8)` 不是整格 → 某步单轴回退致落脚偏离 cell 中心）；**卡顿步进感是保留的**（一步一跳），两者不冲突。文字渲染走 wall-clock（[typewriter.ts](../../packages/reforge/src/text/typewriter.ts) 已按 `performance.now` 算），与移动的 10fps tick 本就是**两套独立时钟**，互不拉扯。
...
物理分辨率提到 **1280×800（4x）**、逻辑保持 **320×200**，但要区分两个不同时点、不同性质的需求：

| | 时点 | 驱动 | 做法 | 要 logical-size manifest 吗 |
|---|---|---|---|---|
| **UI / 文字 HD**（对话 / 菜单 / 字模） | **近期（本决策要做）** | **信息密度**：320 下 CJK 16px 已是下限，想加密只能缩小，但 320 物理放不下更小却清晰的字 → 必须升物理分辨率 | 换**更高分辨率的字模源**（不是把 16px 拉大成 64px——那只会更大不更密）；UI 元素按物理精度真高清渲染，非「320 整体放大」低清 | 否（字模源问题，非 logical-size） |
...
- 故**字模高清化是必做**（非留后）：原版 16px 点阵（[glyph.ts](../../packages/reforge/src/text/glyph.ts) Unifont 来源）放大 4x 糊 → 高清字模（更大点阵 / 矢量；是否保留像素字观感由 spec 定）。
- 「素材 manifest 增 logical-size 字段」**不是本决策必做项**——当前 ×4 整数倍放大、提取器放大版即可；logical-size 抽象等真有「非整数倍 / per-asset 异分辨率」需求（即世界 HD 真正落地）时再上，现在加是用不到的过度抽象。

**理由**：reforge 现在**反着来** —— `player.pos` 存像素、`render.ts TILE_W=32` 绑死素材像素、`drawImage` 按素材原尺寸画。硬伤：① 像素 + iso 步进奇偶错位 = "撞墙半格"bug 根因（[movement.ts](../../packages/reforge/src/movement.ts) 注释）；② UI 锁死 320 → 文字信息密度封顶。格坐标根治半格 + 碰撞 / 触发 / 编辑器按格简单；物理分辨率升 1280 解信息密度封顶 + 给 UI / 字模真高清。是最底层地基，后置 = 大面积返工（坐标 + 全 UI 坐标），必须先于菜单及一切 UI。
