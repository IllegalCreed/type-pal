...
>
> **已拍板、尚未实现**：第二阶段将在薄 E2E 基线后，为四种取物流程实现窄版
> [N6b 强类型脚本模式库](../design-backlog.md#n6b-强类型脚本模式库)。当前文档仍描述已实现的 N6 v1；
> v1 没有参数，N6b 实现前不得把模板或 `callScript` 冒充已支持参数化复用；通用化扩展归第三阶段。
...
1. 在编辑器“剧情 → 脚本库”点「新建可复用脚本」。
2. 创建时填写显示名和稳定 id（默认为 `shared/user/…`，`self` 为「不使用」）。说明与
   `self` 契约在创建后于右侧作者元数据修改；稳定 id 创建后不随显示名变化。
3. 在统一指令树编辑正文。位置无关脚本不需要地图；需要核对实体、坐标、面向或场景切换时，
   从具体调用点进入场景工作台，在真实地图上预览。
4. 当前脚本库没有复制入口；需要独立副本时另建新脚本，再在统一指令树里重写正文。
5. 保存前 validator 会检查悬空引用和非法实体地址，并核验 `self` 字段是
   `none|optional|required`。当前保存门**不**对 canonical `callScript` 做调用环 DFS。
...
- 下半区是可调高度脚本抽屉，页签为「进场脚本 / 传送出口」，选中实体时另有「交互脚本 / 自动行为」；
...
- `必须提供`：显式地址优先，否则继承 caller self；运行时仍为空则拒绝。保存是否因缺
  调用实体而 fail-loud 目前待证，不得写成已实现的保存门。
...
- 共享脚本之间仍应禁止形成 `callScript` 环；这是现行设计约束，不是已实现的保存保护。
  当前保存只查稳定 ScriptId 是否缺失等引用问题，不对 canonical `callScript` 图做 DFS。
- 实体地址悬空、引用目标缺失或作者正文含 v4 `jumpScript`/动态 binding 时，保存和发布
  fail-loud。`self: required` 缺调用实体由运行时拒绝；保存/发布是否同样 fail-loud 待证。
