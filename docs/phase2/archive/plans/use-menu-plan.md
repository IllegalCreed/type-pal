...
**范围(demo 单人):** ✅ 可用列表(过滤 `use` 能力块)+ 选目标(单人=李逍遥)+ 回血/回真气 + 消耗。❌ 多角色目标切换(单人跳)、全体目标(数据留 `applyToAll` 口、本期只做单体)、`triggerScript`/`teleport`/`applyStatus`(留桩,归宿=剧情脚本系统/状态系统,见 [script-system-design](../designs/script-system-design.md))、投掷、买卖。
