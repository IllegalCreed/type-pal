...
**Architecture:** UI/对话语义色 → 固定 RGBA 常量(取原版 pal0 的 UI index 快照,不绑场景 palette);头像(原版 indexed)→ 迁移期烘成 RGBA PNG,运行时直接 drawImage。`pal-extract`(一阶段)**不动**;烘 RGBA 是第二阶段迁移脚本的活(本计划起第一个迁移脚本:头像)。见 [D15](../../decisions.md)、[content-schema §8](../designs/content-schema.md)。
