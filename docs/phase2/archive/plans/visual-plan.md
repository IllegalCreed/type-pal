...
**Architecture:** 两层(../decisions.md)):`text/` 通用文本渲染原语(不绑对话)+ `dialog/` 对话渲染(消费 text-render),从 main.ts 闭包拎出。设计全文见 [visual-design.md](../designs/visual-design.md);外观真值(坐标 / 色值)见 [GLM spec §3](../../reference/dialogue-presentation.md);本计划是「三刀」第②刀(① 数据地基已完成、③ 迁移器留后)。
...
`dialog-box.ts` 顶部,从 [GLM spec §3](../../reference/dialogue-presentation.md) + `present/dialog-box.ts` 端口 bottom style 常量(本 Task 只需 bottom):
```ts
...
按 [design §5 分配表](../designs/visual-design.md) 落地:`locale.ts` `zhLocale` 改/加文本(句2 换成「远处的鬼」台词,某些句加 `<yellow>`/`<cyan>`/`<red>` 标记 + 加 `name.distant-ghost`);`index.ts` 鬼话 `lines` 每句配 `slot`/`portrait`/`speed`/`autoAdvance`:
```ts
lines: [
