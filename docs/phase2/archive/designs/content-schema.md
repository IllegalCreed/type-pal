> **归档快照**：以下完整保留搬迁前的设计和增量记录；不再维护现行合同。
> 当前规范见 [内容工程当前格式](../../specs/content-schema.md)。旧状态、版本与签字按当时语境阅读。
...
>
> **每条决策的「为什么旧引擎不行」证据**见 [engine-debt-audit.md](../audits/engine-debt-audit.md)（文末有「schema 决策 ↔ finding」反查表）。本文只定「应该长什么样」，那份定「为什么必须这样」。
...
- **原版 opcode 怎么办**（✅ 已决,2026-07-02；R2 于 2026-07-14 收口）：**迁移器把原版脚本翻译成结构化脚本**（[script-system-design](script-system-design.md) 的嵌套 AST），**不建永久 opcode 兼容执行器**。理由：①用户验收原话即「原版数据**经迁移器**进新数据结构」——要的是翻译，不是供旧字节码；②双解释器并存 = [engine-debt-audit](../audits/engine-debt-audit.md) P0-5/P0-6（三份 switch / 双解释器语义漂移）的债原样重生，恰是要修掉的不合理；③单人维护两套解释器不现实。可达且不能翻成 clean 命令的源指令只进入迁移期 `MigrationGap` 并阻断写盘；工程内容、编辑器命令目录和运行时均不得出现 `unmigrated` 或原 opcode 占位。旧工程若仍含这类节点，内容校验直接提示重新迁移。
...
迁移范围、反例集和产物门禁见
[`x3-scene-entry-migration-audit.md`](../audits/x3-scene-entry-migration-audit.md)。
...
[`a7-0-music-resource-closure-report.md`](../audits/a7-0-music-resource-closure-report.md)。
