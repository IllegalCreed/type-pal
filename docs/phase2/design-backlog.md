...
> 状态：开放设计，尚未拍板阶段或实现方案。2026-09-03 用户提出；本节只防想法丢失，不授权改 schema。
...
未来可用重复脚本形状统计再筛门传送、NPC 搭话、遇敌、楼梯、巡逻、机关与条件奖励；不与取物首批一起
扩张参数模型。正式立项前仍需用户决定：它属于第二阶段创作核心还是第三阶段效率增强，以及内置模式是否
只读、作者能否创建自己的参数化模式。
