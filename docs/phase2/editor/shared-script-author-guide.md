...
> 适用版本：contentVersion 20 / SAVE 8（2026-09-06 复核；正文旧版本号为当时快照）。脚本模型不带产品版本后缀；作者内容直接使用
...
>
> **开放方向（2026-09-03，未拍板）**：用户提出为宝箱、可见拾取和无精灵搜刮等高频流程建立
> [N6b 强类型脚本模式库](../design-backlog.md#n6b-强类型脚本模式库)。当前文档仍描述已实现的 N6 v1；
> v1 没有参数，N6b 未立项前不得把模板或 `callScript` 冒充已支持参数化复用。
...
直接调用方。contentVersion 20 作者界面不显示“迁移内部实现”页签；若项目仍含脚本分片、旧地址或
