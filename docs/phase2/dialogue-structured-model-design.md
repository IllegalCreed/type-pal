...
> 状态:草案(2026-06-26 立)。第二阶段铁律见 [READ-FIRST.md](READ-FIRST.md);内容总 schema 见 [p0-content-schema.md](p0-content-schema.md);本次决策记录见 [decisions.md](decisions.md) D11。
> **承接** [p1-slice1-dialogue-visual-spec.md](p1-slice1-dialogue-visual-spec.md)(GLM 写的对话框**外观**继承清单)——本文定**数据格式**,那份定**外观真值**;两件正交的事,本文把它们在架构里安顿到一起。
...
- **i18n 一等公民**:对话正文**也**走 text id + locale 查表([D9](decisions.md)),面向英文用户,不内嵌字面字符串。详见 §4。
- **分支留在演出层**:"给玩家俩选项、按选择/剧情走不同支"**不属于对话行**,属于演出 timeline 的 choice action + 世界变量条件([p0-content-schema §3](p0-content-schema.md) + [§6](p0-content-schema.md) 已留好家)。对话行只管"谁说了什么"。本文**只为分支留数据口,不实现**。
...
**text id 命名 / 组织**(per-scene 命名空间、自动生成 vs 手命名)留 schema / 迁移器细化,不在本文定死;只锚定「稳定 id、locale 查表」([D9](decisions.md) 稳定 text id 铁律)。

## 5. 控制符解析的归属(架构要点)

> **原版那套功能 / 结构控制符(`~$`/末尾冒号/分行)从「运行时」前移到「数据生产期」彻底消失;运行时只剩 i18n 富文本的颜色标记轻解析。**

- 原版那几千句:**迁移器**([p0-content-schema §8](p0-content-schema.md))一次性吃掉控制符——功能 / 结构 → 结构化字段,文本 → locale 表(颜色 toggle → `<color>` 标记)。
...
- 一行对话永远只是"谁说了什么"。
- "给俩选项 / 按世界变量走不同支" = 演出 timeline 的 **choice action** + condition([p0-schema §6](p0-content-schema.md) 演出 action + [§3](p0-content-schema.md) 世界变量)。
...
- **多语言实际翻译(en 等)** —— i18n **机制**(text id + locale 表)本次就做([D9](decisions.md));具体译文按需补,locale 表先只有 zh。
