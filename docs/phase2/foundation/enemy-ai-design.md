...
## 7. 难度分级留口(用户预告,本期只留结构)

- `world.difficulty: 'easy' | 'normal' | 'hard'`(默认 normal,进存档)。
- 行为轴:`AiCond.difficulty`(§1)——normal 档规则 = 原版行为(硬验收基线)。
- 数值轴:startBattle 组装敌我数值处预留**难度缩放点**(纯乘法变换,不污染内容数据;
  系数表 = 将来的内容文件,本期不建)。
- 资源/容错/遭遇轴:归难度分级正式立项时设计。
