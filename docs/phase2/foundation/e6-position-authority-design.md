# E6 · 实体定位权威 设计方案（2026-07-05 · 待作者评审）
...
1. **auto 巡逻暂停粒度**：推荐 b「仅被 take 的实体暂停」（贴 2026-07-03 并行拍板）；
   备选 a「主脚本期全体 auto 暂停」（更接近原版观感,但推翻既有拍板）。
2. **隐式 take 的范围**：推荐「位移 op 才 take」；备选「脚本碰过的实体全 take」
   （含 setEntityFacing/Frame——演出转向也锁位置,更强但迁移脚本可能过度锁）。
