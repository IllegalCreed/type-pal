...
> **当前模式（用户 2026-09-25 决定）**：Codex 分派任务，其他 Agent 在隔离范围执行，Codex 独立验收、集成与收口；不再要求 Codex/Kimi/GLM 固定三席签字。下文保留三贤人时期流程作历史参考，与当前模式冲突时以 [`AGENTS.md`](../../AGENTS.md) 顶部「当前临时模式」为准。用户恢复三贤人制度前，不因旧卡的 Kimi/GLM `pending` 阻止 Codex 按已核前提推进。

当前交付链：Codex 定范围和独立工作树 → 贡献者实施/自测并推送 → Codex 复核证据、返工或接收 → Codex 执行所需质量门并合并推送 → 用户仅对需要产品判断/体验验收的部分拍板。高风险仍保留一手证据、停止线、单一 Coding Owner 和必要的用户裁决；候选测试不自动计入官方覆盖率。
...
当任务进入“仅待用户复验”时,不得只回复“请验收”或罗列测试命令。Coding Owner 必须同时在最终回复与任务卡
给出一份最小手工复验清单,至少写清:

- 从产品界面的哪个导航路径进入、选择哪个代表对象。
- 用户需要执行的具体操作;只要求用户判断可见行为,不让用户代替 Agent 跑技术测试。
- 每一步看到什么算通过、出现什么算不通过。
- 哪些历史切片已经验过、这次无需重复;优先把当前 candidate 压缩成 2–5 分钟可完成的检查。
- 涉及响应式、滚动、浮层、焦点或撤销时,给出一个最小代表宽度/操作,不泛泛要求“全页面看看”。

用户只通过部分项目时,通过项保持有效,失败项转 `rework` 并记录精确反例;不得要求用户从头重复全部验收。
...
### Kimi / GLM 并行审查（2026-09-04 用户拍板）

- 同一阶段两席没有输入依赖时，Coding Owner 必须在一条回复中同时给出 Kimi、GLM 两份提示词，用户可
  并行转发；不得无理由先等一席完成再给另一席。
- 两份提示词钉同一任务卡 revision / candidate commit，各自要求从一手证据独立形成结论，不读取或
  锚定另一席的审查输出。
- Reviewer 默认直接在任务卡写入自己的 `premise/design` 或 `accept/counter`、直接证据、返工项与交接日志，
  并提交推送。用户只转发提示词，不负责在账号之间复制审查正文。
- 并行只针对取证与判断；落盘时每席只编辑自己的签字块和自己的交接日志，不改另一席内容、共享准入结论、
  `Status` 或 build/done。提交前同步最新分支；若另一席先落盘，必须保留其改动；push 竞态由 reviewer
  自行 rebase/retry。Coding Owner 在两席都落卡后统一更新主审结论、准入结论和任务状态。
- Reviewer 没有仓库写权限时必须明确报告阻塞，由 Coding Owner 安排隔离分支或其他直接落盘方式；不得把
  复制审查正文默认转嫁给用户。
- 任一席 `counter` 时仍按原门禁停线。若 counter 导致前提、方案或候选发生变化，基于旧 revision 的并行
  `agree/accept` 只保留为历史，不授权推进。
- 仅当后一席必须验证前一席的新改动或结论时串行；提示词中必须写明依赖和为何不能并行。
...
用户于 2026-08-15 拍板:正式上线前不为历史开发版本保留产品兼容层。所有代码审查都必须主动检查本次改动
及其直接调用域是否存在下列形态,不能只等提交者主动说明:

- 编辑器中的“升级到 vXX”入口、旧工程自动升级、运行时旧存档迁移入口。
- 按 content/save/schema version 分支的双 parser、双写、旧字段 fallback、缺字段静默默认或 normalization。
- 以 `legacy` / `compat` / `backward-compatible` / `旧版本` 为理由保留的 optional 字段、adapter、默认参数或死分支。
- 历史类型、upgrader 或 fixture 被 runtime/editor 产品路径 import,导致一次性迁移逻辑进入常驻 bundle。

默认处置是在重迁/重生成当前工程后,同一任务删除旧 upgrader、旧类型、旧 fixture/test、产品入口和所有兼容分支；
Git 才是历史档案。只有仍有无法重新生成的真实输入且当前任务必须消费时,才允许一个隔离的临时转换器；任务卡
必须写清真实输入、唯一调用方、删除条件和用户批准。“以后可能用到”、保留测试覆盖或方便手工打开旧工程都不是
例外理由。浏览器/操作系统兼容、第三方文件格式支持不属于本条“项目旧版本”范围。审查结论必须单列
`旧版本兼容审查: pass | counter`,列出命中位置和“删除/临时保留”的判定；未检查不得签 `accept`。
...
用户于2026-09-25更新：GLM已具备视觉能力，可接受明确委派的浏览器、截图/录屏、布局和交互初审。
首包必须提供实际操作小样；工具/资产不足如实blocked，不以代码推断冒充视觉证据。
Codex仍负责接收复核、整体验收与AI资源生成；GLM的执行贡献不算独立终审或产品实现授权。
2026-09-12的“GLM无多模态、视觉只由Codex执行”限制被本次裁决替代；历史签字与披露保持原文。
