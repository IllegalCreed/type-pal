...
- 两份提示词钉同一任务卡 revision / candidate commit，各自要求从一手证据独立形成结论，不读取或
  锚定另一席的审查输出。
- 默认两席只读，不同时编辑同一任务卡；各自返回可直接登记的 `premise/design` 或 `accept/counter`
  结构化文本、证据与返工项。Coding Owner 收齐后按原意一次写回并注明来源；转录不构成代签。
- 若 reviewer 必须直接提交，提前指定唯一写入方；或使用隔离分支/工作树，不能让两席争写 main 上同一文件。
