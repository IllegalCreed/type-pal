...
焦点竞态；受控并发下 193 文件 / 1,748 项全部通过，断言和超时本身没有放宽。
...
两档都确认覆盖报告精确包含 **608 个生产文件**。fast 执行 486 个测试文件 / 5,691 项测试；full 执行
