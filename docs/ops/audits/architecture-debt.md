...
原先测试批次的单席豁免不自动扩大到架构改造，新卡按自身签字/用户豁免准入。
...
| A2 Reforge战斗宿主（[r1 draft](../tasks/ARCH-REFORGE-BATTLE-1-host-lifecycle.md)） | main.ts:2202–2557，资源准备、会话、结算、战后脚本由大闭包调度；新卡以7f3840e6重新定位2149–2505 | 单独战斗启动/运行所有者，显式生命周期与资源依赖；待本卡准入 | H9与46会话回归；迟到资源、取消、终态写回不漂移 |
...
1. 首批A1[任务卡](../archive/tasks/done/ARCH-REFORGE-MENU-1-session-controller.md)已按本批用户单席豁免完成；
   [回执](../../testing/menu-session-refactor.md)保留模式与证据，后续A2/A3不自动继承豁免。
