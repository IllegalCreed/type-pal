...
| 队列 | 证据与问题 | 拆分方向 | 验证边界 |
|---|---|---|---|
| A1 Reforge菜单/物品宿主 | main.ts:5293、:5368、:6384，菜单态/物品异步执行混入6696行bootGame | 菜单会话拥有面板/光标/临时操作，main仅路由输入与连接有限端口 | 真实H3/H5/H7/H8回归；新控制器直接测；存储与SAVE8不改 |
...
1. 首批A1已准备[任务卡](../tasks/ARCH-REFORGE-MENU-1-session-controller.md)，先以菜单控制器建立模式。
