...
2026-09-24 用户明确授权：“架构治理全部都不用另外两个ai参与，你独自推进”。
本队列 A1–F2 全部由 Codex 独立实施、自验、审查与收口，Kimi/GLM 的 build/done 签字豁免，免补审。
不代签、不宣称独立第三方审查；仍逐卡登记前提、风险、实际证据。此授权不包括擅改玩法、schema/save、
UI 形态或把队列外缺陷混入结构重构；范围变化另行裁决。
...
| 队列 | 证据与问题 | 拆分方向 | 验证边界 |
|---|---|---|---|
| A1 Reforge菜单/物品宿主（已done） | 09429基点main.ts:5293、:5368、:6384，菜单态/物品异步执行混入6696行bootGame | dbe55b55已移出15状态，main7153→6798；MenuSession/ItemUseSession拥有控制状态，窄端口接线 | 28新增、155序列3798步等价、10针、check8440/strict7949通过；后续A2/A3仍待做 |
| A2 Reforge战斗宿主（[r1 review](../tasks/ARCH-REFORGE-BATTLE-1-host-lifecycle.md)） | 7f3840e6 main.ts:2149–2505，资源准备、会话、结算、战后脚本由大闭包调度 | BattleHost/准备单元已落；main6798→6486；独立所有权，不改核心 | 21新增/9针/check8461与真实功能验证通过；统一覆盖门待完成 |
| A3 Reforge世界/帧循环 | main.ts:995、:1780、:4055、:4940、:6288 | 场景生命周期、世界步进、输入仲裁、呈现分开；逐单元推进 | 不同时重写移动模型/碰撞规则；沿用既有原子提交/epoch合同 |
