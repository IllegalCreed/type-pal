...
2026-09-07 用户明确免复验通过，已 done；下文缺陷复现保留修前证据，不代表本候选仍共用存储键。
...
当前跟进：[EDITOR-SAVE-CONFLICT-1](../../archive/tasks/done/EDITOR-SAVE-CONFLICT-1-stale-author-snapshot.md) r1 三席终审 accept、无返工，
2026-09-07 用户明确验收通过，已 done，候选未漂移。下列是修前反例，不表示新候选仍静默覆盖。
...
已开始只读状态门与恢复基础模块，尚未接入目录暂存/自动恢复，A-03仍未标修复。
