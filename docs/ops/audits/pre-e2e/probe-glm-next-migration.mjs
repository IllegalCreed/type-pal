...
function expectedRejection(run, pattern) {
  try {
    return { value: run(), rejected: false }
  } catch (error) {
    if (error.code === 'ERR_ASSERTION' || !pattern.test(error.message)) throw error
    return { rejected: true, error: error.message }
  }
}
const pathRefusal = /符号链接|symbolic link|symlink|越界/
...
const CENSUS_EXTS = /\.(ts|tsx|mts|mjs)$/
...
function scanSymbolCalls(text, symbol, path) {
  const ast = ts.createSourceFile(
    ...
    out = []
  assert.equal(ast.parseDiagnostics.length, 0, 'census syntax must parse')
  const locals = new Set([symbol])
  const imports = (node) => {
    if (ts.isImportSpecifier(node) && node.propertyName?.text === symbol) locals.add(node.name.text)
  ...
  const visit = (node) => {
    if (
      ...
      out.push({
        ...
        block: node.arguments.map((x) => x.getText(ast)).join(','),
      })
...
function censusCalls(symbol) {
  if (!censusCache.has(symbol)) {
    ...
    for (const path of walkCensusFiles()) {
      ...
      for (const call of scanSymbolCalls(text, symbol, path))
...
try {
  ...
  if (want('E01')) {
    ...
    const order = calls
      .filter(([kind]) => kind === 'write' || kind === 'rename')
      .map(([, path]) =>
        path.includes('/stage/')
          ? 'stage'
  ...
  if (want('E02')) {
    ...
    calls = []
    const beforeTarget = get(`${repo}/projects/pal/${file}`, 'utf8')
    const result = expectedRejection(
      () => commitMigrationTransaction(repo, changes),
      /修改|冲突|快照|偏离/,
    )
    const finalText = get(`${repo}/projects/pal/${file}`, 'utf8')
    const targetWrites = calls.filter(([, path]) => path.startsWith(`${repo}/projects/pal/`))
    const correct = finalText === beforeTarget && targetWrites.length === 0
    if (MODE === 'contract')
      assert.equal(correct, true, 'E02: 冲突拒绝或保留都必须不覆盖作者新值、项目零写入')
    note(
      'E02',
      correct ? 'covered' : 'reproduced',
      JSON.stringify({
        result,
        authorPreserved: finalText === beforeTarget,
        targetWrites,
        pending: hasPendingMigrationTransaction(repo),
      }),
    )
  }
  ...
  if (want('E03')) {
    ...
    assert.ok(recoverError.includes('提交窗口被修改'), `E03: 恢复路径同等复核,实际 ${recoverError}`)
  ...
  const runMaterialize = (repo, path, bytes) =>
    materializePalAssets({
      repo,
      catalog: {
        version: 1,
  ...
  for (const id of ['E06', 'E08'].filter(want)) {
    const repo = freshRepo(id.toLowerCase())
    ...
    calls = []
    const result = expectedRejection(
      () => runMaterialize(repo, 'assets/migrated/videos/a.mp4', Buffer.from('NEW')),
      pathRefusal,
    )
    const changes = [...calls],
      outsideAfter = get('/virtual-outside/videos/a.mp4', 'utf8')
    const correct = result.rejected && outsideAfter === 'OUTSIDE_ORIGINAL' && changes.length === 0
    let json
    if (id === 'E08') {
      const repo2 = freshRepo('json-guard')
      links.set(`${repo2}/projects/pal/content`, '/virtual-outside/content')
      putProject(repo2, before)
      calls = []
      const refused = expectedRejection(
        () =>
          commitMigrationTransaction(repo2, [
            {
      ...
      assert.equal(hasPendingMigrationTransaction(repo2), false)
      assert.equal(
        get('/virtual-outside/content/items.json', 'utf8'),
        serializeMigrationJson(before, file).toString(),
      )
      json = {
        refused,
        calls: [...calls],
        boundary: 'JSON事务允许临时staging目录操作，目标/正式journal未提交',
      }
    }
    if (MODE === 'contract')
      assert.equal(correct, true, `${id}: 物化必须以明确路径拒绝在任何修改前终止并保留原字节`)
    note(
      id,
      correct ? 'covered' : 'reproduced',
      JSON.stringify({ result, beforeFiles, outsideAfter, changes, json }),
    )
  }
  if (want('E07')) {
    const outcomes = []
    for (const variant of ['deep', 'self', 'race']) {
      const repo = freshRepo(`e07-${variant}`),
        path = 'assets/migrated/a.bin',
        bytes = Buffer.from('NEW')
      if (variant === 'deep') links.set(`${repo}/projects`, '/virtual-outside/deep')
      else if (variant === 'self')
        links.set(`${repo}/projects/pal/${path}`, '/virtual-outside/self.bin')
      else {
        put(`${repo}/projects/pal/${path}`, 'STALE')
        readHooks.set(`${repo}/projects/pal/${path}`, () =>
          links.set(`${repo}/projects/pal/assets/migrated`, '/virtual-outside/race'),
        )
      }
      const target =
        variant === 'deep'
          ? '/virtual-outside/deep/pal/assets/migrated/a.bin'
          : variant === 'self'
            ? '/virtual-outside/self.bin'
            : '/virtual-outside/race/a.bin'
      put(target, 'ORIGINAL')
      calls = []
      const result = expectedRejection(() => runMaterialize(repo, path, bytes), pathRefusal)
      const external = get(target, 'utf8'),
        externalChanges = outsideWrites()
      const correct =
        external === 'ORIGINAL' &&
        externalChanges.length === 0 &&
        (variant === 'self'
          ? !result.rejected
            ? files.get(`${repo}/projects/pal/${path}`)?.equals(bytes)
            : true
          : result.rejected)
      outcomes.push({ variant, result, external, externalChanges, correct })
    }
    assert.equal(
      outcomes.find((x) => x.variant === 'self').correct,
      true,
      'E07: 自身链接必须安全替换或明确拒绝，外部文件不变',
    )
    const failures = outcomes.filter((x) => !x.correct).map((x) => x.variant)
    if (MODE === 'contract')
      assert.deepEqual(failures, [], 'E07: 独立验证深父链与TOCTOU拒绝，不能首红掩盖其它子场景')
    note('E07', failures.length ? 'reproduced' : 'covered', JSON.stringify({ outcomes, failures }))
  }
  restoreVirtualFs()
  ...
  if (want('E12')) {
    const keep = [
      ...
      {
        what: '当前raw到canonical producer与已批准源解码',
        where: 'packages/migrate/src/pal-migration.ts:391-499',
        why: '保留真实在用的当前producer；历史profile没有消费者时按既定current-only纪律准备退役，不按文字名称误删当前源格式工具',
      },
    ]
    ...
    const candidates = [
      ...
      },
      {
        ...
        zeroProd: true,
        plan: '先核真实输入/唯一消费者与既有批准；无获准消费者则依current-only纪律在正式卡退役，旧测试迁至当前合同；不发明新的历史重放产品需求',
      },
    ]
