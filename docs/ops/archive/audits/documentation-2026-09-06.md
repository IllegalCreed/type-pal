...
`docs/phase2/archive/designs/content-schema.md:8-30`、`save-system-design.md:6-30` 和
`capability-map.md:4-8,132` 也明确当前为 content20 / SAVE8。以下文档却仍把 19 写成当前或适用版本：

- `packages/migrate/README.md:3-5`；
- `docs/phase2/archive/designs/asset-pipeline.md:3-6`；
- `docs/phase2/archive/designs/script-system-design.md:3-11`；
- `docs/phase2/guides/shared-script-author-guide.md:3-9,91-93,125-129`；
- `docs/phase2/archive/designs/editor-design.md:251-256`；
- `docs/phase2/archive/designs/project-lifecycle-design.md:326-335`。
...
- `:31,41,75` 仍以“为 MMO 留口”作为二阶段理由；
  `docs/phase3/backlog.md:28-34` 后来明确裁决二阶段**不为 MMO 留任何口**，只因
  当前产品需要而选择干净、可测架构。
...
- `docs/ops/archive/acceptance/2026-07-checklist.md` 混合 2026-07 已完成项目和未勾项目，仍写状态面板不存在等旧事实，且没有
  “历史验收批次”标识；它不能继续作为全项目当前验收清单。

`docs/ops/guides/browser-verification.md` 和 acceptance checklist 也没有进入
...
- `docs/phase2/archive/plans/project-lifecycle-p1-filesource-plan.md:3-17` 与
...
- `docs/phase2/archive/audits/phase1-audit-tracker.md:59-69,81,94-99,107,122` 仍把投掷、毒、商店、过场、
  跟随者、资源 manifest 等写成未实现；capability map 已记录后续完成事实。它是 2026-07 审计快照，不是当前缺口表。
- `docs/phase2/archive/plans/project-foundation-plan.md:5`、`foundation/actor-model-design.md:5`、
  `foundation/actor-c0-plan.md:5`、`foundation/e7-follower-riding-design.md:3` 仍写待审/待实现；对应架构和能力早已落地。
- `docs/phase2/archive/designs/model-design.md:3` 仍写 C1-2 review，`dialogue/visual-design.md:3` 仍写草案；
  二阶段总入口又把 visual plan 标待实现。
- `docs/phase2/archive/designs/content-schema.md:1-8` 顶部仍叫 P0 草案，下面却追加 current content20 契约；
...
```text
docs/lore/ideas/guijie
docs/ops
docs/ops/audits
docs/ops/archive/evidence
docs/phase1/status
docs/phase2/archive/designs
docs/phase2/guides
docs/phase2/specs
docs/phase2/archive/plans
docs/phase2/archive/audits
docs/phase2/archive/slices/indoor
```
...
| 源文件 | 次数 | 性质 / 处置 |
|---|---:|---|
| `docs/lore/ideas/guijie/beat-sheet.md:173,175` | 2 | `../../` 多退一级；当前目标存在 |
| `docs/ops/archive/tasks/done/OPS-TST-PERF-release-wallclock.md:212-214,219,232` | 5 | 相对层级错；大部分目标又已被 current-only 删除，应改历史 commit/path，不恢复旧代码 |
| `docs/ops/archive/tasks/done/OPS-TST-PERF-test-fixture-stratification.md:39-44` | 6 | 同上；仅部分当前目标仍存在 |
| `docs/ops/archive/tasks/done/X7-1-manifest-project-workbench.md:46,54-63` | 11 | 全部少退一级；AGENTS 与大多数源码目标存在，历史 `EntryPointTab.tsx` 已删除 |
...
| `docs/phase2/archive/designs/project-design.md:42` | 1 | 历史 `editor/src/index.ts` 已删除 |
| `docs/phase2/archive/designs/project-lifecycle-design.md:53` | 1 | 历史 `reforge/src/loader.ts` 已删除 |
| `docs/phase2/archive/audits/phase1-audit-tracker.md:149` | 1 | 重复 `foundation/` 路径 |
| `docs/phase2/archive/designs/menu-design.md:10` | 1 | 少退一级到 phase3 |
...
- `docs/ops/archive/tasks/done/ED-DS-3-editor-design-system-adoption-gate.md:69-70` 的两张旧卡名；
- `docs/ops/archive/tasks/done/ED-PROJECT-STARTUP-IA-1-project-entry-startup-workbench.md:157` 的旧入口任务卡名。
...
- 新的大卡把顶部保留为短的当前摘要、准入与最终回执；大段独立复核、性能日志和截图清单放
  `docs/ops/archive/evidence/<task-id>/`，卡内链接过去。
