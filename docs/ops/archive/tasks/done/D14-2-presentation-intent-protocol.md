...
- 代码锚点:
  ...
  - `docs/phase2/archive/audits/engine-debt-audit.md` §6（Interpreter/CutsceneController 方向）、
...
- 文档:
  - 协议词汇表入 `docs/phase2/archive/designs` 或 `docs/phase2/presentation`；backlog 议题 5/12/14
    剩余②状态更新。
- 视觉 / 手工验证:
...
- 跳过的检查及原因: 视觉回放验收按 **D28 拍板改走分段 e2e**（求雨/酒剑仙/结局未实跑段
  转 e2e-checkpoints 碎片验证，见「分段 e2e 待办」）；Codex 自证到类型+单测+构建层。
...
### 分段 e2e 待办（D28 拍板，2026-08-07）

- 求雨 RNG / 酒剑仙 RNG / 结局视频三段未实跑 → 建 e2e-checkpoints 碎片：
  起始存档（`?e2e-load` 秒进）+ 流程说明 + 验证点（帧序 / 文本 / fade 值序列，
  K7 确定性）+ 结束存档接力。
- 碎片边界由作者（仙剑专家）定义；验证走确定性输入 + `__tpE2e.dumpSave()` 状态断言，
  不做 Kimi 视觉并排门禁。
...
- 2026-08-07 用户: 拍板 D28——演出/分镜段最终验证走分段 e2e(求雨/酒剑仙/结局转
  e2e-checkpoints 碎片),Kimi 视觉降级为抽验;见「分段 e2e 待办」。
...
任务卡: docs/ops/archive/tasks/done/D14-2-presentation-intent-protocol.md
