...
- 已拍板决策 / 铁律:
  - `docs/phase2/archive/designs/model-design.md:8-12,69-108`: 功能/结构控制码必须在数据生产期消失，运行时只消费
    结构化字段和 locale 富文本。
...
- 已知坑 / 审计文档:
  - `packages/game/src/present/dialog-box.ts:56-170`: 一阶段完整真值；颜色和速度状态可跨源行持续，`~NN`
    立即结束当前行并忽略其后字符。
  - `docs/phase2/archive/audits/n-dialog-text-audit.md:290-334`: 二阶段结构化目标与已知跨段速度缺口。
...
- 2026-07-15 Codex: build 完成并签 review accept。唯一 cue/rows schema、旧工程 upgrader、有状态迁移 decoder、
  ...
  `docs/phase2/archive/audits/n1-dialogue-migration-audit.md`。Next:Opus 代码/架构/视觉主审；不得标 done。
...
- 2026-07-15 Codex: D1 文档返工已落地。独立重扫 `data/extracted/events/all.json` 得 18 个 `-`/17 站点,
  ...
  Evidence: `docs/phase2/archive/audits/n1-dialogue-migration-audit.md`「残留与合法性」+ 本卡 Build 摘要。
