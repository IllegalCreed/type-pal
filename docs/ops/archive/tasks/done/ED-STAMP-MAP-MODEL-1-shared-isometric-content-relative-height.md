...
| 第一阶段 | N/A：第一阶段没有组合模板作者工作台；本卡不改变一阶段忠实还原数据。 | `docs/phase2/READ-FIRST.md:24-41`；`docs/phase2/reference/phase1-knowledge-harvest.md` 无组合作者模型条目。 |
...
- 已拍板决策 / 铁律：
  ...
  - `docs/phase2/decisions.md:374-395`：高度属于格子实例，不属于 tileset；本卡保留该核心，只纠正绝对/相对空间和
    `flat` 分叉。
  - `docs/ops/archive/tasks/done/W7G-composite-tile-stamps.md:20-25,138-188`：模板非链接、runtime 只消费普通矩阵、稳定局部层与
    显式 collision；旧三签只作为历史证据，不授权本卡的新前提。
  - `docs/ops/archive/tasks/done/ED-MAP-MULTI-TILESET-1-multi-tileset-map-authoring.md`：Kimi/GLM 已用 223 图 / 4.0M 非空格
...
任务卡：docs/ops/archive/tasks/done/ED-STAMP-MAP-MODEL-1-shared-isometric-content-relative-height.md
