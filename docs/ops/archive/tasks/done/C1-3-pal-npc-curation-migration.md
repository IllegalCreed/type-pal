...
**GLM 非阻塞观察（不阻塞 done）**：
`docs/ops/archive/evidence/C1-3-first-batch-proposal.md` 人类可读批准表已生成，但该 evidence 目录不在
git 跟踪范围（docs/ops/archive/evidence/ 未入库）。批准的权威记录是 `pal-c1-npc-first-batch.ts` 中的
...
- 第一批只读 draft 已生成并对真实 parent/source 做逐 locator 预校验：李大娘
  ...
  `docs/ops/archive/evidence/C1-3-first-batch-proposal.md`。
...
任务卡：docs/ops/archive/tasks/done/C1-3-pal-npc-curation-migration.md
当前状态：review；Codex 已 accept；Kimi/GLM pending；不得标 done，不得修改实现文件。
必读：AGENTS.md、CLAUDE.md、docs/phase2/READ-FIRST.md、C1-3 任务卡、
docs/ops/archive/evidence/C1-3-first-batch-proposal.md，以及 C1-1/C1-2 任务卡。
...
先读: AGENTS.md；docs/phase2/READ-FIRST.md；本卡全文；docs/phase2/guides/actor-presets.md；
