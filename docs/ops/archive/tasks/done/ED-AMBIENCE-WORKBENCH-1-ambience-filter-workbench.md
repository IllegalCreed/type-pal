...
| 第一阶段 | 第一阶段按 palette/nightColors 在最终呈现链选择夜色，证明氛围不是单张地图或单个精灵属性；仍不存在可复用的二阶段作者工作台。 | `packages/game/src/core/event-system.ts:4005-4012`; `packages/game/src/core/palette-fade.ts:91-99`; `docs/phase2/archive/designs/ambience-design.md:12-34`。 |
...
| 本任务目标 | 不改 schema/runtime 语义；左目录、中 Hero + 字段 + 真实场景滤镜预览、右引用 / 说明。预览底图走现有场景 renderer，multiply 走 runtime/editor 唯一共享 helper。 | 用户 2026-08-22 明确要求“列表位置应该放列表，中间才是标题、基本信息，还有一个氛围滤镜预览的场景效果，右侧属性面板……你设计”；`docs/phase2/specs/editor-design-system.md:458-479`。 |
...
- 已拍板决策 / 铁律:
  ...
  - `docs/phase2/specs/editor-design-system.md:458-479`: 对象型列表/主编辑/Inspector 与媒体型大预览合同。
  - `docs/phase2/specs/editor-design-system.md:440-455`: 引用/诊断共享行骨架，危险删除展示影响并阻断被引用对象。
  - `docs/ops/tasks/ED-REFERENCE-UI-1*`、`ED-DIAGNOSTIC-UI-1*`: 引用与诊断消费共享组件；普通诊断内联，不为零问题虚构新 Tab。
  - `docs/ops/archive/tasks/done/ED-AUDIO-WORKBENCH-1-audio-resource-workbench.md`: 同族“左目录、中当前对象+真实预览、右次级信息”工作台，
...
- 已知坑 / 历史:
  - `docs/phase2/archive/designs/ambience-design.md:59-67`: 氛围在最终帧、战斗也生效；编辑器场景画布此前故意恒白天。
  - `docs/ops/archive/tasks/done/ED-CATALOG-CONTROLS-1-global-catalog-controls.md:94-100`: 旧审计因氛围只有短表而排除搜索；本卡改变的是页面 IA，
    不以强塞搜索为“统一”。
  - `docs/ops/archive/tasks/done/ED-INSPECTOR-TABS-1-global-inspector-tabs.md:88-97`: 旧页无 Inspector 的历史判断；本卡新增真实三栏后以新裁决为准。
- 不得重新引入:
...
- 修改文件:
  ...
  - 规范与任务记录：`docs/phase2/archive/designs/ambience-design.md`、`docs/phase2/specs/editor-design-system.md`、本任务卡与看板。
- 实现摘要:
