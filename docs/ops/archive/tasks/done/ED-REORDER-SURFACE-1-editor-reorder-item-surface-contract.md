...
| 当前二阶段 | registry 有 17 家族 / 29 adoption / 32 数据路径，但没有 contentSurface / railLayout 分类；已确认 2 个严格 edge-to-edge、5 个专用连续结构、16 个已有 frame、6 个 inset/gap 无完整 frame。脚本方案的历史产品真值是横向对象卡；reorder wrapper 插入真实 div 后旧 child selector 失活，曾退化为全宽大行。 | `packages/editor/src/ui/design-system/reorder-adoption.json:1-476`；`docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md:3618-3631`；`packages/editor/src/ui/design-system/reorder.tsx:1122-1145`；`packages/editor/src/ui/ScriptEditor.tsx:188-268` |
| 本任务目标 | 29 项逐项登记内容 / surface 分类；6 个红项迁到正确 owner；门禁同时证明组件语义、边界、gap/inset、窄宽可用性，合法对象卡和连续列表保持不变。 | `docs/phase2/specs/editor-design-system.md` 的 DS-F.4、DS-C.4d、RF-21 |
...
- Kimi:
  - premise: **verified（2026-08-30 独立直读 registry 全 29 项、公共 reorder/recipes 层、四桶代表面 TSX+CSS、
    ...
    - **对象卡历史真值实锤**:N3-1 P7-R4(2026-07-25)用户验收约束“方案必须平铺;每张方案卡直接提供
      方案详情”(docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md:3618-3631);当前
...
任务卡: docs/ops/archive/tasks/done/ED-REORDER-SURFACE-1-editor-reorder-item-surface-contract.md
...
先读: 本任务卡全部签节(尤其 KS1-KS4 与 GM-S1~S5 必落钉)、AGENTS.md、
  docs/phase2/READ-FIRST.md、docs/phase2/specs/editor-design-system.md(DS-F.4、DS-C.4d、RF-21)、
