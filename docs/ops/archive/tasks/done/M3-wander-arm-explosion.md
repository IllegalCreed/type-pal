...
- 既有设计：`docs/phase2/archive/designs/script-model-m3-design.md:101-116,134-140` 已明确“goto 单前驱内联、多前驱提共享 Script(callScript)”和 M3c `callScript` 共享库；当前实现漏掉了共享库部分，才退化为递归内联。
- 分片真值：`docs/phase1/04-decisions.md:24`、`docs/phase1/05-events-schema.md:113-127` 与 `packages/pal-extract/src/events/slice.ts:1-203` 已实现“单场景可达归 scene、多个场景可达归 shared、全局物品/法术/敌 AI 强制 shared”。`docs/phase2/archive/designs/content-schema.md:23-40,50-67,125-147` 又明确第二阶段场景自包含、跨场景稳定 ref、`scenes/ + shared/`。
...
- 已知坑 / 审计：`docs/phase2/archive/audits/n-event-script-audit.md`、`am-asset-migrate-audit.md`、`phase1-knowledge-harvest.md` E6。
...
- 缺签豁免: N/A
- done 准入结论: **三方审查签字齐（Codex+Opus+GLM accept），用户于 2026-07-13 确认全部验收通过，任务转 `done`。**
...
- Reviewer: Opus + GLM
- 审查结论: **Codex、Opus、GLM 三方 accept**；实现、自测、代码/架构审查、迁移覆盖、体积门禁和前台活体验证均通过，详见签字行。
- Opus 非阻塞观察(O1-O3):
...
- 必须返工项: 无
- Accept / rework: **三方 accept，用户验收通过，任务已完成。**
...
- 2026-07-13 GLM: 覆盖/门禁复验签 **accept**；654 报告口径、43,503 指令边、三重体积门禁、chunk 引用完整性和 overlay 白名单五项通过。Next: 用户验收。
- 2026-07-13 User: 确认全部验收通过，批准任务转 `done`。Next: 无，归档。

## 下一位 Agent 提示词

无下一位 Agent 提示词，三方审查和用户验收均已通过，任务完成并归档。
