...
- 已知坑 / 审计文档：
  - `docs/ops/archive/tasks/done/M3-wander-arm-explosion.md:89-138,329-349`：M3 已完成分片、resolver、call/jump、存档 ref 化和体积门禁；N6 不能重复发明运行时。
  - `docs/ops/archive/tasks/done/MG2-incremental-migration-merge.md`：人工修改必须通过 base/theirs/ours 合并保留；作者脚本属于 ours-only 内容。
  - `docs/phase2/archive/designs/script-system-design.md:69-87,120-143`：早期设计已预留 `callScript`，同时明确模板与共享脚本是两层概念。
...
- 修改文件:
  ...
  - `docs/phase2/guides/shared-script-author-guide.md`：命令/模板/共享脚本选择规则与内部脚本边界。
- 实现摘要:
