...
| 当前二阶段 | 已有 DS 基础设施，但页面采用不完整；审计仍发现大量原生控件、legacy class、inline style 与独立滚动 / overlay 实现 | `docs/phase2/specs/editor-design-system.md:340`、`:936`；`packages/editor/scripts/audit-legacy-controls.mjs`；当前 census 见下 | verified |
...
- `docs/phase2/specs/editor-design-system.md:340`：按钮与 legacy 控件规范。
- `docs/phase2/specs/editor-design-system.md:936`：v2.8 中“同语义必须共享 owner，不做截图补丁”的用户裁决。
...
- `docs/ops/archive/tasks/done/ED-DS-2-editor-design-system-foundation.md`：已完成的基础组件建设，不重开。
- `docs/ops/archive/tasks/done/ED-AUDIT-2-editor-systematic-audit.md`：已完成的历史审计，不把旧完成状态当成当前覆盖证据。
...
- [x] `docs/phase2/specs/editor-design-system.md` 更新公共合同、采用规则、allowlist 与审查清单。
