...
- 用户明确指出：**默认 UI 在引擎，不在工程**。这与
  `docs/phase2/archive/designs/project-design.md:21-25,68-83,162-168,186-207` 的早期拍板一致。
- “工程自包含”指**工程拥有的内容资源全部位于工程内**，不是把引擎运行壳的默认 UI、字形和光标
  复制进每个工程。A5 工程 ZIP 只打工程；A8 独立发行包由“引擎壳 + 工程”共同闭包。
- `docs/phase2/archive/audits/a7-resource-closure-audit.md:35-50` 后来声称项目生命周期/路线图覆盖了早期
...
- 已拍板决策 / 铁律:
  ...
  - `docs/phase2/READ-FIRST.md:9-20`:运行时只保留一套干净模型；旧格式只在边界升级；稳定 id 不靠下标；
    迁移缺陷先修上游并重生成。
  - `docs/phase2/archive/designs/project-design.md:21-25,68-83,162-168,186-207`:工程内容资产与引擎
    chrome 的原始边界真值。
  ...
  - `docs/phase2/archive/designs/project-lifecycle-design.md`:FSA/HTTP/保存/克隆使用同一工程文件模型；其中把默认 UI/字形
    算工程素材的文字按本卡用户裁决勘误。
- 一阶段真值:
...
- 已知坑 / 审计文档:
  - `docs/phase2/archive/audits/a7-resource-closure-audit.md`:catalog/resolver/MG2 总设计可复用，但 §2.1 和 A7-2
    分片边界需按本卡纠正。
  - `docs/ops/archive/tasks/done/A7-0-resource-closure-registry.md`、`A7-1-sfx-asset-closure.md`、
...
- 修改文件:
  ...
  - 文档/任务：A7 审计、project design/lifecycle、asset pipeline、roadmap、capability-map、开发命令与
    [`A7-2 结果报告`](../../../../phase2/archive/audits/a7-2-static-images-engine-chrome-report.md)。
- 实现摘要:
