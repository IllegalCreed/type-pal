...
Branch: main（来源候选`codex/cursor-tools-tests-r1`，tip`85f2a824`）
...
- Kimi/GLM：隔离准备阶段未请求，不代签；Cursor不替代其它席位。
- 原build准入：not opened；下方记录用户随后对本包实现、验证与提交的授权，不倒填原阶段签名。
- done准入：尚待本卡收口；不以材料accept或单次测试通过冒充全部验收。
- 原分配只允许隔离实验；正式接入与修复归Codex实施，见下方交接。

## 下一位Agent提示词

以下为隔离准备阶段的历史提示词；现行实施判断见文末，不再要求Cursor重做。
...
## Codex正式接入与修复 — 2026-09-25

- 用户明确要求“验证之后提交推送”，由Codex完成本卡正式测试接入、目录工具漏改修复与质量门。
  此授权只针对本卡，不代签Kimi/GLM、不改变其它任务门禁；原draft准备文字保留为时点记录。
- Cursor候选`85f2a824`已由Codex在独立分支`e11489dd`签材料accept。
  本次将有业务断言的T01～T05与T07/T08迁至文档工具和编辑器正式runner；
...
- 本轮用户要求Codex验证后提交推送，连同其先前授权的架构治理由Codex独立实施，
  作为本卡F1工具与文档工具窄修的单席推进依据；Kimi/GLM未参与、不代签，豁免不外推到其它任务。
  Codex自验accept，本卡done。工具路径漏改已修且正式回归转绿；完整E2E/剧情视觉不借此关闭。

交接：无下一位Agent提示词，本卡收口；main推送与已合并工作树清理由Codex统一完成。
