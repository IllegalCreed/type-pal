...
- `docs/phase2/reference/phase1-knowledge-harvest.md:142-154`：autoScript 必须用真实执行链验证，
  不能用手写近似模拟；触发脚本与自动行为的历史语义不能在重构中混为一谈。
- `docs/phase2/reference/phase1-knowledge-harvest.md:310-312`：一阶段 call stack 是旧架构机制，
  二阶段目标应是结构化 AST，而不是把 IP/label 换一个名字继续暴露。
- `docs/ops/archive/tasks/done/R2-script-single-model.md`：唯一语义命令模型和迁移 fail-loud 继续成立；本卡不复活旧
  opcode 解释器。
- `docs/ops/archive/tasks/done/N6-shared-script-authoring.md`：作者共享脚本的稳定 id、引用安全、lazy load 和 MG2
...
- 历史结论: pending（当时等待验收 `c3d620a9` P7-R12 场景出口触发竞态修复；P7-R11 已被
  后续候选取代）。
- 用户结论: **accept（2026-08-06，确认性验收）**。用户基于最终摘要确认 N3-1 通过；C8/ED-5I
  联合回归亦已完成。与卡头状态、下方最终验收记录及提交 `e70987d6` 一致。
- 后续任务: 无。N3-1 已 `done`；编辑器摘要行 polish 仅作独立后续观察。
...
- **独立报告已齐**：
  - Kimi：`docs/ops/archive/audits/kimi-p7-r13-source-semantics-audit.md`；
  - GLM：`docs/ops/archive/audits/glm-p7-r13-source-semantics-audit.md`；
  - Codex 合并裁决：
    `docs/ops/archive/audits/codex-p7-r13-three-way-adjudication.md`。
...
- 敌方 `0x28` 新毒首次成功落槽时，立即执行一次 `enemyTicks[0]` 并保存推进后的 cursor；
  ...
  `docs/phase2/archive/designs/poison-system-design.md`。
...
任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
...
docs/ops/archive/audits/codex-r13-4-confirm-control-flow-audit.md，以及
...
`docs/ops/archive/tasks/done/OPS-TST-PERF-test-fixture-stratification.md` 已把日常开发门与发布门拆开：
...
docs/ops/archive/tasks/done/OPS-TST-PERF-test-fixture-stratification.md；packages/migrate/src/experimental/
...
```text
任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md；N3-1 总体仍 build，R13-6A
...
`docs/phase2/reference/phase1-knowledge-harvest.md:389,418,481,503`；当前模型和消费点见
...
`skillUseCounts` + 满 9 移除技能（见 `docs/ops/archive/tasks/done/JS1-jiu-shen-nine-use-limit.md`）。
...
- 剩余 110 sites 全部为 `unclassified-reachable-source-site`：actors 36/38
  ...
  `docs/ops/archive/tasks/done/B11-1-player-casualty-scripts.md`（用户 2026-08-05 拍板，draft）。
...
任务卡: docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md（R13-Z 节
  「R13-6C 设计草案」）
...
##### 段转移备注定位（2026-08-05，Codex 插桩核实，纠正此前"×1 站点"表述）

为裁决「引用目标含段转移(按 end 处理)」观察，Codex 在 translate-events.ts:360 插桩
（定位后已还原）并复核冻结历史报告，结论：

- 该备注键在冻结历史 R13-4 v9 报告（R13-Z 权威输入）中计数为 **573**，不是 1；
  R13-Z 门禁输出 `translation-note:...=1` 的 "1" 是**观察数**（每个备注键聚合为一条
  观察），不是触发次数，也不是单个站点。此前卡内「×1（审计池备注）」表述不精确，
  应予纠正为「×1 观察 / 573 次触发」。
- 该观察无站点地址（报告级计数，不挂 source site），且**无 closure evidence 机制**：
  对比 setPalette 备注有 `pal-palette-resolution` 证据族 + 用户拍板闭环，本备注键
  没有对应证据族，R13-0 设计的「逐条验证 target transition disposition」未落成
  机器可校验的闭包。
- 573 次触发均为「引用目标体以 advance/reset（跨段转移）结束 → 翻译按 end 处理」，
  分布全场景（s001-s267、shared/scc-*）；路径样例（如
  `L_133@e7 > scene/s001/L-94/e9/... -> L_94`）已留档。

待用户裁决（A/B 重新评估）：

- **A（推荐）**：接受为已知排除——573 池整体按「目标体跨段继续由所在 flow 的
  stage 图承接，引用处按 end 停」的既定翻译语义处置，新增一条 resolution evidence
  （参照调色盘先例），观察归零；不重开逐站点审计。
...
先读: AGENTS.md、docs/phase2/READ-FIRST.md、本卡 R13-Z 节（发布/重放/金丝雀清单）、
  docs/ops/archive/tasks/done/B11-1-player-casualty-scripts.md（伤亡演出真值）。
验证清单:
...
##### Codex 对两个记录项的核查（2026-08-06）

**记录项 1（s093 默认落点“虚空”）→ 误判，迁移忠实、落点合法**：

- 溯源：s093 无 onEnter/start；默认入口来自 scene-082 事件流 `setPartyPos → loadScene
  (sceneId=94→93)`（gap=3，迁移到达扫描命中），`partyPosToGrid` 换算 = (92,69)——**忠实
  来自原版源数据**，非迁移缺陷、非兜底值。
- 等距菱形网格（grid.ts）：地图名义 64×128 是菱形轴上限，合法 col/row 范围是
  [0, width+height−1]=[0,191]；**全仓 62/80 场景入口 col > 地图名义宽度**（s000=96、
  s003=143 等），(92,69) ∈ 合法区间。Kimi 以 col 92 > width 64 判“虚空”是把正交
  网格套在 iso 网格上的误读；编辑器同场景地图渲染正常亦佐证。
- 直接 `?scene=s093` 全黑：s093 无 onEnter 脚本、真实游玩总经相邻场景命名落点进入；
  直达加载非受支持路径，不属迁移缺陷。**结论：不改迁移，卡内记误判撤销**。

**记录项 2（编辑器切换指令摘要行裸 id）→ 属实，非阻塞 polish**：

- `cmd-detail` 摘要行显示 `legacy-001`（行为 id），而槽列表/编辑表单显示具名标签
...
任务卡: docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md（「C8/ED-5I 联合
  验收回归清单」）、docs/ops/archive/tasks/done/C8-item-use-mechanisms.md、
...
```text
返工任务: N3-1 R13-6C+6D Kimi counter R1-R5；任务卡 docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
...
```text
复审任务：N3-1 R13-6B implementation review；任务卡 docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
...
docs/phase2/reference/phase1-knowledge-harvest.md；本卡 R13-6A formal publication closure、
...
先读：AGENTS.md、CLAUDE.md、docs/phase2/READ-FIRST.md、docs/phase1/game-mechanics.md、
docs/phase2/reference/phase1-knowledge-harvest.md；本卡 R13-6A publication evidence、R13-6B
...
- 2026-08-03 GLM：完成 R13-6B 源数据/迁移/测试矩阵设计审查，签 **agree**，附 G1 风险钉。
  ...
  **304 须按即死形态（独立 kill/instantKo effect）处理，不得套 resourceDelta(-1)**。
  另：`docs/phase2/archive/audits/script-census.md:83` 把 0x5F 误标为 jumpIfScene（实为 killPlayer，
...
GLM 签字或任务状态。任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
...
- docs/ops/archive/audits/codex-r13-5-enemy-source-semantics-audit.md；
...
任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
审计：docs/ops/archive/audits/codex-r13-5-enemy-source-semantics-audit.md
...
- docs/ops/archive/audits/codex-r13-4-confirm-control-flow-audit.md；
...
任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
审计：docs/ops/archive/audits/codex-r13-4-confirm-control-flow-audit.md
...
- docs/phase2/specs/{script-system-design,save-system-design}.md。
...
- docs/ops/archive/audits/glm-p7-r13-source-semantics-audit.md、
...
- docs/ops/archive/audits/codex-p7-r13-three-way-adjudication.md（分批与口径的唯一事实源）
- 两份独立审计报告（证据，不是签字）
...
任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
独立报告：docs/ops/archive/audits/kimi-p7-r13-source-semantics-audit.md
GLM 报告：docs/ops/archive/audits/glm-p7-r13-source-semantics-audit.md
Codex 合并裁决：docs/ops/archive/audits/codex-p7-r13-three-way-adjudication.md
...
任务卡：docs/ops/archive/tasks/done/N3-1-script-control-flow-modernization.md
独立报告：docs/ops/archive/audits/glm-p7-r13-source-semantics-audit.md
Kimi 报告：docs/ops/archive/audits/kimi-p7-r13-source-semantics-audit.md
Codex 合并裁决：docs/ops/archive/audits/codex-p7-r13-three-way-adjudication.md
当前状态：rework；P7-R13 build 准入 blocked。只读设计复审，不得修改实现、生成产物、
baseline、任务状态或 P7 最终 accept。
...
先读:
  - AGENTS.md、CLAUDE.md、docs/phase2/READ-FIRST.md；
  - docs/phase2/specs/{phase1-knowledge-harvest,n-event-script-audit}.md；
...
先读:
  ...
  - docs/phase2/specs/{script-census,n-event-script-audit,
...
先读:
  ...
  - docs/phase2/archive/designs/save-system-design.md。
重点源码:
...
先读:
  ...
  - docs/phase2/archive/designs/script-system-design.md；
