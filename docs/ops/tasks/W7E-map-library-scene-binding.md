...
- 已拍板决策 / 铁律:
  ...
  - `docs/ops/tasks/ED-1-editor-authoring-closure-audit.md`：三签确认 map index、稳定 id、场景绑定和显式旧格式迁移方向。
...
- 已知坑 / 审计文档:
  - `docs/phase2/editor/editor-authoring-closure-audit-2026-07-13.md` P0-2：无索引导致未引用地图重开丢发现能力。
  - `docs/ops/tasks/W7D-nlayer-map-schema.md`：OwnMap v1 图层/碰撞结构已定，不得重新考证或改写。
  - `docs/ops/tasks/W7B-tileset-library.md`：tileset 注册与上传管线已定，地图只引用稳定 tileset id。
  - `docs/ops/tasks/MG2-incremental-migration-merge.md`：迁移产物必须三方结构化合并，不能因新 index 退回 atomic 整文件冲突。
- 不得重新引入:
