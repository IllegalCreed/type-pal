...
- 第二阶段完整任务卡必须包含上下文锚点;无锚点不得进入 `build`。
...
- 必改二: agree。已在完整模板和工作流加入第二阶段上下文锚点,并写明无锚点不得进入 `build`。
...
**重点 2：上下文锚点必填项**——**⚠ 阻塞项**。锚点要求只写了"第二阶段任务"，第一阶段任务（packages/game / pal-extract）无锚点要求。但第一阶段有 CLAUDE.md 忠实还原铁律 + engineering-notes 432 fix + 73 单元审计 + harvest + game-mechanics，这些都是"动格前必读"的锚点。如果第一阶段任务无锚点要求，铁律 #9（动格前先读一阶段知识）在协作流程里无约束力——这正是我们做 73 单元审计的全部意义。**建议：把"第二阶段任务"扩成"所有非小改任务"，或补一句第一阶段锚点至少包括 CLAUDE.md + 相关 engineering-notes 章节。**
...
**结论**：五项必改全部落实、Opus 立场转述准确无歪曲、模板完整。**accept**——阻塞项（第一阶段锚点）补上即定稿，其余建议非阻塞可下轮顺手。
