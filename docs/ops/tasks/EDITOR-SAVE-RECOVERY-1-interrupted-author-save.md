...
以下是固定验收合同，不自行等同已跑结果；当前逐项证据由最终收口及各批回执提供，供两席终审核实。测试数字来自实际提交树，不预填预计通过数。
...
#### GLM · batch-r1 C1–C4返工交接原文（3fe58baa，未接收）

以下保留GLM候选原回执，不代表Codex认可全部声明；撤追踪/计数问题由后续本席复核纠正。
...
### Codex · fa8af7c2返工复核与接收侧补证（2026-09-11）

用户允许裁定剩余待证的优先级/归属。原候选仍有合并冲突标记、W6清空IO轨迹、S4不完整替身及回执不足，
**不原样accept**；本席保留其有效测试，在主树做限定接收修订，不更改生产或产品前提。
原候选定向实际11文件/181项（非180）；Codex新增真实可选ambiences删除两态、补W6同字节IO反证、
Lock形状/读后释放正控并清理文档冲突及unused。首次全仓还抓到B3叠加故障造成的顺序依赖，
...
### Codex · 574012e5剩余项接收复核（2026-09-12，counter）

本续批不接收；**已接收的21项及main的6,201项fast基线不回滚**，r2设计签字保持。
...
不替GLM重写整套IDB模型后称其原候选通过。修同一续批一次性交付，不重签、不转Kimi、不标done。
...
- C3：显式abort/S1同库等已修，但requestError仍发complete并提交；独立替身oracle红。
  S3实际只失败于getAll、未进入put；须补定点写请求错误与同库无残留。
- C4：43唯一ID齐，摘要计数仍错（主分类应22/3/14/1/3）；C4a/b删除await previous仍绿、尚未证明排队互斥。
  C4c写错优先负控红有效；读回错只证端到端冲突，移除本点仍在后续守卫拒绝，须收窄声明或补最早边界见证。
- 本席定向/相邻12文件200项绿、editor typecheck绿、biome exit0（1 info）；原NC5及S2提前resolve负控红。
  未跑本续批完整check/ratchet/严格fast，不把GLM的6,706项或覆盖回执写成独立实跑。
...
本轮为无痕窗口里的**关闭编辑页**与OS真实目录，不冒称整浏览器重启；原生OS目录重启/恢复中撤权仍缺补证。
...
### Codex并行切片 · project-io边界（2026-09-12）

用户要求GLM工作期间Codex也继续推进。由Codex核project-io原37个未覆盖臂，新增独立
project-io-admission.test.ts；不碰GLM的workspace-open-identity.test.ts或本人回执。
先补图章载入、地图索引/孤立工作副本、catalog读取失败、旧diff提示、部分写集及原页恢复清理警告；
...
清理旧分片不能只看零调用：ScriptDrawer仍有旧命令调用，但位于正常启动已提供canonical会话的回退路径；
...
完整check6,796、官方ratchet和单次严格fast6,308均exit0，editor201文件/2,069项；生产分母/源码不变。
wp新增16臂命中，行402/423、分支395/435；重点两模块未覆盖82→66，E3/E4保留分母，GLM未接收内容未计入。
本批不重复视觉流程，r2设计签字保持、不标done。
无下一位Agent提示词；Codex自持本批质量与集成，GLM沿用上方返工提示词。
...
完整check6,806/官方ratchet/单次严格fast6,318均exit0，editor202文件/2,079项；生产源码与分母不变。
重点两模块新增wp5+io1臂，剩余66→60；全仓分支+8无回退，E3/E4保留分母。
...
- 2026-09-11 Codex：fa8af7c2经独立复核后做接收修订；GLM贡献保留，本席补W6/S4/B3与删除回归，
  不把原候选或43项整包称为全通过。后续GLM继续IDB/权限/PAL及当前证据表，Codex保留性能/产品修复/最终质量门。
...
文档回填提交后核git ls-tree HEAD -- data/extracted为空，对70e31345完整diff严格仅白名单。
2. C3：IDB替身request error默认abort，不得再complete或提交；支持定点put注错，让getAll/get先正常通过。
用真实handle-store走到put，分别证明新记录失败不残留、已有记录失败不覆盖，保留读取失败和显式abort用例。
不需做完整IDB模拟器，仅修被测调用使用的边界；不要改产品补偿坏替身。
3. C4a/b：去5ms sleep，用entered/deferred证明第一位持有时第二位未进入，第一位失败释放后第二位才进入；
删除各自await previous的单点负控必须业务红。C4c第一半有效；第二半可收窄为端到端冲突并标冗余防御，
或补真实最早收口见证，不能用后续stateForReplay代为拒绝声称本点已钉死。
4. 当前43行主分类实计22/3/14/1/3，贡献人数另列。程序生成唯一ID/分类/当前数字，撤回26/4/15/1/3摘要。
W3未登记pending仍如实待证，先核真实pending资源合同，不把合法附属二进制造为应拒绝。
建议固化Codex已通过的W9全快照+creates/closes/removes见证，替代只核一个字节/creates的弱断言。

隔离反证参考 /tmp/codex-glm-c1c4-review.WRWkBW/review.config.mts，可重建。只改白名单测试/helper及
本人回执，不改生产、旧21项断言、全局配置/超时、baseline、原探针或真实工程。每个新边界独立跑正控/反证，
再跑定向/相邻/typecheck/完整check和同口径覆盖；提交推送后从最终远端树核范围和回执，一次性交Codex。
不代签、不标done、不转Kimi，不要求用户复制审查正文。GLM仍为测试贡献者，终审披露不变。
```

### 给GLM（历史：574012e5续批返工，C1–C4）
...
C2：W9把坏字节和匹配摘要放进待保存输入，保持同项目id/合法原基线，不再改坏磁盘后传另一工程的好files；
直接验证decoder错误与零副作用，并做新路径负控。W3补真实未登记pending/输出冲突，而非用私有域测试代替。
O4去timer轮询，改entered/deferred+finally释放；P5/B6名称、正控与实际恢复/新页证据一致，不改产品迎合标题。
C3：IDB每次open/request独立，事务暂存到complete才提交、abort撤销；S1在同一数据库开关故障后验证无残留，
不要重新建空Map当证明。保留有效S2时间点负控，并补abort后数据状态、独立请求及承诺的request/句柄错误。
C4：逐行输出43项当前表，补P7/B6，W10只算一个ID，贡献作者另列；不得用重复计数凑43。
按真实file:line纠正O8/P10/B8：非Error防御保留待证；Node fallback尾链及beforeClose+afterRead组合可自动化，
不因需构造边界就称单测不可达。撤回“B3修复design-system”的错误归因，K5/审计超时按本次裁定保留。

只修白名单测试/helper及本人回执，不改生产、旧21项断言、配置、baseline、原探针或用户工程。
参考本席 /tmp/codex-glm-final-review.3VdVLn/review.config.mts 可自行重建；本人跑正控与新路径反证，
再跑定向/相邻/typecheck/完整check和同口径覆盖，从实际候选生成数字与当前表。
全部修完提交推送并核远端SHA，一次性交Codex；不代签、不标done、不转Kimi、不让用户搬运意见。
```

### 给GLM（历史：已接收测试之外的剩余工作，一次性交付）
...
按顺序连续补余项，不逐组等待：
1. S1/S2/S3的真实IDB函数错误/事务/句柄，P1其余子项与B2基线身份。
2. O4/O5/O6、P5/P7、B6的PAL/沙盒真实打开恢复链，构造合法共享fixture。
3. W3/W9未覆盖类型、B5回调/snapshot，以及O8/P10/B8逐分支可达性分类。
4. 更新43项/SR当前状态表，逐项完整/部分/未做，给精确测试名/断言/原生待补。
不沿用08c1ee09的12/19/12当当前完成统计，不仅追加“已修正”而让原表继续冒称全覆盖。
补你所称本轮三次失败/最终check的候选SHA、命令、退出码、日志；若未保存须如实写证据缺失，
不得仅凭隔离绿归为无关环境，也不要拿旧6681项日志证明6687项候选。

只追加白名单测试/本批helper及本人回执日志；已接收21项不削弱，生产基线aa87c305不动，
不改产品、旧测试、配置、baseline、探针/用户工程。W10合法删除由Codex补齐，不再重复。
用真实调用链、合法正控、单一故障、内部gate；故障之间先复原，零IO断言前不得清空轨迹。
定向/相邻/typecheck/完整check、同口径覆盖及对应负控按实际提交树生成；有真产品红如实保留，
整批完成后提交推送核远端SHA，一次性交Codex。不要代签、标done或让用户搬运意见。
```

### 给GLM（历史：batch-r1整批返工，08c1ee09）
