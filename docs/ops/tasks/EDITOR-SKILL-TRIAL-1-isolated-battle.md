...
Status: rework
Phase: phase2
Capability: D-04/D-05修复方案调整；用户提出共享战斗模拟器，范围与预设持久化待设计，不借此启动第三阶段X5
Coding Owner: Codex
Reviewer: Kimi / GLM
...
Revision: r2首批实施草案与交互草图，2026-09-19；UI/保存两项选择待确认，尚未冻结签审。r1前提冻结`e58834f6389a40ffe9f187e6a8051f552e964d79`及三席签字保留为历史，不授权新范围。
...
已将方向收敛为[首批实施草案](../../testing/battle-simulator-r2-design.md)，包括真实源码四向证据、
四目录字段/继承/覆写、推荐工程内文件、现有保存事务/撤销/重开接入、一次性内存启动协议、10组验收边界。
同任务交互草图已制作，供用户确认四目录及技能快速启动；草图不是实际 BattleSession，也未修改编辑器导航。

首批建议：四类基础配置完整保存/重开→独立真实战斗/结算→技能/敌队共享入口，并接回已有单敌试打入口。
敌方残血/初始毒/状态需新增正式初始化能力，留第二批，不在首版挂无效控件；其他业务页新按钮随后复用。
不把所有拓展升为Q1/Q2的额外门槛。实现后仍须Codex功能视觉验证，草图不抵扣。

本轮用户“能开始实现”授权推进准备，但不推定豁免既有r2三签和新UI产品门；**没有改产品或正式测试**。
已异步询问：①命名预设/方案随工程保存，还是仅本机；②先保存工程再试打，还是支持未保存完整工程快照。
推荐分别为随工程/先保存；未答不算批准。草图结构、首批范围与这两项确认后冻结r2，给Kimi/GLM并行设计提示词。

### build前（r2草案，独立于下方r1历史签字）

- Codex：pending（直接证据与实施方案已落附件；等待产品边界收敛后签冻结r2）。
- Kimi：pending。
- GLM：pending。
- build准入：关闭；不改Status为build，不借他卡签字或把草图当产品实现。

## 当前方向与待决边界（r2准备，不是build授权）
...
背包只存物品与数量、测试金钱归方案，不再在我方预设重复保存；从业务页进入临时方案，不强迫先建立四份记录。该层级建议仍待确认。
...
- UI应有足够配置空间；当前建议独立一级“战斗模拟器”及四个目录页、方案内执行试打，先出结构草图让用户确认，不把小弹窗逐层堆大；本轮不直接修改导航。
...
无下一位Agent提示词，等待用户确认本轮草图与保存选择；随后Codex冻结r2并同时给两席设计审查提示词。
首批实施草案已落附件，但产品边界未收敛，当前不得实现；不要求用户搬运审查正文。两席r1历史签字不改写。
以下为历史设计交接，不代表现在还缺签字。
