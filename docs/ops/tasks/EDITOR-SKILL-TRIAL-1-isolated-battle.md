...
此前已核r2三席设计准入并开始首批实现；2026-09-20发现我方人数前提不完整，当前暂停受影响实现并等待下方产品裁决；r1不实施。用户已指出选择开局没有必要，并提出我方/敌方/战场/道具与可保存我方配置的共享战斗模拟器；不再询问旧小弹窗选项。

## 2026-09-20：我方人数前提反例与待裁决
...
无下一位Agent提示词，等待用户人数裁决；GLM九批接收独立进行，不受本卡等待影响。
...
当前无下一位Agent提示词，等待用户人数裁决；下列r2设计提示词已执行完毕，仅留历史，不可用来越过上方counter。
裁决后只重核受影响前提/设计，首批完整实现与自测后才给实现终审提示词，当前不得标done。
