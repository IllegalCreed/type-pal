...
| 维度 | 当前真值 | 直接证据 |
|---|---|---|
| 原版 / primary source | N/A：这是第二阶段编辑器本地工程与开发流程，不存在原版游戏对应功能。 | `docs/phase2/editor/project-lifecycle-design.md:1-6` 明确本文件属于 Reforge 项目生命周期设计。 |
| 第一阶段 | N/A：第一阶段没有 FSA 工程、最近目录、评审样例工作区或 PAL 三方迁移发布链。 | `docs/phase2/READ-FIRST.md` 的阶段边界；`docs/phase2/editor/project-lifecycle-design.md:14-36` 把该能力定义为第二阶段 local-first 工程模型。 |
...
- 已知坑 / 审计文档:
  - `docs/phase2/editor/project-lifecycle-design.md:22-47,74-108` 仍把 PAL 描述成稳定种子，需要按“开发快照 ->
    显式晋升 -> 稳定种子”修订。
...
- 修改文件:
  ...
  - 文档：本卡与 `docs/phase2/editor/project-lifecycle-design.md` §20。
- 实现摘要:
...
请审查任务卡 `docs/ops/tasks/ED-PAL-WORKSPACE-MODES-1-pal-development-sandbox-seed-lifecycle.md` 当前
`review` 实现。先读 `AGENTS.md`、`docs/phase2/READ-FIRST.md`、任务卡上下文锚点与
`docs/phase2/editor/project-lifecycle-design.md` §20；重点核查 `workspace-context.ts`、
...
`docs/phase2/editor/project-lifecycle-design.md` §20；重点核查 workspace × 九操作矩阵、marker / ZIP /
