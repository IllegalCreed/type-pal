...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | build | R13-6C+6D 实现+自验完成（dry-run open=0/0），待三方实现审查；剩余 runtime/save/browser/重迁双跑 + 正式发布 | `--r13-6c --r13-6d` dry-run 通过（源账 open=0/0、运行时 0 issues）；fast 78/570 绿 |
