...
|---|---|---|---|---|
| EDITOR-SAVE-RECOVERY-1 | 编辑器保存中断恢复 | build | Codex：余下分支、UI/OS权限与性能收口 | [任务卡](tasks/EDITOR-SAVE-RECOVERY-1-interrupted-author-save.md) PAL原生跨上下文API恢复补证通过，新增13项回归；待核未覆盖臂119→110，不标整卡完成。 |
...
[编辑器覆盖率确定性修复](audits/pre-e2e/coverage-determinism.md)亦已三签收口；当前fast为6,249项，覆盖率未下调，不代表作者保存恢复整卡已验收；废弃分支退役的分母变化已单列记录。
