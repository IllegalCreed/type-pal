...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | build | R13-6C 证据层已落地（dry-run observations 4→1），续做 6C seal/rewind 发布，再 R13-6D | 6C/6D 三方设计 agree 齐；`--r13-6c` dry-run `open sites=0 / observations=1`（剩段转移备注） |
