...
| D14-2 | 演出意图协议 + CutsceneController（议题 5/12/14 剩余②） | draft | 设计冻结完成（Codex agree），待 Kimi / GLM 设计压测签字 | PresentationIntent 词汇表（五能力+wait）+ CutsceneController（统一取消 + busy() 单一占用句柄）；cameraPanFx 收口 |
