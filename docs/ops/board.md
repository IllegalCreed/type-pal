...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | build | Codex 继续关闭 R13-Z；source sites 已归零，剩 8 条观察级条目待用户裁决后跑最终 runtime/save/browser/remigration 门禁 | B11-1 落地 + actor-casualty 证据族后 dry-run `open sites=0 / observations=8`（3×敌方0x68 + 4×setPalette + 1×段转移备注）；N3-1/C8/ED-5I 仍未完成 |
