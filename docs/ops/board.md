...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | build | Codex 继续关闭 R13-Z；source sites 已归零，剩 8 条观察级条目待裁决（3×lossy 需 R13-6C 取代 6A seal，4×setPalette + 1×段转移备注需用户拍板） | B11-1 + actor-casualty 证据族后 dry-run `open sites=0 / observations=8`；敌方 0x68 已按结构化毒模型入 6B 内容；N3-1/C8/ED-5I 仍未完成 |
