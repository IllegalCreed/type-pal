...
> **⚠ 2026-08-07 额度状态：GLM 已恢复（可补审补签）；Kimi 仍缺席**（额度耗尽）。
> GLM 恢复后优先补审：D14-2 覆盖矩阵实现期复审（+补签）、D14-1 设计复核（+build 复核）、
> W9/B10-1 设计压测签字、D14-3 设计压测 + 覆盖矩阵。Kimi 席位项仍待 Kimi 恢复后补审。
...
| D14-1 | 对话系统外观继承（版式/头像/光标/字体/自动播放） | build | 单 Agent 实现完成自证通过（reforge 821 + audit 0 意外折行），双审缺席待补审 | maxRight=320 全宽语义落地；11102 行仅 6 条超限行折行；待 Kimi 抽验/分段 e2e |
| D14-3 | 奖励/事件总线统一收尾（议题 14 剩余③） | draft | 设计冻结完成（Codex agree），双审缺席待补审 | RewardEvent 内部通道 + 统一 reward-gain（偷窃/炼成替换）；giveItem 自动呈现 v1.1 留口（涉 schema） |
