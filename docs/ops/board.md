...
| D14-2 | 演出意图协议 + CutsceneController（议题 5/12/14 剩余②） | build | Kimi 单审 agree（GLM 缺席代班）；实现完成自证通过（reforge 816），待 Kimi 视觉验收 + review | PresentationIntent + CutsceneController（busy/cancelAll/资源分域）；host 六方法经 controller 委托；GLM 恢复后补审 |
