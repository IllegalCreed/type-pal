...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | rework | User：验收 `ae6d1d9c` P7-R8；通过后交 GLM 合并终审 | 通用编辑器、真实地图预览、方案/引用/创建与步骤详情层级已连续返工；尚未重签 accept |

## 阻塞

| ID | 任务 | 状态 | 负责人/下一步 | 一句话备注 |
|---|---|---|---|---|
| C8 | 物品用途机制、运行时与迁移闭环 | blocked | N3-1 最终 accept 后补设计签字，Codex 迁移剩余 20 件，GLM 代审 | 已确认仍有 20 件真实待迁移（14 剧情、2 放置、4 通用机制）；目标改为 100 件可运行、0 条物品用途待迁移诊断 |
