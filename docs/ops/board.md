...
| ED-ITEM-ALCHEMY-SURFACE-1 | 炼蛊皿与紫金葫芦双炼化工作台 | draft | 待 Kimi/GLM premise/design 签字；不得实现 | 物品模块两独立页；item268 配方 / item270 灵葫档位，现 ItemData 唯一 owner |
...
| MIG-PAL-STORE0-SHOP-BOUNDARY-1 | PAL Store[0] 奖励表与商店边界收口 | draft | 待 Kimi/GLM 独立 premise/design 签字；不得实现 | migration-only：Shop只留1..20、items不变；双炼化页面见 ED-ITEM-ALCHEMY-SURFACE-1 |
