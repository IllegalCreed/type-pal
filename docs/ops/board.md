...
第一阶段允许行为不漂移的结构优化，也检查实现bug；纯重构与行为修正分提交。首批A1先走独立准入，不扩张为整仓同时重写。
...
|---|---|---|---|---|
| ARCH-REFORGE-MENU-1 | [运行时菜单/物品控制器](tasks/ARCH-REFORGE-MENU-1-session-controller.md) | review | Codex核统一门禁 | main少355行/15状态移出；check8439、等价/负控/最小视觉通过；coverage进行中 |
