...
| ED-ITEM-ALCHEMY-SURFACE-1 | 炼蛊皿与紫金葫芦双炼化工作台 | build | Codex 唯一 Coding Owner；Store0 migration 后实施 | 三签齐：双独立 route、唯一 ItemData owner、registry 两 adoption 迁绑、`===` fail-loud |
...
| MIG-PAL-STORE0-SHOP-BOUNDARY-1 | PAL Store[0] 奖励表与商店边界收口 | build | Codex 唯一 Coding Owner；migration-only 先行 | 三签齐：过滤不重编号、buy-only invariant、writes=1、item268/270 永久区分 |
