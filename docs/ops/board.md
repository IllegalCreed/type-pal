...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | rework | User：验收 `5b6bb58e` P7-R9；通过后交 GLM 合并终审 | 已修复切场景后实体脚本、地图焦点与黄色范围不同步；通用编辑器和真实预览仍待用户整体验收 |
| C8 | 物品用途机制、运行时与迁移闭环 | review | Kimi + GLM 只读终审 `88277465`；签齐后待 N3-1/用户联合验收 | 已达 100 runnable / 0 item-use diagnostics；20 件/21 根、统一私有脚本编辑闭环与 MG2 零计划已自验 |
