...
| ED-ITEM-ALCHEMY-SURFACE-1 | 炼蛊皿与紫金葫芦双炼化工作台 | blocked | 等 MIG-PAL-CRAFT-FAILURE-MESSAGE-1 三签与重迁后复验 | UI 已实现；PAL item268 failure 文案被 producer 漏迁，旧 Codex accept 转 counter |
...
| MIG-PAL-CRAFT-FAILURE-MESSAGE-1 | PAL 炼蛊失败原文迁移闭环 | draft | 待 Kimi/GLM premise/design 签字；不得实现 | L39595 原文存在，producer 丢终端失败臂；目标只新增 item268 unavailableMessage |
