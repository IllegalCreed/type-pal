...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | build | Codex 继续关闭 R13-Z source debt；归零后跑最终 runtime/save/browser/remigration 门禁 | 新增 item-unusable-use 证据族 + successor 扩到 scriptDesc/314/352/372/373 后 dry-run 降至 `110 sites / 118 observations`（无写盘）；剩余全为 actor 机制缺口 + 8 条观察级决策条目；N3-1/C8/ED-5I 仍未完成 |
