...
| ED-ITEM-ALCHEMY-SURFACE-1 | 炼蛊皿与紫金葫芦双炼化工作台 | blocked | 等 MIG-PAL-CRAFT-FAILURE-MESSAGE-1 重迁完成后复验 | UI 已实现；PAL item268 failure 文案被 producer 漏迁，旧 Codex accept 转 counter |
...
| MIG-PAL-CRAFT-FAILURE-MESSAGE-1 | PAL 炼蛊失败原文迁移闭环 | build | Codex 唯一 Coding Owner；strict translator → invariant → 重迁 | 三签齐且无 counter；五连 0x20 链尾→L_39595；exact diff 三文件 + writes=1/双零 |
