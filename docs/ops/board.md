...
|---|---|---|---|---|
| N3-1 | 结构化控制流、实体具名行为与内部脚本退役 | build | Codex 开始 R13-Z 发布闭包；完成后交三方终审与用户验收 | R13-6B 三方 implementation accept 已齐（`fd2e4353` / `88221ef7`）；R13-Z 已增加门控 item scriptOnThrow 源证据（实测关闭 160 sites，尚未发布）；下一门禁是 append-only authority、无 open-debt、runtime matrix、浏览器/存档/全量重迁回归；N3-1/C8/ED-5I 仍未完成 |
