...
function makeGs(role0: {
  level: number, hp: number, maxHP: number, mp?: number, maxMP?: number, exp?: number,
  attackStrength?: number, magicStrength?: number, defense?: number, dexterity?: number, fleeRate?: number,
}): GameState {
  const gs = createInitialGameState({ x: 0, y: 0, facing: 'down' })
  const rt = gs.PlayerRolesRuntime
  rt.rgwLevel[0] = role0.level
  rt.rgwHP[0] = role0.hp; rt.rgwMaxHP[0] = role0.maxHP
  rt.rgwMP[0] = role0.mp ?? 0; rt.rgwMaxMP[0] = role0.maxMP ?? 30
...
describe('battleWonLevelUp —— D11 战斗胜利升级', () => {
  it('exp 进阈值 → 升 1 级 + stat 成长 + HP/MP 满 + exp 余数(user mock 场景)', () => {
    ...
    const gs = makeGs({ level: 1, hp: 50, maxHP: 100, mp: 10, maxMP: 30, exp: 0, attackStrength: 20, defense: 10 })
  ...
  it('exp 不够 → 不升级,exp 累积(余数 = 旧+得),不动属性/HP', () => {
    ...
    const results = battleWonLevelUp({
      gs, partyMembers: [0], expGained: 40,
      levelUpExp: expTable({ 1: 100 }), levelUpMagic: [], rng: rng0(),
    })
  ...
  it('exp 远超阈值 → 连升多级(余数正确)', () => {
    ...
    battleWonLevelUp({
      gs, partyMembers: [0], expGained: 250,
      levelUpExp: expTable({ 1: 100, 2: 100, 3: 100 }), levelUpMagic: [], rng: rng0(),
    })
  ...
  it('死队员(runtime hp=0)不获 exp / 不升级', () => {
    ...
    const results = battleWonLevelUp({
      gs, partyMembers: [0], expGained: 999,
      levelUpExp: expTable({ 1: 100 }), levelUpMagic: [], rng: rng0(),
    })
  ...
  it('升级学新法术:level-up-magic[j][role] level<=新等级 + magic!=0 + 未学 → AddMagic', () => {
    ...
    const results = battleWonLevelUp({
      gs, partyMembers: [0], expGained: 100, // 升到 2 级
      levelUpExp: expTable({ 1: 100 }), levelUpMagic: lum, rng: rng0(),
    })
  ...
  it('满级(MAX_LEVELS=99):exp 仍扣但不再升级 / 不长属性', () => {
    ...
    battleWonLevelUp({
      gs, partyMembers: [0], expGained: 250,
      levelUpExp: expTable({ 99: 100 }), levelUpMagic: [], rng: rng0(),
    })
