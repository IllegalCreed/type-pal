...
describe('dialog-history', () => {
  it('push 追加;空/纯空白跳过', () => {
    ...
    pushDialogHistory(h, 1, '')
    expect(h).toEqual([{ scene: 1, text: '你好' }])
  })
  ...
  it('环形缓冲:超过 CAP 丢最旧(长度=CAP、首条是第 5 条)', () => {
    ...
    expect(h[0]).toEqual({ scene: 1, text: 'line5' }) // 最旧 5 条被丢
    expect(h[h.length - 1]).toEqual({ scene: 1, text: `line${DIALOG_HISTORY_CAP + 4}` })
  })

  it('连续同 scene 同 text 去重(同一行被多 tick re-commit 不重复入)', () => {
    const h: DialogHistoryEntry[] = []
    pushDialogHistory(h, 1, '重复')
    pushDialogHistory(h, 1, '重复')
    expect(h).toEqual([{ scene: 1, text: '重复' }])
  })

  it('不同 scene 的相同 text 不去重(scene 维度)', () => {
    const h: DialogHistoryEntry[] = []
