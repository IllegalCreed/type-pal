/**
 * ARCH-REGRESSION-LAB-GLM-1 · G07 第一阶段模块边界（正式跨模块回归）。
...
import { getCurrentMapNum, setCurrentMapNum } from './scene-system.js'
...
describe('G07 第一阶段模块边界', () => {
  test('G07-01 scene 写 mapNum → event-system 对话历史按当前图号入账（跨模块写读顺序）', () => {
    const previousMapNum = getCurrentMapNum()
    try {
      setCurrentMapNum(7)
      ...
      setCurrentMapNum(previousMapNum)
    }
  })
  ...
  test('G07-04 装备脚本经 event 表执行：scriptOnEquip → 全局命令表 → 效果层 → 派生 getter', () => {
    ...
    const previousEvents = [...getGlobalCommands()]
    try {
      setGlobalEvents([
      ...
      setGlobalEvents(previousEvents)
    }
  })
})
