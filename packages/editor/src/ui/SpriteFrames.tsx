...
/** 把 baked canvas 画进一个 <canvas>(整数放大居中,pixelated)。 */
function FrameCell(props: { canvas: HTMLCanvasElement | undefined; scale: number }) {
  const ref = useRef<HTMLCanvasElement>(null)
  const { canvas, scale } = props
  useEffect(() => {
    const c = ref.current
    ...
    ctx.drawImage(canvas, (c.width - w) / 2, c.height - h - 4, w, h) // 底对齐
  }, [canvas, scale])
  return <canvas ref={ref} width={60} height={80} className="fcell-canvas" />
}
        ...
        {layout.kind === 'directional' ? (
                  ...
                  {Array.from({ length: fpd }, (_, fi) => {
                    ...
                    return (
                      <div key={idx} className={`fcell${fi === 0 ? ' stand' : ''}`} style={{ borderColor: `color-mix(in srgb, ${DIR_COLOR[dir]} 45%, var(--line))` }}>
                        ...
                        <FrameCell canvas={baked[idx]} scale={2} />
          ...
          <div className="cells" style={{ flexWrap: 'wrap' }}>
            {loaded.frames.map((_, idx) => (
              <div key={idx} className="fcell"><span className="fidx">{idx}</span><FrameCell canvas={baked[idx]} scale={2} /></div>
            ))}
          </div>
        )}
          ...
          <div className="poselist">
            {Object.entries(sprite.poses ?? {}).map(([name, pose]) => (
              <div key={name} className="posecard">
                <b>{name}</b>
                <div className="pf">{pose.frames.map((fi) => <FrameCell key={fi} canvas={baked[fi]} scale={1.3} />)}</div>
