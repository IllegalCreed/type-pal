...
function definitelyPresentLeading(
  ...
  if (!initializer) return fail()
  if (ts.isStringLiteral(initializer)) return initializer.text.trim() ? true : fail()
  if (!ts.isJsxExpression(initializer) || !initializer.expression) return fail()
...
function catalogCallsites(file: string, content: string) {
  ...
  function isCatalogRowAliasExpression(node: ts.Expression): boolean {
    if (ts.isIdentifier(node)) return node.text === 'DsCatalogRow'
    if (ts.isPropertyAccessExpression(node)) return node.name.text === 'DsCatalogRow'
    if (
    ...
    if (ts.isConditionalExpression(node)) {
      return (
        isCatalogRowAliasExpression(node.whenTrue) || isCatalogRowAliasExpression(node.whenFalse)
      )
    }
    return false
  }
  ...
  function visit(node: ts.Node) {
    ...
    if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      if (tag === 'DsCatalogRow') openings.push(node)
      else if (tag.endsWith('.DsCatalogRow')) namespaceTags.push(tag)
    }
  ...
  return openings.map((opening) => {
    ...
    if (spreads.length)
      throw new Error(
        `${file}: DsCatalogRow spread attributes are forbidden by the adoption registry`,
      )
    ...
    const slots = Object.fromEntries(
      slotNames.map((name) => {
        const attribute = jsxAttributes.find(
...
function heroMediaCallsites(root = uiRoot) {
  return productionTsxFiles(root).flatMap((file) => {
      ...
      if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
        ...
        if (tag === 'DsObjectHero') {
          if (node.attributes.properties.some(ts.isJsxSpreadAttribute))
            throw new Error(
              `${source}: DsObjectHero spread attributes can hide an unregistered media owner`,
            )
    ...
    if (aliases.length)
      throw new Error(
        `${source}: DsObjectHero aliases evade the Hero media census: ${aliases.join(', ')}`,
      )
    if (namespaceTags.length)
      throw new Error(
        `${source}: namespace DsObjectHero tags evade the Hero media census: ${namespaceTags.join(', ')}`,
      )
    return callsites
  })
}
...
describe('catalog row content adoption gate', () => {
  test('closes over every production DsCatalogRow callsite with a bound content decision', () => {
    ...
    const recorded = matrix.entries.map((entry) => `${entry.source}@${entry.jsxFingerprint}`)
    expect(matrix.version).toBe(4)
    expect([...recorded].sort()).toEqual(actual.map((entry) => entry.identity).sort())
    expect(
      new Set(recorded).size,
      'matrix contains duplicate source + fingerprint identities',
    ).toBe(recorded.length)
    expect(
      actualByIdentity.size,
      'production contains ambiguous duplicate source + fingerprint identities',
    ).toBe(actual.length)
    expect(
      new Set(matrix.entries.map((entry) => entry.id)).size,
      'matrix contains duplicate stable surface ids',
    ...
    for (const entry of matrix.entries) {
      ...
      expect(callsite.slots.leading, `${entry.id} leading presence`).toBe(
        entry.leading === 'present',
      )
      ...
      if (entry.idPresentation === 'reference-exact') {
        ...
        expect(entry.identitySlot, `${entry.id} diagnostic is not a stable object identity`).toBe(
          'none',
        )
      }
    ...
    const idsFor = (field: string, value: string): string[] =>
      matrix.entries
        .filter((entry) => entry[field] === value)
        .map((entry) => entry.id)
        .sort()
    expect(idsFor('titleKind', 'derived-content')).toEqual(['enemy-team/catalog', 'shop/catalog'])
    ...
    expect(idsFor('identitySlot', 'title')).toEqual([])
  ...
  test('closes every production object Hero media owner without raw glyph fallbacks', () => {
    ...
    const recorded = matrix.heroMedia.map((entry) => `${entry.source}@${entry.jsxFingerprint}`)
    ...
    for (const entry of matrix.heroMedia) {
      ...
      expect(
        actualEntry?.hasRawText,
        `${entry.id} must not render raw text or emoji as media`,
      ).toBe(false)
  ...
  test('keeps every catalog family on one leading-slot strategy', () => {
    ...
    for (const [family, values] of strategies) expect([...values], family).toHaveLength(1)
  ...
  test('detects nested production calls, semantic drift, aliases, and scoped reorder ownership', () => {
    ...
    try {
      ...
      const before = catalogCallsites(
        'Consumer.tsx',
        'const view = <DsCatalogRow title={name} meta={id} />',
      )[0]
      const after = catalogCallsites(
        'Consumer.tsx',
        'const view = <DsCatalogRow title={name} meta={otherId} />',
      )[0]
      ...
      expect(() =>
        catalogCallsites('Spread.tsx', 'const view = <DsCatalogRow title="x" {...props} />'),
      ...
      expect(() =>
        catalogCallsites('FalsyLeading.tsx', 'const view = <DsCatalogRow leading={0} title="x" />'),
      ).toThrow(/statically non-empty/)
      expect(() =>
        catalogCallsites(
          'Pointer.tsx',
