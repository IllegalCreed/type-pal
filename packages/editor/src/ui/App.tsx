    ...
    <div className="editor">
      ...
      <div className="body">
        ...
        {mode === 'actor' ? (
          ...
          <DataMode sprites={state.sprites} skills={Object.fromEntries(state.skills.map((sk) => [sk.id, sk]))} items={Object.fromEntries(state.items.map((i) => [i.id, i]))} locale={state.locale} assetBase={project.assetBase} session={session} enemies={state.enemies ?? []} enemyTeams={state.enemyTeams ?? []} />
        ) : mode === 'event' ? (
          <EventMode
            scenes={state.scenes}
            locale={state.locale}
