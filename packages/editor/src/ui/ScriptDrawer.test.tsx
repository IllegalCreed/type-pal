...
describe('ScriptDrawer command aggregate draft', () => {
  ...
  test('人物对话称谓与指令草稿一次完成、一次撤销和重做保持原子', async () => {
    ...
    const speakerRow = [...host.querySelectorAll<HTMLElement>('.drawer-form .cf-row')].find(
      (row) => row.textContent?.includes('显示称谓'),
    )!
    ...
    const command = committed.scenes[0]!.entities[0]!.pages?.[0]?.trigger?.stages[0]?.body[0] as unknown as {
      cue: {
    ...
    const undoneCommand = undone.scenes[0]!.entities[0]!.pages?.[0]?.trigger?.stages[0]?.body[0] as unknown as {
    ...
    const redoneCommand = redone.scenes[0]!.entities[0]!.pages?.[0]?.trigger
      ?.stages[0]?.body[0] as unknown as { cue: { identity: { speakerOverride?: string }; rows: Array<{ speed?: number }> } }
