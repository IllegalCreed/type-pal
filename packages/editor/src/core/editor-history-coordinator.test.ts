...
const scriptState = (): ScriptEditorStateV5 => ({
  scenes: [],
...
describe('EditorHistoryCoordinator', () => {
  test('私有正文与 shell ref 一次撤销/重做，顺序保持成对', () => {
    ...
    const scriptV5 = new ScriptV5EditSession(scriptState())
    const coordinator = new EditorHistoryCoordinator(legacy, scriptV5)
    coordinator.dispatch(new AddItemPrivateScriptV5Command('private', '私有正文'), shellCommand())
    expect(legacy.getState().items[0]!.use!.effects).toHaveLength(1)
    expect(scriptV5.getState().items[0]!.use!.effects).toHaveLength(1)

    expect(coordinator.undo()).toBe(true)
    expect(legacy.getState().items[0]!.use!.effects).toEqual([])
    expect(scriptV5.getState().items[0]!.use!.effects).toEqual([])
  ...
  test('第二笔 dispatch 抛错时沉默恢复第一笔，不能 redo 复活半状态', () => {
    ...
    expect(() =>
      coordinator.dispatch(
        new AddItemPrivateScriptV5Command('private', '私有正文'),
    ...
    expect(scriptV5.canUndo()).toBe(false)
    expect(legacy.redo()).toBe(false)
    expect(scriptV5.redo()).toBe(false)
    expect(legacy.isDirty()).toBe(false)
    expect(scriptV5.isDirty()).toBe(false)
  })
