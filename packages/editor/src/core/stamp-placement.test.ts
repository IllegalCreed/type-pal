...
describe('W7G stamp placement planning', () => {
  ...
  test('placement 反向索引与直接扫描一致且 id 确定递增', () => {
    ...
    for (const [key, owner] of direct.visual) expect(indexed.visualOwnerByKey.get(key)).toBe(owner)
    for (const [key, owner] of direct.collision)
      expect(indexed.collisionOwnerByKey.get(key)).toBe(owner)
