...
const txLog = { completed: 0, aborted: 0, stagedAtAbort: 0, putCalls: 0, putSuccesses: 0 }
  ...
  const db = {
    ...
    transaction(store: string, mode: IDBTransactionMode) {
      ...
      const tx = {
        ...
        objectStore() {
          const request = (read: () => unknown, isPut = false) => {
            const req = {
          ...
          return {
            ...
            put: (value: import('./handle-store.js').WorkspaceHandleRecord) => {
              ...
              return request(() => saved.workspaceId, true)
            },
          }
        },
      }
...
beforeEach(() => {
  ...
  Object.assign(txLog, { completed: 0, aborted: 0, stagedAtAbort: 0, putCalls: 0, putSuccesses: 0 })
...
test('F6: 宿主在非空写集暂存后 abort → 拒绝、写集丢弃不发布、事务单次终结', async () => {
  ...
  expect(txLog.putCalls).toBe(1)
  expect(txLog.putSuccesses).toBe(1)
...
test('F6: 已登记工作区更新在 put 成功后 abort,保留完整旧记录;同库重试正常提交', async () => {
  const disk = memoryAuthorDirectory()
  const ctx = context('abort-update', UUID_A)
  await saveWorkspaceHandle(ctx, 'original', disk.dir)
  const before = new Map(records)
  const putsBefore = txLog.putCalls
  const successesBefore = txLog.putSuccesses
  const completedBefore = txLog.completed
  idbOptions.abortNextWrite = true
  await expect(saveWorkspaceHandle(ctx, 'changed', disk.dir)).rejects.toThrow(
    'IndexedDB 事务已中止',
  )
  expect(txLog.putCalls - putsBefore).toBe(1)
  expect(txLog.putSuccesses - successesBefore).toBe(1)
  expect(txLog.stagedAtAbort).toBe(1)
  expect(txLog.aborted).toBe(1)
  expect(txLog.completed).toBe(completedBefore)
  expect(records).toEqual(before)
  delete idbOptions.abortNextWrite
  await saveWorkspaceHandle(ctx, 'changed', disk.dir)
  expect([...records.keys()]).toEqual([UUID_A])
  expect(records.get(UUID_A)).toMatchObject({ name: 'changed', handle: disk.dir })
  expect(txLog.completed).toBe(completedBefore + 1)
})
...
function queuedLocksHost(requests: Array<{ name: string; mode: string }>) {
  ...
  return {
    request: async (
      name: string,
      options: { mode: string },
      callback: (lock: { name: string; mode: string }) => Promise<unknown>,
    ) => {
      expect(options.mode).toBe('exclusive') // 本模型仅实现生产调用使用的独占请求。
      ...
      try {
        return await callback({ name, mode: options.mode }) // 传递宿主锁,等待回调完成
      } finally {
...
test('F6: 同名注册锁排队:持锁回调悬挂期间等待者不进入且悬挂期品牌有效;释放后按序执行', async () => {
  ...
  try {
    ...
    expect(() => assertWorkspaceRegistrationLock(saved, UUID_A)).not.toThrow()
  } finally {
    releaseHolder()
  ...
  expect(requests).toEqual([
    { name: `type-pal-workspace:${UUID_A}`, mode: 'exclusive' },
    { name: `type-pal-workspace:${UUID_A}`, mode: 'exclusive' },
  ])
})
...
test('F5: 真实品牌跨 caller await 保持有效;成功退出后失效', async () => {
  ...
  expect(await settled).toEqual({ ok: true, value: 'done' })
  expect(() => assertWorkspaceRegistrationLock(saved, UUID_A)).toThrow(
    '拒绝未经 workspace identity lock 授权的操作',
  )
})

test('F5: 真实品牌在异常 caller 的 await 期间有效,异常退出后失效且错误原样传播', async () => {
  ...
  if (result.ok) throw new Error('异常 caller 被错误地当作成功')
