/**
 * Command 接口 + 示例命令(D-B0 command/undo 地基)。
 *
 * 所有编辑 = Command:apply 产新态(不可变)、invert 把「apply 后的态」还原回 apply 前。
 * EditSession 用 apply/invert 驱动 undo/redo。B1 各模式发各自命令,本文件先给一个
 * MoveEntityCommand 证明引擎可用。
 *
...
function withEntityPos(
  ...
  return {
    ...
    scenes: state.scenes.map((scene) => {
      ...
      return {
        ...scene,
        entities: scene.entities.map((e) => (e.id === entityId ? { ...e, pos: newPos } : e)),
      }
    }),
  }
}
