...
export type LifecycleCommandBodyLocation =
  | { root: 'scenes'; path: readonly (string | number)[] }
  | { root: 'sharedScripts'; path: readonly (string | number)[] }

export interface LifecycleCommandBody {
  label: string
  location: LifecycleCommandBodyLocation
...
export function collectEntityLifecycleCommandBodies(
  state: EditorState,
  sceneId: string,
  entityId: string,
  ...
  const entity = scene.entities[entityIndex] as unknown as AuthorSceneDef['entities'][number]
  const result: LifecycleCommandBody[] = []
...
function assertLifecycleCommand(value: unknown): asserts value is EntityLifecycleCommand {
  checkAuthorCommands([value], 'lifecycle command')
  if (!isLifecycleCommand(value)) throw new Error('当前 lifecycle editor 只接受四种 lifecycle leaf')
}

function cloneRoot(state: EditorState, location: LifecycleCommandBodyLocation): unknown {
...
function withValidatedRoot(
  ...
  validateAuthorScenes(next.scenes)
  if (next.sharedScripts) checkAuthorScriptLibrary(next.sharedScripts)
  const missing = collectMissingEntityAddressReferences(next)[0]
  if (missing)
    throw new Error(
      '当前 lifecycle editor: ' +
        missing.path +
...
abstract class LifecycleEditCommand implements Command {
  abstract readonly label: string
  abstract apply(state: EditorState): EditorState
  abstract invert(state: EditorState): EditorState

  constructor(
    protected readonly location: LifecycleCommandBodyLocation,
...
export class InsertEntityLifecycleCommand extends LifecycleEditCommand {
  readonly label = '插入实体生命周期命令'
  private readonly command: EntityLifecycleCommand
...
export class UpdateEntityLifecycleCommand extends LifecycleEditCommand {
  readonly label = '修改实体生命周期命令'
  private readonly command: EntityLifecycleCommand
  private previous: EntityLifecycleCommand | undefined
...
export class DeleteEntityLifecycleCommand extends LifecycleEditCommand {
  readonly label = '删除实体生命周期命令'
  private removed: EntityLifecycleCommand | undefined
