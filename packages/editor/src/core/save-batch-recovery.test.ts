...
beforeEach(() => {
  ...
  // 只在每个测试开始时保存一次；同库切换故障不能把当前替身误存为原环境。
...
function installControllableIndexedDb(faults: Faults = {}) {
  ...
  const database = {
    ...
    transaction: () => {
      ...
      let settled = false
      // 当前 handle-store 每个事务只有一个请求，且不 preventDefault；读/写请求共用终结合同。
      const settle = (request: IDBRequest, abortAfterRequestSuccess = false) => {
        queueMicrotask(() =>
          queueMicrotask(() => {
            if (settled) return
            settled = true
            if (request.error || abortAfterRequestSuccess) {
      ...
      const tx = {
        ...
        objectStore: () => ({
          ...
          put: (value: WorkspaceHandleRecord) => {
            ...
            settle(req, db.faults.abortAfterRequestSuccess)
            return req
          },
        }),
      }
...
test('S1: 同一数据库上 open 失败不误报成功、请求对象独立、失败后无残留', async () => {
  ...
  expect(originalIndexedDbDescriptor).toEqual(originalDescriptor)
})
...
test('S3: request error 传播；字段漂移/换绑登记被拒，原记录不被覆盖', async () => {
  ...
  // Codex接收补证：必须对同一个旧记录尝试更新失败，而非只检查另一个未触及记录。
  await expect(underLock(workspace, 'attempted-overwrite', flakyHandle)).rejects.toThrow(
    'request io failure',
  )
  expect(db.putCalls).toBe(putCallsBefore + 2)
  expect(db.records.get(workspace.workspaceId)?.name).toBe('original')
...
// Codex接收补证：验证环境替身自身的错误终结，不能以坏替身证明产品正确。
test.each([
  'put',
  'get',
  'getAll',
] as const)('IDB 边界合同：%s 请求失败只 abort 一次、不 complete/提交', async (kind) => {
  db.records.clear()
  installControllableIndexedDb({ requestErrorAll: true })
  const database = await new Promise<IDBDatabase>((resolve, reject) => {
    const request = indexedDB.open('type-pal-editor', 2)
  ...
  await new Promise<void>((resolve) => {
    const tx = database.transaction('project-handles', kind === 'put' ? 'readwrite' : 'readonly')
    tx.oncomplete = () => {
      terminal.push('complete')
      resolve()
    }
    tx.onabort = () => {
      terminal.push('abort')
    ...
    const store = tx.objectStore('project-handles')
    const request =
      kind === 'put'
        ? store.put({ workspaceId: 'boundary-oracle' })
        : kind === 'get'
          ? store.get('boundary-oracle')
          : store.getAll()
    request.onerror = () => {
      errorSeen = true
    }
  ...
  expect(terminal).toEqual(['abort'])
  expect(db.records.has('boundary-oracle')).toBe(false)
})
