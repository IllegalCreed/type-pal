...
describe('openLocalProject current canonical boundary', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    reforge.readJson.mockResolvedValue({ contentVersion: 19 })
  ...
  test('opens current canonical content through the canonical loader only', async () => {
  ...
  test('rejects a non-current contentVersion before loading project data', async () => {
    ...
    await expect(openLocalProject(directory(`future-v${version}`))).rejects.toThrow(
      `contentVersion ${version}；开发期编辑器只接受当前 contentVersion 19`,
    )
  ...
  test('wraps canonical loader failures without falling back to an older loader', async () => {
    ...
    await expect(openLocalProject(directory('bad-current'))).rejects.toThrow(
      'canonical v19 内容无效(bad current schema)',
    )
    expect(reforge.dispose).toHaveBeenCalledOnce()
  })
})
