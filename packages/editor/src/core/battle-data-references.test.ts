...
function fixture(): EditorState {
  return {
    actors: [
      {
        ...
        battler: {
          ...
          baseStats: { level: 1, maxHP: 10, maxMP: 10, attackStrength: 1, magicStrength: 1, defense: 1, dexterity: 1, fleeRate: 1 },
          initialEquipment: {},
    ...
    items: [],
    enemies: [
      { id: 'enemy-a', name: 'name.enemy-a', battleSprite: 'battle.enemy', yPosOffset: 0, stats: { health: 1, level: 1, exp: 0, cash: 0, attackStrength: 1, magicStrength: 1, defense: 1, dexterity: 1, fleeRate: 1, physicalResistance: 0, poisonResistance: 0, elemResistance: { wind: 0, thunder: 0, water: 0, fire: 0, earth: 0 }, dualMove: false, collectValue: 0 }, ai: { resistanceToSorcery: 0, rules: [{ at: 'act', do: { kind: 'cast', skillId: 'skill-a' } }, { at: 'act', do: { kind: 'transform', enemyId: 'enemy-b' } }] }, sounds: {} },
      { id: 'enemy-b', name: 'name.enemy-b', battleSprite: 'battle.enemy', yPosOffset: 0, stats: { health: 1, level: 1, exp: 0, cash: 0, attackStrength: 1, magicStrength: 1, defense: 1, dexterity: 1, fleeRate: 1, physicalResistance: 0, poisonResistance: 0, elemResistance: { wind: 0, thunder: 0, water: 0, fire: 0, earth: 0 }, dualMove: false, collectValue: 0 }, ai: { resistanceToSorcery: 0 }, sounds: {} },
    ],
...
describe('battle data deletion references', () => {
  ...
  test('blocks enemy team and cross-enemy actions but ignores owner-internal self references', () => {
    const state = fixture()
    state.enemies![1]!.ai.rules = [{ at: 'act', do: { kind: 'summon', enemyId: 'enemy-b', count: 1 } }]
  ...
  test('collects skill effects and poison relations as blocking poison references', () => {
