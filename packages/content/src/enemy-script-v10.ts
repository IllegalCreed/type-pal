...
export type EnemyOnDefeatedLeafV10 = Extract<
  AuthorCommandV5,
  {
    kind:
      | 'dialog'
...
export type EnemyOnDefeatedCommandV10 =
  ...
  switch (kind) {
    ...
    case 'any':
      ...
      condition.of.forEach((entry, index) => {
        checkEnemyAiConditionV10(entry, `${path}.of[${index}]`)
      })
      return
    case 'not':
      exactKeys(condition, ['kind', 'cond'], path)
      checkEnemyAiConditionV10(condition.cond, `${path}.cond`)
      return
    default:
...
export function checkBattleChoreographyBody(
  ...
  value.forEach((action, index) => {
    checkBattleChoreographyAction(action, `${path}[${index}]`)
  })
}

export function checkBattleChoreographyV10(value: unknown, path: string): void {
  ...
  value.forEach((rawHook, index) => {
    ...
    if (hook.when !== undefined) checkEnemyAiConditionV10(hook.when, `${hookPath}.when`)
    checkBattleChoreographyBody(hook.body, `${hookPath}.body`)
  })
}
...
function checkHookCommand(
  ...
  checkBattleChoreographyAction(command, path)
}
...
function checkTransition(
  ...
  switch (kind) {
    ...
    case 'branch':
      ...
      checkEnemyAiConditionV10(transition.cond, `${path}.cond`)
...
export function checkEnemyHookFlow(value: unknown, path: string): asserts value is EnemyHookFlow {
  ...
  for (const [stateId, rawState] of Object.entries(states)) {
    ...
    state.body.forEach((command, index) => {
      checkHookCommand(command, `${statePath}.body[${index}]`, effectIds)
...
export function checkEnemyAiV10(value: unknown, path: string): void {
  ...
  if (ai.rules !== undefined) {
    ...
    ai.rules.forEach((rawRule, index) => {
      ...
      if (rule.when !== undefined) checkEnemyAiConditionV10(rule.when, `${rulePath}.when`)
  ...
  if (ai.hooks !== undefined) {
    ...
    for (const channel of ['ready', 'turnStart'] as const)
      if (hooks[channel] !== undefined)
        checkEnemyHookFlow(hooks[channel], `${path}.hooks.${channel}`)
  }
}
...
const ON_DEFEATED_LEAF_KEYS = {
  ...
  stopScript: ['kind'],
} as const satisfies Record<EnemyOnDefeatedLeafV10['kind'], readonly string[]>

export function checkEnemyOnDefeatedCommandsV10(
  value: unknown,
  path: string,
): asserts value is EnemyOnDefeatedCommandV10[] {
  if (!Array.isArray(value)) throw new Error(`${path}: 期望 EnemyOnDefeatedCommandV10[]`)
  value.forEach((rawCommand, index) => {
    ...
    if (kind === 'branch') {
      ...
      checkAuthorConditionV5(command.cond, `${commandPath}.cond`)
      checkEnemyOnDefeatedCommandsV10(command.then, `${commandPath}.then`)
      if (command.else !== undefined)
        checkEnemyOnDefeatedCommandsV10(command.else, `${commandPath}.else`)
      return
    }
    const allowed = ON_DEFEATED_LEAF_KEYS[kind as EnemyOnDefeatedLeafV10['kind']]
    if (!allowed)
      throw new Error(`${commandPath}.kind: onDefeated context 不支持命令 ${JSON.stringify(kind)}`)
    exactKeys(command, allowed, commandPath)
    checkAuthorCommandsV5([command], commandPath)
    if (kind === 'dialog') checkCommands([command], commandPath)
