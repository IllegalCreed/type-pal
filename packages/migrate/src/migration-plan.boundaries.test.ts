...
describe('createMigrationPlan · 分类与 summary', () => {
  test('保留/新增/删除/未管理文件：精确 target/writes/deletes/summary', () => {
    ...
    const oursUnmanaged = snap(
      { 'content/a.json': { v: 1 }, 'content/untracked.json': { z: 1 } },
      ['content/a.json'],
    )
  ...
  test('同输入重复调用稳定且输入快照不被修改', () => {
    ...
    const theirs = snap({ 'content/a.json': { v: 2 } }, ['content/a.json'])
    const baseBefore = structuredClone([...base.files])
    ...
    expect(isDeepStrictEqual([...base.files], baseBefore)).toBe(true)
  })
})
...
describe('createMigrationPlan · 原子地图（字节/hash 合同）', () => {
  ...
  test('ours=base + theirs 改 → 生成 theirs 值的 write；hash 按序列化字节比较', () => {
    ...
    const theirs = snap({ [mapFile]: v4map({ layers: [{ id: 'L1', name: '底', tiles: [[1], [1]] }] }) }, [mapFile])
    const plan = createMigrationPlan(base, ours, theirs)
    expect(plan.conflicts).toEqual([])
    expect(plan.writes.get(mapFile)).toEqual(v4map({ layers: [{ id: 'L1', name: '底', tiles: [[1], [1]] }] }))
  ...
  test('真实字段变化产生 write；未知字段经 canonical 序列化归一 → 字节合同下同版本无 write', () => {
    ...
    const plan2 = createMigrationPlan(base, ours, snap({ [mapFile]: v4map({ meta: 1 }) }, [mapFile]))
    expect(plan2.writes.size).toBe(0)
    // 字节合同直证：同 hash ⟺ 同序列化字节
    expect(sha256(serializeMigrationJson(v4map(), mapFile))).toBe(
      sha256(serializeMigrationJson(v4map(), mapFile)),
    )
  })
})
