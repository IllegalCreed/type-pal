...
const characters = validateCharacters(read('content/characters.json'))
...
const charsById = byId(characters)
...
describe('demo 工程:真实 JSON 迁移保真 + buildWorld 端到端', () => {
  ...
  test('buildWorld:李逍遥实例值 = 迁移前 initialWorld() 真值', () => {
    const w = buildWorld(manifest.startWorld, charsById)
    const li = w.party[0]
  ...
  test('effectiveStat:叠装备后 = 迁移前 oracle(防御 32+9=41 / 武术 33+2=35)', () => {
    const li = buildWorld(manifest.startWorld, charsById).party[0]
