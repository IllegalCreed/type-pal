...
function loadP6Fixture() {
  ...
  const p6 = buildP6ScriptMigrationIR({
    frozenAudit: frozen,
    p5: p5.ir,
    p5Ledger: p5.ledger,
  })
...
describe.skipIf(!existsSync(extracted))('N3 P6 PAL shared closure shadow migration', () => {
  beforeAll(() => {
    fixture = loadP6Fixture()
  }, 180_000)

  test('31 pending bodies close with complete tail classification and body conservation', () => {
    const report = validateP6ScriptMigrationIR({
      frozenAudit: fixture.frozen,
      p5: fixture.p5.ir,
      p5Ledger: fixture.p5.ledger,
      ir: fixture.p6.ir,
  ...
  test('local calls inline with scheduling evidence and tail transfers stay explicit', () => {
    ...
    expect(fixture.p6.ir.localFlows.filter((flow) => flow.entry === 'call-inline')).toHaveLength(34)
    expect(
  ...
  test('author edits and forged shared closure targets fail closed with zero writes', () => {
    ...
    const forged = JSON.parse(JSON.stringify(fixture.p6.ir)) as typeof fixture.p6.ir
