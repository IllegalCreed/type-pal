// 装备面板 Canvas UI(D17)。布局取自一阶段 draw-inventory.ts(sdlpal itemmenu.c PAL_ItemSelectMenu)。
// 列表只含可装物(equippableItems 已过滤,无原版红色禁用项)。320 逻辑坐标,调用方已 ctx.scale。
...
/** 大世界装备面板(单人):可装列表(3 列网格 + 数量)+ 选中物详情(框/图标/描述)。 */
