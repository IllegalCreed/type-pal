...
function activeEntity(
  ...
  options: LegacyScriptHostAdapterOptionsV5,
): string | undefined {
  return target?.scene === options.currentSceneId() ? target.entity : undefined
}

/**
 * 把 v5 leaf 的画面/音频/战斗外壳副作用交给现有 ScriptHost。
 * canonical world-state 已由 ProjectScriptRuntimeHostV5 先行处理；这里不持有第二份真值。
 */
export async function executeLegacyScriptHostEffectV5(
  host: ScriptHost,
  ...
  switch (command.kind) {
    ...
    default: {
      ...
      throw new Error(
        `ScriptProjectRuntimeV5: 未实现 leaf ${(unhandled as RuntimeLeafCommandV5).kind}`,
      )
    }
  }
}
