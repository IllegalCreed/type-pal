...
describe('D6 三处 await 分别 abort（真实在途取消 + 进入见证 + 同步结局观察）', () => {
  ...
  async function sequenceAbort(verifyBlocked: (outcome: string) => void, trace: string[] = []) {
    ...
    const slow = new FrameSequenceReader(
      {
        readBytes: async () => {
          enteredRead = true
          trace.push('read-entered')
          const bytes = await readGate
          trace.push('read-completed')
          return bytes
        },
      },
      identity,
    ...
    try {
      ...
      controller.abort()
      await tick() // 排空拒绝传播
      verifyBlocked(observed.outcome) // 同步断言：底层未放行时外层已结束
    } finally {
      // 进入见证或主断言失败也必须结束实际任务；这里不放断言覆盖最初错误。
      controller.abort()
      releaseRead?.(arrayBufferOf(slowBytes))
      await tick()
      await tick()
      settled = await pendingSlow.then(
        () => 'fulfilled',
        (error: unknown) => (error as Error).name,
      )
      trace.push(`playback-consumed:${settled}`)
    }
    return { slow, frames, settled }
  }
  test('sequence 在途 abort：真实 entered 后取消即拒绝；finally 释放同一底层并消费原播放，迟到零提交', async () => {
    const { slow, frames, settled } = await sequenceAbort((outcome) =>
  ...
  test('sequence 主断言失败仍释放实际读取并消费原播放，保留最初断言错误', async () => {
    const trace: string[] = []
    let originalFailure: unknown
    const outcome = await sequenceAbort(() => {
      try {
        expect('injected failure').toBe('expected success')
      } catch (error) {
    ...
    expect(outcome).toBe(originalFailure)
    expect(trace).toEqual(['read-entered', 'read-completed', 'playback-consumed:AbortError'])
  })
