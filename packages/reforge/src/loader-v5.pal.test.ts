...
test('正式 PAL contentVersion 11 工程通过 loader、历史 sidecar 验签与全场景校验', async () => {
  const project = await loadProjectV5From(projectFileSource('pal'))
  const scenes = await loadAllScenesV5(project)

  expect(project.manifest.contentVersion).toBe(11)
...
])('仓库 HTTP fixture $id 已同步为 canonical content 11', async ({ id, entry, scenes: count }) => {
