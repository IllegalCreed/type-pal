...
describe('装备菜单状态机', () => {
  ...
  test('equipApply:满槽换装 → 留 pick-role + 选中变换下的旧件(可空格来回切换对比)', () => {
    ...
    const s1 = equipConfirmItem(openEquipMenu(w0, 'li-xiaoyao')) // 选 267
    ...
    const r2 = equipApply(state, world) // 再 Space → 换回
  ...
  test('equipApply:空槽换装 → 回 list(原版 wLastUnequippedItem==0)', () => {
    ...
    const s1 = equipConfirmItem(openEquipMenu(world, 'li-xiaoyao'))
    const r = equipApply(s1, world)
  ...
  test('equipBackToList:pick-role → list(重算背包),清选中', () => {
    ...
    const s2 = equipBackToList(s1, w0)
