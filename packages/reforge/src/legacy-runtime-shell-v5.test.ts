...
describe('legacy runtime shell v5', () => {
  test('projects canonical page/behavior/hook selection without copying script bodies', () => {
    ...
    refreshLegacySceneBindingsV5(legacy, scene(), world)
  ...
  test('adapts stable shared and item-private refs only inside the runtime shell', () => {
    ...
    const legacy = legacyItemsFromV5(items)
    ...
    expect(isV5RuntimeScriptRef(shared.script)).toBe(true)
  ...
  test('flattens only the active scene into a non-persistent scratch view', () => {
    ...
    expect(legacyWorldScriptScratchV5(world, 's001')).toMatchObject({
      flags: { done: true },
      entityState: { e001: 2 },
      entityStage: {},
