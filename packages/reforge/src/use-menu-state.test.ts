...
describe('使用菜单状态机', () => {
  test('openUseMenu:pick-item,列出可用物 + initialCursor 恢复光标(越界 clamp)', () => {
    ...
    const s = openUseMenu(w)
    expect(s.active).toBe(true)
    expect(s.phase).toBe('pick-item')
    expect(s.items.length).toBe(3)
    expect(openUseMenu(w, 2).cursor).toBe(2) // 恢复上次光标
    expect(openUseMenu(w, 99).cursor).toBe(2) // 越界 clamp 到末项(共 3)
  })
  test('useConfirm:单体(观音符)→ pick-target;脚本类(土灵珠)→ 直接执行不选目标', () => {
    const w = initialWorld()
    const oneAlly = useConfirm(useMoveCursor(openUseMenu(w), 'right'), w) // idx1 观音符
    ...
    const script = useConfirm(openUseMenu(w), w) // idx0 土灵珠(scene/triggerScript)
