...
export async function loadRunnableProjectFrom(source: FileSource): Promise<RunnableProject> {
  ...
  if (manifest.contentVersion === CONTENT_VERSION) return loadProjectV5From(source)
  const project = typeof manifest.id === 'string' ? `工程 "${manifest.id}"` : '工程'
  throw new Error(
    `${project}: runtime 只接受 contentVersion ${CONTENT_VERSION} 或 13，收到 ${String(manifest.contentVersion)}`,
  )
}
