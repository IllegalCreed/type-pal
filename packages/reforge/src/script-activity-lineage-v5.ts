import type { FlowActivityLeaseV5, FlowRuntimeCoordinatorV5 } from './script-world-v5.js'
 ...
 */
export async function withRegisteredScriptActivityLineageV5<T>(
  runtimeKey: object,
  signal: AbortSignal,
  body: () => T | Promise<T>,
): Promise<T> {
 ...
 */
export async function withScriptActivityLineageV5<T>(
  runtimeKey: object,
  ...
  let activity: FlowActivityLeaseV5 | undefined = coordinator.beginActivity()
  ...
  try {
    return await withRegisteredScriptActivityLineageV5(runtimeKey, signal, body)
  } finally {
    activity.close()
  }
}
