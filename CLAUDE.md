...
- **硬性质量零诊断（用户2026-09-27）**：lint/格式/typecheck中的error、warning、info全部清零才可验收。
  ...
  warning/info/跳过/截断都会非零退出；既有存量清理单列，不把责任转给无产品写权限的贡献者。
...
```bash
pnpm check          # docs + coverage/quality-tools + 各包 typecheck/test + 严格零诊断lint
pnpm check:docs     # 文档链接、目录索引、任务状态与现行版本检查
pnpm lint           # 全仓 Biome 零诊断硬门（error/warning/info/截断均拒绝）
pnpm test:quality-tools # 零诊断判据与真实 Biome 进程回归
...
- Static quality acceptance requires zero error/warning/info diagnostics, not just exit 0. Existing lint debt is not waived.
  Cleanup and machine-gate tightening are complete: `docs/ops/archive/tasks/done/QUALITY-ZERO-1-static-diagnostics.md`;
